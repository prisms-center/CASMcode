...
TEST(ExampleCrystallographySite, SiteConstructor) {
  ...
  //   information on the degrees of freedom (DoF) allowed on a particular sublattice of the
  //   infinte crystal.
  //
  // The allowed DoF may be:
  // - discrete "occupants", represented using Molecule
  // - continuous valued properties (displacement, magspin, etc.), represented using SiteDoFSet.
  //
  // Molecules are composed of atoms, which may be isotropic, or may have properties which are
  //    anisotropic.
  //
  //      Molecules have:
  //      - a name string
  ...
  //     tolerance.
  //
  //   The function `Molecule::make_atom` constructs a Molecule with a single isotropic atom. The molecule's
  //     name and the atom's name are equivalent.
  //
  //   The function `Molecule::make_vacancy` constructs an atom-like Molecule with the special name "Va" to
  //     indicate that it represents a vacancy.
  //
  // SiteDoFSet represent continuous degrees of freedom (DoF) which are "local" to a particular
  ...
  Site site_1 {
    Coordinate { Vector3d {0., 0., 0.}, lattice, CASM::FRAC },
  ...
  // Construct a Site with coordinate, fixed occupation, and displacement (explicit types)
  Site site_2 {
    Coordinate { Eigen::Vector3d {0., 0., 0.}, lattice, CASM::FRAC },
    std::vector<Molecule> { make_atom("A") },
    std::map<std::string, SiteDoFSet> { {disp_dofset.type_name(), disp_dofset} } };
