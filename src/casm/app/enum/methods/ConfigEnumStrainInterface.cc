...
std::string ConfigEnumStrainInterface::desc() const {
  ...
  std::string custom_options =
      ...
      "    Note: \n"
      "    - If some \"qi\" in the range [1, DoF space dimension] are missing, "
      "then enumeration \n"
      ...
      "    If true, any grid points outside the largest ellipsoid inscribed "
      "within the extrema\n"
...
struct MakeEnumerator {
  MakeEnumerator(ConfigEnumStrainParams const &_params,
                 Eigen::MatrixXd const &_axes, bool _make_symmetry_adapted_axes,
...
void parse(InputParser<ConfigEnumStrainParams> &parser,
           ConfigEnumInput const &initial_state, Eigen::MatrixXd &axes,
...
void ConfigEnumStrainInterface::run(
  ...
  // axes: column vector matrix (user input, else Identity matrix of dimension
  ...
  auto params_parser_ptr = parser.parse_as<ConfigEnumStrainParams>(
      named_initial_states[0].second, axes, sym_axes_option);
  report_and_throw_if_invalid(parser, log, error_if_invalid);
  ...
  ConfigEnumStrainInterface_impl::MakeEnumerator make_enumerator_f{
      params, axes, sym_axes_option, dof_space_output};
