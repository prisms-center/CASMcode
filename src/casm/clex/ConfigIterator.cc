...
namespace CASM {
  ...
  */
  ...
  Transition &ConfigIterator<Transition, PrimClex>::operator*() {
    return m_primclex->supercell(m_scel_index).transition(m_config_index);
  }
  ...
  */
  ...
  void ConfigIterator<Configuration, PrimClex>::set_selected(bool _select) {
    m_primclex->supercell(m_scel_index).config(m_config_index).set_selected(_select);
  }
  ...
  int ConfigIterator<Configuration, PrimClex>::config_list_size() const {
    return m_primclex->supercell(m_scel_index).config_list().size();
  }
  ...
  int ConfigIterator<Transition, PrimClex>::config_list_size() const {
    return m_primclex()->supercell(m_scel_index).transition_list().size();
  }
