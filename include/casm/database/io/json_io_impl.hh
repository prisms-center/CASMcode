...
/// \param name_key Read object names from kwargs[name_key]. Expects array of
/// string. \param sel_key Read selection name for kwargs[sel_key]. Expect
/// string. \param on_error Indicates how to handle names that do not exist in
/// database. OnError::WARN is not allowed and treated as OnError::THROW.
///
/// Notes:
/// - Reads selection from 'sel_key' first, using "NONE" if 'sel_key' does not
///   exist. Then reads names from 'name_key' array and adds them to the
///   selection.
/// - If 'name_key' and 'sel_key' do not exist return "NONE" selection.
///
template <typename DataObject>
Selection<DataObject> make_selection(Database<DataObject> &db,
                                     const jsonParser &kwargs,
                                     std::string name_key, std::string sel_key,
                                     OnError on_error) {
