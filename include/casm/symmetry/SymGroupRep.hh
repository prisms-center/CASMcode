...
class SymGroupRep : public std::vector<SymOpRepresentation *> {
 public:
  ...
  /// SymGroupRep::NO_HOME is safeguard to prevent usage of this constructor
  /// when full constructor should be used
  ...
  /// Necessary because SymGroupRep directly manages heap-allocated
  /// SymOpRepresentations
  ...
  /// \brief Reference to MasterSymGroup for which this SymGroupRep is a group
  /// representation If a MasterSymGroup contains this SymGroupRep, it must be
  /// the master_group() of this SymGroupRep A SymGroupRep that is not contained
  /// in a MasterSymGroup may (and often does) still have a valid master_group()
  ...
  /// \brief Returns true if this SymGroupRep has valid pointer to a
  /// MasterSymGroup
  ...
  /// \brief Sets the representation of operation at entry 'op_index' of this
  /// group representation Throws if this group representation already contains
  /// entry at 'op_index' \param op_index Index of group operation to be set
  /// \param new_rep New representation to be recorded in this SymGrouprep for
  /// operation at entry 'op_index' of master_group()
  void set_rep(Index op_index, const SymOpRepresentation &new_rep);

  /// \brief Sets the representation of operation at entry 'base_rep.index()' of
  /// this group representation Equivalent to
  /// this->set_rep(base_rep.index(),new_rep) Throws if this group
  /// representation already contains entry at 'base_rep.index()' \param
  /// base_rep Fully initialized SymOpRepresentation that has valid
  /// 'base_rep.index()' value \param new_rep New representation to be recorded
  /// in this SymGroupRep for operation at entry 'base_rep.endex()' of
  /// master_group()
  void set_rep(const SymOpRepresentation &base_rep,
               const SymOpRepresentation &new_rep);

  /// \brief Sets the representation of operation at entry 'base_rep.index()' of
  /// this group representation Throws if this group representation already
  /// contains entry at 'base_rep.index()' If base_rep.index() is invalid, it
  /// will attempt to find base_rep in the master_group() to resolve index
  /// \param base_rep SymOp corresponding to an operation in master_group()
  /// \param new_rep New representation to be recorded in this SymGroupRep for
  /// operation at entry 'base_rep.endex()' of master_group()
  ...
  /// \brief pointer to MatrixXd corresponding to SymOpRepresentation at entry
  /// 'i' of this SymGroupRep Returns nullptr if entry 'i' is uninitialized or
  /// if entry 'i' cannot be represented as a Matrix
  Eigen::MatrixXd const *MatrixXd(Index i) const;

  /// \brief pointer to MatrixXd corresponding to SymOpRepresentation at entry
  /// '_op.index()' of this SymGroupRep Returns nullptr if entry '_op.index()'
  /// is uninitialized or if entry '_op.index()' cannot be represented as a
  /// Matrix
  Eigen::MatrixXd const *MatrixXd(const SymOpRepresentation &_op) const;

  /// \brief pointer to Permutation corresponding to SymOpRepresentation at
  /// entry 'i' of this SymGroupRep Returns nullptr if entry 'i' is
  /// uninitialized or if entry 'i' cannot be represented as a Permutation
  Permutation const *permutation(Index i) const;

  /// \brief pointer to Permutation corresponding to SymOpRepresentation at
  /// entry '_op.index()' of this SymGroupRep Returns nullptr if entry
  /// '_op.index()' is uninitialized or if entry '_op.index()' cannot be
  /// represented as a Permutation
  Permutation const *permutation(const SymOpRepresentation &_op) const;

  /// \brief Returns SymGroupRepID of this SymGroupRep in its master_group. If
  /// this is not contained in a master_group, symrep_ID().empty()==true
 ...
 private:
  ...
  /// Pointer version of constructor is private for internal construction of
  /// master-less representations
  SymGroupRep(const MasterSymGroup *_home) : m_master_group(_home) {}
};

/// \brief Make a copy of representation on vector space 'V' that is transformed
/// into a representation on vector space 'W' 'trans_mat' is the unitary matrix
/// that isomorphically maps 'V'->'W' (i.e., [w = trans_mat * v] and [v =
/// trans_mat.transpose() * w] ) If the original representation to be
/// transformed is just a temporary standalone SymGroupRep, be sure to delete it
/// before falling out of scope
...
/// SymGroupRepHandle is used to provide easy remote access to a SymGroup
/// representation The head group may be a subgroup of the MasterSymGroup where
/// the SymGroupRep is stored.
