...
namespace {
  ...
  ///
  /// Note:
  /// - The properties "relaxed_mag_basis" and "relaxed_magmom" are no longer supported. Now
  ///   magnetic properties are supported with the "magspin" flavors (see AnisoValTraits
  ///   documentation). If they are encountered they are silently ignored.
  ...
  static void _from_json_legacy(xtal::SimpleStructure &simple_structure,
    ...
    try {
      ...
      simple_structure.lat_column_mat = json["relaxed_lattice"].get<Eigen::Matrix3d>().transpose();
      ...
      if(json.contains("relaxed_energy")) {
        simple_structure.properties["energy"] = json["relaxed_energy"].get<Eigen::MatrixXd>();
      }
      if(json.contains("relaxed_forces")) {
        simple_structure.atom_info.properties["force"] = json["relaxed_forces"].get<Eigen::MatrixXd>().transpose();
      }
      // note: silently ignoring legacy "relaxed_mag_basis" & "relaxed_magmom"

      // Read atom types
      std::vector<Index> atoms_per_type = json["atoms_per_type"].get<std::vector<Index> >();
      std::vector<std::string> atom_type = json["atom_type"].get<std::vector<std::string> >();
      ...
      simple_structure.atom_info.coords =
        to_cartesian_matrix * json["relaxed_basis"].get<Eigen::MatrixXd>().transpose();
  ...
  ///
  /// Note: "permute" enables skipping species that are excluded from output. It is populated with
  /// indices of sites that should be included.
  static jsonParser &_types_to_json(std::vector<std::string> const &names,
    ...
    for(Index i = 0; i < names.size(); ++i) {
      if(excluded_species.count(names[i]))
        continue;
  ...
  ///
  /// Note: "permute" enables skipping species that are excluded from output. It includes only
  /// columns of the coords matrix that should be included.
  static jsonParser &_coords_to_json(Eigen::MatrixXd const &coords,
                                     jsonParser &json,
                                     std::string field_name,
    ...
    for(Index i : permute) {
      tjson.push_back(to_coord_mode_matrix * coords.col(i), jsonParser::as_array());
    }
    return json;
  }
  ...
  /// Note: "permute" enables skipping species that are excluded from output. It includes only
  /// columns of the coords matrix that should be included.
  static jsonParser &_properties_to_json(std::map<std::string, Eigen::MatrixXd> const &properties,
                                         jsonParser &json,
                                         std::string field_name,
                                         std::vector<Index> const &permute) {
  ...
  static void _coords_from_json(Eigen::MatrixXd &coords,
                                jsonParser const &json,
                                std::string field_name,
                                Eigen::Matrix3d const &to_cartesian_matrix) {
    if(json.contains(field_name)) {
      coords = to_cartesian_matrix * json[field_name].get<Eigen::MatrixXd>().transpose();
    }
  }


  /// Read SimpleStructure global, atom_info, and mol_info properties from JSON
  static void _properties_from_json(std::map<std::string, Eigen::MatrixXd> &properties,
                                    jsonParser const &json,
    ...
    for(std::string const &field : allowed_field_names) {
      ...
      if(it != json.end()) {
        for(auto it2 = it->begin(); it2 != it->end(); ++it2) {
          properties[it2.name()] = (*it2)["value"].get<Eigen::MatrixXd>().transpose();
        }
      }
    }
  }
  ...
  /// \param json A JSON object, from which the xtal::SimpleStructure JSON is read. Expects JSON
  ...
  static void _from_json_current(xtal::SimpleStructure &simple_structure,
    ...
    COORD_TYPE coordinate_mode = json["coord_mode"].get<COORD_TYPE>();
    simple_structure.lat_column_mat = json["lattice"].get<Eigen::Matrix3d>().transpose();
    ...
    ::_properties_from_json(simple_structure.properties, json, {"global_dofs", "global_vals"});

    // Read atom_info (if exists)
    auto &atom_info = simple_structure.atom_info;
    ::_types_from_json(atom_info.names, json, "atom_type");
    ::_coords_from_json(atom_info.coords, json, "atom_coords", to_cartesian_matrix);
    ::_properties_from_json(atom_info.properties, json, {"atom_dofs", "atom_vals"});
  ...
  static void _check_sizes(xtal::SimpleStructure::Info const &info, std::string info_type) {
    ...
    for(auto const &property_pair : info.properties) {
      if(info.names.size() != property_pair.second.cols()) {
        std::stringstream ss;
        ss << info_type << ".properties[" << property_pair.first << "\"].cols(): "
           << property_pair.second.cols()
           << " != " << info_type << ".names.size(): " << info.names.size();
        throw libcasm_runtime_error(ss.str());
      }
    }
...
namespace CASM {
  ...
  /// \param json A JSON object, into which the xtal::SimpleStructure JSON will output. This function
  ///        will overwrite existing attributes with the same name, but will not erase any other
  ///        existing attributes.
  /// \param excluded_species Names of any molecular or atomic species that should not be included
  ///        in the output
  ...
  jsonParser &to_json(xtal::SimpleStructure const &simple_structure,
    ...
    // Note: _types_to_json checks excluded_species and populates "atom/mol_permute" with indices of
    //   sites that are included. This is used to only output the coordinates and properties of the
    //   correct sites in _properties_to_json and _coords_to_json

    // Output mol_info
    auto const &atom_info = simple_structure.atom_info;
    std::vector<Index> atom_permute;
    ::_types_to_json(atom_info.names, json, "atom_type", excluded_species, atom_permute);
    ::_coords_to_json(atom_info.coords, json, "atom_coords", atom_permute, to_coord_mode_matrix);
    ...
    ::_types_to_json(atom_info.names, json, "mol_type", excluded_species, mol_permute);
    ::_coords_to_json(mol_info.coords, json, "mol_coords", mol_permute, to_coord_mode_matrix);
  ...
  /// \param simple_structure xtal::SimpleStructure to read from JSON
  /// \param json A JSON object, from which the xtal::SimpleStructure JSON is read. Expects JSON
  ///        formatted either i) as documented for `to_json` for xtal::SimpleStructure or ii) if the
  ///        "atoms_per_type" attribute exists, using the legacy `properties.calc.json` format as
  ///        documented for `from_json_legacy`.
  ///
  /// Note:
  /// - This is the expected format for `properties.calc.json`
  ///
