...
struct Checks {
  Checks(std::string _type, std::string _no_proj_command,
...
TEST(AppTest, ProjectCommands) {
  ...
  std::vector<Checks> command = {
      Checks("composition", "-d", "-h"), Checks("sym", "", "-h"),
      Checks("bset", "", "-h"),          Checks("enum", "", "-h"),
      Checks("select", "", "-h"),        Checks("query", "", "-h")};
  ...
  for (auto it = command.begin(); it != command.end(); ++it) {
    p.popen(autotools::abs_ccasm_path() + " " + it->type + " " +
            it->no_proj_command);
