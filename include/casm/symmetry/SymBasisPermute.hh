...
/// - The result of transforming (b,0,0,0) is stored in
/// SymBasisPermute::data()[b]
...
///   rep.data()[u.sublattice()].sublattice(); \endcode
/// - Additional translations, such as those in supercell factor group
/// operations,
///   may be stored in SymOp::integral_tau(). Those additional contributions
///   can be included with:
///   - u.unitcell() += (prim.lattice().inv_lat_column_mat() *
///   op.integral_tau()).cast<long>();
///
class SymBasisPermute : public SymOpRepresentation {
 ...
 public:
  ...
  size_type size() const { return m_ucc_permute.size(); }

  /// Return UnitCellCoord that (b,0,0,0) transforms to
  UnitCellCoord operator[](Index b) const { return m_ucc_permute[b]; }

  /// Return UnitCellCoord that (b,0,0,0) transforms to
  UnitCellCoord at(Index b) const { return m_ucc_permute.at(b); }

  /// Get this from a SymOp
  SymBasisPermute const *ucc_permutation() const override { return this; };

  /// Get underlying data (data()[b] is the result of transforming (b,0,0,0))
  const std::vector<UnitCellCoord> &data() const { return m_ucc_permute; }

  /// Get underlying integer transformation amtrix
  const Eigen::Matrix3l &matrix() const { return m_point_mat; }
};
