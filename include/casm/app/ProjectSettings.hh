...
/// Contains:
/// - The project name
/// - Parameters that control which basis sites are included in the neighborlist
/// and the order of
...
/// - List of cluster expansions (via a vector of ClexDescription, which is key
/// type that allows
...
///
/// Also contains:
/// - EnumeratorHandler: a dictionary-like class for holding standard
/// enumeration methods and plugins
/// - HamiltonianModules: a dictionary-like class for holding standard and
/// (todo) custom
///    traits for anisotropic value types, dof type, and symmetry
///    representations
/// - QueryHandler: a dictionary-like class for holding query functors, plugins,
/// and aliases for
///    querying object properties and formatted output
///
/// Optionally contains:
/// - The associated project root directory path and a DirectoryStructure object
/// which,
///   if present, allows creating project directories
///
/// Compiler and linker settings:
/// - For easier debugging, each setting is returned as a pair of value and the
/// source where the value was obtained from. This could be "project_settings"
/// for values set explicitly in this object or read from the project settings
/// JSON file, or it could be an environment variable, or it could be a default
/// value.
/// - The environment variables checked and default values are: (using
/// implementation in RuntimeLibrary.cc)
///   - cxx: default="g++", check for environment variables: `CASM_CXX`, `CXX`
///   - cxxflags: default="-O3 -Wall -fPIC --std=c++17", check for environment
///   variable: `CASM_CXXFLAGS`
///   - soflags: default="-shared -lboost_system", check for environment
///   variable: `CASM_SOFLAGS`
///   - casm_includedir: (where to find the "casm" headers directory tree)
///     - default=(attempts to find in standard locations relative to the
///     `ccasm` executable),
///       - for example: if <prefix>/bin/ccasm exists then check
///       <prefix>/include/casm
...
///     `ccasm` executable),
///       - for example: if <prefix>/bin/ccasm exists then check
///       <prefix>/{lib,lib64,lib/x86_64-linux-gnu}libcasm.{dylib,so}
///     - check for environment variables:
///       - `CASM_LIBDIR`
...
/// - Prior to v0.4: JSON file attribute "curr_properties" was expected to be an
/// array of string
///   containing properties that must exist in properties.calc.json for a
///   configuration to be
...
class ProjectSettings {
 public:
  ...
  ///
  /// \param project_name Name of new CASM project. Use a short title suitable
  /// for prepending to file names.
  ///
  explicit ProjectSettings(std::string project_name);

  /// Construct CASM project settings for a new project (to be persisted on
  /// disk)
  ///
  /// \param project_name Name of new CASM project. Use a short title suitable
  /// for prepending to file names. \param root Path to new CASM project
  /// directory
  ///
  ...
  /// required_properties is a map of properties names required for a
  /// calculation to be complete:
  ///   traits<Configuration>::name -> calctype -> {propname1, propname2, ...}
  typedef std::map<ObjectTypeName,
                   std::map<CalcTypeName, std::vector<std::string>>>
  ...
  /// calculation to be complete:
  ///   traits<ObjectType>::name -> calctype -> {propname1, propname2, ...}
  void set_required_properties(
      required_properties_map_type const &_required_properties);
  ...
  std::vector<std::string> const &required_properties(
      std::string type_name, std::string calctype) const;
  ...
  /// \param calctype string
  void set_required_properties(
      std::string type_name, std::string calctype,
  ...
  /// Insert a ClexDescription into ProjectSettings. Fails if existing
  /// ClexDescription with same name
  ...
  /// Set default ClexDescription. Will overwrite existing ClexDescription with
  /// same name
  ...
  /// Combines cxx, cxxflags, casm_includedir, and boost_includedir to make
  /// compiler options string
  std::string compile_options() const;

  /// Combines cxx, soflags, casm_libdir, and boost_libdir to make shared
  ...
  /// query_alias is a map by object type name of query alias {name,value}
  /// pairs:
  ///   traits<Configuration>::name -> {query alias name, query alias value}
  typedef std::map<ObjectTypeName, std::map<QueryAliasName, QueryAliasValue>>
      query_alias_map_type;
  ...
  /// pairs:
  ///   traits<Configuration>::name -> {query alias name, query alias value}
  void set_query_alias(std::string type_name, std::string alias_name,
  ...
  /// Set neighbor list weight matrix (will delete existing Clexulator source
  /// and compiled code)
  ///
  /// Note:
  /// - Clexulators are dependent on the neighbor list parameters. If you
  /// changes these parameters
  ///   you must regenerate all clexulators. This is no longer done by this
  ///   function.
 ...
 private:
  ...
  std::map<std::string, notstd::cloneable_ptr<notstd::Cloneable>>
      m_query_handler;
...
struct jsonConstructor<ProjectSettings> {
  ...
  ///
  /// Note:
  /// - After construction, root dir is not set (`ProjectSettings::has_dir() ==
  /// false`)
...
void write_project_settings(ProjectSettings const &set,
...
/// - `path_in_project` does not need to be a root dir exactly, it may be
/// anywere inside a CASM project
/// - Throws if `path_in_project` is not inside any CASM project or reading
/// fails
