...
void ViewOption::initialize() {
  ...
  m_desc.add_options()("images,i", po::value<int>(&m_images)->default_value(0),
                       "Number of images between initial and final state when "
                       "viewing diff_trans_configs.")(
      "relaxed,r",
      "Attempt to display corresponding relaxed structures to the given "
      "configurations.");
...
int view_command(const CommandArgs &args) {
  ...
  try {
    po::store(po::command_line_parser(args.argc() - 1, args.argv() + 1)
                  .options(view_opt.desc())
                  .positional(p)
                  .run(),
              vm);
    // po::store(po::parse_command_line(argc, argv, view_opt.desc()), vm); //
    // can throw
    ...
    err_log() << view_opt.desc() << std::endl;
  ...
  if (set.view_command().empty()) {
    err_log() << "Error in 'casm view': No command set. Use 'casm settings "
                 "--set-view-command' to set the command to open visualization "
                 "software. It should take one argument, the path to a POSCAR "
                 "to be visualized. For example, to use VESTA on Mac: casm "
                 "settings --set-view-command 'casm.view \"open -a "
                 "/Applications/VESTA/VESTA.app\"'.\n";
  ...
  for (const auto &config : config_select.selected()) {
    if (vm.count("relaxed") && is_calculated(config)) {
      ...
      log() << "Obtaining relaxed structure from:\n";
      ...
      file.close();
      if (vm.count("relaxed")) {
        log() << "No relaxed structure found."
              << "\n";
      }
      log() << config.name() << ":\n";
      Popen popen;
