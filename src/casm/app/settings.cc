...
namespace {
std::string _wdefaultval(std::string name,
                         std::pair<std::string, std::string> val) {
  return name + ": '" + val.first + "' (" + val.second + ")\n";
}

std::string _wdefaultval(std::string name,
                         std::pair<fs::path, std::string> val) {
...
const std::string &SettingsOption::input_str() const { return m_input_str; }
...
void SettingsOption::initialize() {
  ...
  m_desc.add_options()("list,l", "List project settings")(
      "new-property", po::value<std::string>(&m_input_str),
      "Create cluster expansions for a new property")(
      "new-bset", po::value<std::string>(&m_input_str),
      "Create a new basis set")("new-calctype",
                                po::value<std::string>(&m_input_str),
                                "Create a new calculation type")(
      "new-ref", po::value<std::string>(&m_input_str),
      "Create a new calculation reference")(
      "new-eci", po::value<std::string>(&m_input_str),
      "Create a new set of effective cluster interactions (ECI)")(
      "set-formation-energy", po::value<std::string>(&m_input_str),
      "Specify the cluster expansion to use for formation energy")(
      "new-clex", po::value<std::string>(&m_input_str),
      "Create a new cluster expansion")(
      "set-default-clex",
      po::value<std::string>(&m_input_str)->value_name(ArgHandler::clex()),
      "Set the cluster expansion that CASM uses or acts on by default")(
      "erase-clex",
      po::value<std::string>(&m_input_str)->value_name(ArgHandler::clex()),
      "Erase the specified cluster expansion. Does not erase underlying bset, "
      "eci, etc.")("clex",
                   po::value<std::string>()->value_name(ArgHandler::clex()),
                   "The cluster expansion for which to set property, bset, "
                   "calctype, ref, or eci")(
      "set-property",
      po::value<std::string>(&m_input_str)->value_name(ArgHandler::property()),
      "Set the property being cluster expanded")(
      "set-bset",
      po::value<std::string>(&m_input_str)->value_name(ArgHandler::bset()),
      "Set the basis set")(
      "set-calctype",
      po::value<std::string>(&m_input_str)->value_name(ArgHandler::calctype()),
      "Set the calculation type")(
      "set-ref",
      po::value<std::string>(&m_input_str)->value_name(ArgHandler::ref()),
      "Set the calculation reference")(
      "set-eci",
      po::value<std::string>(&m_input_str)->value_name(ArgHandler::eci()),
      "Set the effective cluster interactions (ECI)")(
      "set-all",
      po::value<std::vector<std::string> >(&m_input_vec)->multitoken(),
      "Set the current property, calctype, ref, bset, and eci all at once.")(
      "set-view-command", po::value<std::string>(&m_input_str),
      "Set the command used by 'casm view'.")(
      "set-view-command-video", po::value<std::string>(&m_input_str),
      "Set the command used by 'casm view' for videos.")(
      "set-cxx", po::value<std::string>(&m_input_str),
      "Set the c++ compiler. Use '' to revert to default.")(
      "set-cxxflags", po::value<std::string>(&m_input_str),
      "Set the c++ compiler options. Use '' to revert to default.")(
      "set-soflags", po::value<std::string>(&m_input_str),
      "Set the shared library compilation options. Use '' to revert to "
      "default.")("set-casm-prefix", po::value<std::string>(&m_input_str),
                  "Set the casm prefix. Use '' to revert to default.")(
      "set-casm-includedir", po::value<std::string>(&m_input_str),
      "Set the casm includedir. Use '' to revert to default.")(
      "set-casm-libdir", po::value<std::string>(&m_input_str),
      "Set the casm libdir. Use '' to revert to default.")(
      "set-boost-prefix", po::value<std::string>(&m_input_str),
      "Set the boost prefix. Use '' to revert to default.")(
      "set-boost-includedir", po::value<std::string>(&m_input_str),
      "Set the boost includedir. Use '' to revert to default.")(
      "set-boost-libdir", po::value<std::string>(&m_input_str),
...
struct Data {
  ...
  Data(PrimClex *_primclex, DirectoryStructure const &_dir,
       ProjectSettings &_set, ClexDescription &_desc)
      : primclex(_primclex),
        dir(_dir),
        set(_set),
        desc(_desc),
        log(CASM::log()),
        err_log(CASM::err_log()),
  ...
  int update() {
    ...
    ClexDescription tdesc(desc.name, property.first, calctype.first, ref.first,
                          bset.first, eci.first);
    ...
    bool res =
        ...
        try_new(
            "eci", eci,
            [&]() {
              return contains(dir.all_eci(tdesc.property, tdesc.calctype,
                                          tdesc.ref, tdesc.bset),
                              tdesc.eci);
            },
            [&]() {
              return dir.new_eci_dir(tdesc.property, tdesc.calctype, tdesc.ref,
                                     tdesc.bset, tdesc.eci);
            });
    ...
    if (clex_exists(dir, tdesc) && set.set_default_clex(tdesc)) {
      ...
      if ((desc.property != tdesc.property) ||
          (desc.calctype != tdesc.calctype) || (desc.ref != tdesc.ref)) {
        read_chem_ref = true;
        read_configs = true;
      }

      bool clear_clex = false;
      if ((desc.property != tdesc.property) || (desc.bset != tdesc.bset) ||
          (desc.eci != tdesc.eci)) {
        clear_clex = true;
      }
      ...
      if (primclex) {
        primclex->refresh(read_settings, read_composition, read_chem_ref,
  ...
  /// if do_not_create and check_f():
  ///   return success;
  bool try_new(std::string set_name, pair_type set,
               std::function<bool()> check_f, std::function<bool()> new_f) {
    ...
    if (_cmode == do_not_create && !check_f()) {
      err_log << "Error: The " << set_name << " named '" << set.first
              << "' does not exist.\n";
      err_log << "  Check your input or use --new-" << set_name
              << " to create it first.\n";
...
int settings_command(const CommandArgs &args) {
  ...
  try {
    ...
    const po::options_description &desc =
        settings_opt.desc();  // I'm tired of fixing merge conflicts, this is
                              // ugly and should not stay like this.

    try {
      po::store(po::parse_command_line(args.argc(), args.argv(), desc),
                vm);  // can throw
      ...
      if (!vm.count("help")) {
        if (option_count == 0) {
          log() << "Error in 'casm settings'. No option selected." << std::endl;
          call_help = true;
        } else if (option_count > 1) {
          log() << "Error in 'casm settings'. Use one option (other than "
                   "--clex) at a time."
                << std::endl;
          call_help = true;
        }
      ...
      if (vm.count("desc")) {
        ...
        log()
            ...
            << "      - Specifies compiler to use. In order of priority: \n"
               ...
               "      - Specifies location to find CASM header files and "
               "shared\n"
               "        libraries for runtime compilation and linking.         "
               "\n"
               "        In order of priority: \n"
               "        1) User specified by via 'casm settings' (use '' to "
               "clear) \n"
               ...
               "      - Specifies location to find boost header files and "
               "shared\n"
               "        libraries for runtime compilation and linking.         "
               "\n"
               "        In order of priority: \n"
               ...
               "        3) $CASM_BOOST_PREFIX/include and "
            ...
            << "      casm settings --set-view-command-video 'casm.view \"open "
               "-a /Applications/Ovito/ovito.app\"'\n"
            << "      - Sets the command used by 'casm view' to open         \n"
            << "        visualization software.                              \n"
            << "      - Will be executed with '/path/to/POSCAR' as an        \n"
            << "        argument, the location of a POSCAR for a "
  ...
  if (root.empty()) {
    ...
    err_log() << "current_path: " << fs::current_path() << std::endl;
    err_log() << std::endl;
    return ERR_NO_PROJ;
  }
  ...
  if (vm.count("clex")) {
    ...
    if (it == set.cluster_expansions().end()) {
      ...
      err_log() << vm["clex"].as<std::string>()
                << " not found. expected basis set specifications file at: "
                << dir.bspecs(clex_desc.bset) << "\n"
                << std::endl;
  ...
  Data d(args.primclex, dir, set, clex_desc);

  typedef Data::pair_type pair_type;

  // disply all settings
  if (vm.count("list")) {
    print_summary(set, log());
    return 0;
  }
  ...
  // create new cluster expansion settings group
    ...
    if (set.insert_clex(clex_desc)) {
      log() << "Created new cluster expansion named '" << single_input
            << "'\n\n";
    } else {
      log() << "Could not create new cluster expansion named '" << single_input
            << "'.\n\n";
      return 1;
    }
    ...
    if (clex_exists(dir, clex_desc) && set.set_default_clex(clex_desc)) {
      ...
      log() << "Set '" << clex_desc.name
            << "' as default cluster expansion.\n\n";
      return 0;
    } else {
      log() << "Could not set '" << clex_desc.name
            << "' as default cluster expansion.\n\n";
      return 1;
    }
  }
  ...
  // erase cluster expansion settings group
    ...
    if (set.default_clex().name == single_input) {
      log() << "Coult not erase the cluster expansion named '" << single_input
            << "' "
    ...
    if (set.cluster_expansions().size() == 1) {
      log() << "Could not erase the cluster expansion named '" << single_input
            << "' "
    ...
    if (set.erase_clex(set.clex(single_input))) {
      ...
      log() << "Erased cluster expansion named '" << single_input << "'\n\n";
      return 0;
    } else {
      log() << "Could not erase cluster expansion named '" << single_input
            << "'.\n\n";
      return 1;
    }
  }
  ...
  // set clex to use by default for formation energy
    ...
    if (clex_desc.property != "formation_energy") {
      err_log() << "Attempting to use cluster expansion '" << clex_desc.name
    ...
    if (set.insert_clex(clex_desc)) {
      ...
      log() << "Now using cluster expansion '" << single_input
            << "' as the default for formation_energy.\n\n";
      return 0;
    } else {
      log() << "Could not use cluster expansion '" << single_input
            << "' as the default for formation_energy.\n\n";
      return 1;
    }
  }
  ...
  // set default clex
    ...
    if (set.set_default_clex_name(single_input)) {
      ...
      log() << "Switched to cluster expansion '" << single_input << "'.\n\n";
      return 0;
    } else {
      log() << "Could not switch to cluster expansion '" << single_input
            << "'.\n\n";
      return 1;
    }
  }
  ...
  // set all
    ...
    if (multi_input.size() != 5) {
      log() << "Error using --set-all: Expected 5 arguments: 'property' "
               "'calctype' 'ref' 'bset' 'eci'"
            << std::endl;
      return ERR_INVALID_ARG;
    }
  ...
  // set cxx
    ...
    log() << "Set " << _wdefaultval("cxx", set.cxx());
    log() << "Compile command is now: '" << set.compile_options() << "'\n\n";
    log() << "Shared object compile command is now: '" << set.so_options()
          << "'\n\n";
  ...
  // set cxxflags
    ...
    log() << "Set " << _wdefaultval("cxxflags", set.cxxflags());
    log() << "Compile command is now: '" << set.compile_options() << "'\n\n";
  ...
  // set soflags
    ...
    log() << "Set " << _wdefaultval("soflags", set.soflags());
    log() << "Shared object compile command is now: '" << set.so_options()
          << "'\n\n";
  ...
  // set casm prefix
    ...
    log() << "Set " << _wdefaultval("casm_includedir", set.casm_includedir());
    log() << "Set " << _wdefaultval("casm_libdir", set.casm_libdir());
    log() << "Compile command is now: '" << set.compile_options() << "'\n\n";
    log() << "Shared object compile command is now: '" << set.so_options()
          << "'\n\n";
  ...
  // set boost prefix
    ...
    log() << "Set " << _wdefaultval("boost_includedir", set.boost_includedir());
    log() << "Set " << _wdefaultval("boost_libdir", set.boost_libdir());
    log() << "Compile command is now: '" << set.compile_options() << "'\n\n";
    log() << "Shared object compile command is now: '" << set.so_options()
          << "'\n\n";
  ...
  // set 'casm view' command
    ...
    log() << "Set view command to: '" << set.view_command() << "'\n\n";
  ...
  // set 'casm view' command
    ...
    log() << "Set video view command to: '" << set.view_command_video()
          << "'\n\n";
