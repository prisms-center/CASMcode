...
namespace test {
  ...
  void expect_excluded_point_cluster(std::vector<Orbit<LocalSymCompare<IntegralCluster>>> const &orbits,
                                     UnitCellCoord const &unitcellcoord);

  void expect_included_point_cluster(std::vector<Orbit<LocalSymCompare<IntegralCluster>>> const &orbits,
                                     UnitCellCoord const &unitcellcoord,
                                     int multiplicity);
}
...
TEST_F(ClusterSpecsTest, ScelPeriodicTest) {
  ...
  EXPECT_EQ(scel_periodic_orbits[2].size(), 48);  // 2 elements per unit cell * 24 unit cells
  EXPECT_EQ(scel_periodic_orbits[3].prototype().size(), 2);
  EXPECT_EQ(scel_periodic_orbits[3].size(), 48);  // 2 elements per unit cell * 24 unit cells
  EXPECT_EQ(scel_periodic_orbits[4].prototype().size(), 2);
  EXPECT_EQ(scel_periodic_orbits[4].size(), 48);  // 2 elements per unit cell * 24 unit cells
}
...
TEST_F(ClusterSpecsTest, LocalTest) {
  ...
  //
  // Note: Use `PrimPeriodicSymCompare` here to include all factor group operation + translation
  //       combinations in the cluster's invariant group. The `spatial_transform` necessary for
  //       equivalence is included in the invariant subgroup operation.
  PrimPeriodicSymCompare<IntegralCluster> prim_periodic_sym_compare {shared_prim,
                                                                     shared_prim->lattice().tol()};
  auto phenomenal_cluster_group = make_invariant_subgroup(phenomenal_cluster,
                                                          shared_prim->factor_group(),
                                                          prim_periodic_sym_compare);

  // The value max_length[b], is the max site-to-site distance within a cluster for that cluster to
  // be included in branch b. The b==0 value is ignored.
  std::vector<double> max_length {
    0, // null cluster
    0  // point clusters
  };

  // For a site to be added to clusters in branch b it must be a distance less than cutoff_radius[b]
  // to any site in the phenomenal cluster. The b==0 value is ignored.
  ...
  LocalMaxLengthClusterSpecs cluster_specs {
    shared_prim,
  ...
  //   OrbitPrinterOptions printer_options;
  //   printer_options.coord_type = CART;
  //   printer_options.sym_info_opt = SymInfoOptions {printer_options.coord_type};
  //   printer_options.delim = char();
  //   printer_options.orbit_print_mode = ORBIT_PRINT_MODE::PROTO;
  //   Printer<IntegralCluster> printer {printer_options};
  //
  ...
  //
  //   log() << "factor group:\n";
  //   brief_description(log(), shared_prim->factor_group(), shared_prim->lattice(), printer_options.sym_info_opt);
  //   log() << std::endl;
  //
  //   log() << "invariant group:\n";
  //   brief_description(log(), phenomenal_cluster_group, shared_prim->lattice(), printer_options.sym_info_opt);
  //   log() << std::endl;
  //
  //   print_clust(
  //     orbits.begin(),
  ...
  // expect point cluster {b, i, j, k} to be found in an orbit with specified multiplicity
...
namespace test {
  ...
  // Check that point cluster consisiting of `unitcellcoord` is not found in orbits
  void expect_excluded_point_cluster(std::vector<Orbit<LocalSymCompare<IntegralCluster>>> const &orbits,
                                     UnitCellCoord const &unitcellcoord) {
  ...
  // Check that point cluster consisiting of `unitcellcoord` is found in orbits with `multiplicity`
  void expect_included_point_cluster(std::vector<Orbit<LocalSymCompare<IntegralCluster>>> const &orbits,
                                     UnitCellCoord const &unitcellcoord,
                                     int multiplicity) {
    IntegralCluster test {orbits.begin()->prototype().prim()};
