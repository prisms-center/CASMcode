...
///
/// A hyperplane reference, R, maps input vector coordinates, x, to output
/// scalar value, y:
/// - \code y = R.dot(x) \endcode
///
/// A HyperPlaneReference is constructed with a global reference, but may be
/// specialized to give a different R for a particular Supercell or
/// Configuration.
class HyperPlaneReferenceBase : public Reference {
 public:
  ...
  /// \param _input The 'input' function maps a Configuration to an
  /// Eigen::VectorXd,
  ///        typically the composition of a Configuration
  /// \param _supercell_ref A map of scelname to Eigen::VectorXd specializing
  /// the
  ...
  ///
  HyperPlaneReferenceBase(std::string _name, std::string _desc,
  ...
  ///
  const Eigen::VectorXd &global() const { return m_global_ref; }
  ...
  /// \brief Return the 'input' function that maps a Configuration to
  /// coordinates
 ...
 protected:
  ...
  ///
  Eigen::VectorXd &global() { return m_global_ref; }
  ...
  ///        specialized references
  ///
  std::map<std::string, Eigen::VectorXd> &config() { return m_config_ref; }
...
class HyperPlaneReference : public HyperPlaneReferenceBase {
 public:
  ...
  /// optional '_supercell_ref' and '_config_ref'.
  ///
  HyperPlaneReference(const Eigen::VectorXd &_global_ref, InputFunction _input,
                      SpecializedRef _supercell_ref = SpecializedRef(),
                      SpecializedRef _config_ref = SpecializedRef())
      : HyperPlaneReferenceBase(Name, Desc, _global_ref, _input, _config_ref,
                                _supercell_ref) {}
  ...
  /// \brief Access the global reference
  ///
  Eigen::VectorXd &global() { return HyperPlaneReferenceBase::global(); }
