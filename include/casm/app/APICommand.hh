...
///
/// - Do not inherit from this directly, instead inherit from
/// APICommand<OptionType>
/// - See the run_api_command function for context when implementing virtual
/// functions
...
class APICommand : public APICommandBase {
 public:
  ...
  APICommand(const CommandArgs &_args, OptionType &_opt)
      : APICommandBase(_args), m_opt(_opt) {}

  int count(std::string s) const { return opt().vm().count(s); }

  const po::variables_map &vm() const { return opt().vm(); }

  const OptionType &opt() const { return m_opt; }
...
std::unique_ptr<CommandType> make_api_command(
    CommandArgs const &args, typename CommandType::OptionType &opt);

/// Parse command line options and make API command. Throws for parsing errors.
template <typename CommandType>
std::unique_ptr<CommandType> make_api_command(
    std::string cli_str, PrimClex &primclex,
