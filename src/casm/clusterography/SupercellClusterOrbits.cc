...
//     `xtal::UnitCellCoordIndexConverter`, and not `SupercellSymInfo`. However,
//     in this context they are always going to be used with `SupercellSymInfo`
//     present to provide the permute group.
...
std::set<Index> make_cluster_site_indices(IntegralCluster const &cluster,
  ...
  for (xtal::UnitCellCoord const &uccoord : cluster) {
    cluster_site_indices.insert(
        sym_info.unitcellcoord_index_converter()(uccoord));
  }
...
IntegralCluster make_cluster(std::set<Index> const &cluster_site_indices,
  ...
  for (Index site_index : cluster_site_indices) {
    cluster.elements().push_back(
        sym_info.unitcellcoord_index_converter()(site_index));
  }
  return cluster;
}
...
///
std::set<Index> permute_cluster_site_indices(
    Permutation const &inverse_permute,
...
bool ClusterSiteIndicesCompare::operator()(std::set<Index> const &LHS,
                                           std::set<Index> const &RHS) const {
