...
/// \brief A sparse container of ECI values and their corresponding orbit
/// indices.
///
/// \ingroup ClexClex
class ECIContainer {
 public:
  ...
  ///        [eci_begin, eci_end)) containing the orbit indices of the
  ///        corresponding ECI values.
  ///
  template <typename SparseECIIterator, typename OrbitIndexIterator>
  ECIContainer(SparseECIIterator eci_begin, SparseECIIterator eci_end,
  ...
  const std::vector<double> &value() const { return m_value; }

  /// \brief const Access orbit indices of ECI values
  const std::vector<size_type> &index() const { return m_index; }
