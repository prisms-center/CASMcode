...
namespace CASM {
  ...
  namespace Completer {
    ...
    void SettingsOption::initialize() {
      ...
      ("set-default-clex", po::value<std::string>(&m_input_str)->value_name(ArgHandler::clex()), "Set the cluster expansion that CASM uses or acts on by default")
      ("erase-clex", po::value<std::string>(&m_input_str)->value_name(ArgHandler::clex()), "Erase the specified cluster expansion. Does not erase underlying bset, eci, etc.")
      ("clex", po::value<std::string>()->value_name(ArgHandler::clex()), "The cluster expansion for which to set property, bset, calctype, ref, or eci")
      ("set-property", po::value<std::string>(&m_input_str)->value_name(ArgHandler::property()), "Set the property being cluster expanded")
      ("set-bset", po::value<std::string>(&m_input_str)->value_name(ArgHandler::bset()), "Set the basis set")
      ("set-calctype", po::value<std::string>(&m_input_str)->value_name(ArgHandler::calctype()), "Set the calculation type")
      ("set-ref", po::value<std::string>(&m_input_str)->value_name(ArgHandler::ref()), "Set the calculation reference")
      ("set-eci", po::value<std::string>(&m_input_str)->value_name(ArgHandler::eci()), "Set the effective cluster interactions (ECI)")
  ...
  int settings_command(const CommandArgs &args) {
      ...
      try {
        ...
        if(!vm.count("help")) {
          if(option_count == 0) {
            ...
            log() << "Error in 'casm settings'. Use one option (other than --clex) at a time." << std::endl;
            call_help = true;
          }
        }
        ...
        if(vm.count("desc")) {
          ...
          log() << "\n";
          log() << "    Often it is useful to try multiple different basis sets, \n" <<
    ...
    if(vm.count("clex")) {
      ...
      if(it == set.cluster_expansions().end()) {
        ...
        err_log() << vm["clex"].as<std::string>() << " not found. expected basis set specifications file at: " << dir.bspecs(clex_desc.bset) << "\n" << std::endl;
    ...
    // create new cluster expansion settings group
      ...
      if(clex_exists(dir, clex_desc) && set.set_default_clex(clex_desc)) {
        ...
        log() << "Set '" << clex_desc.name << "' as default cluster expansion.\n\n";
        return 0;
      }
      else {
        log() << "Could not set '" << clex_desc.name << "' as default cluster expansion.\n\n";
        return 1;
      }
    }
    ...
    // set all
      ...
      if(multi_input.size() != 5) {
        log() << "Error using --set-all: Expected 5 arguments: 'property' 'calctype' 'ref' 'bset' 'eci'" << std::endl;
        return ERR_INVALID_ARG;
      }
