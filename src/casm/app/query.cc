...
bool QueryOption::verbatim_flag() const { return vm().count("verbatim"); }
...
int QueryCommandImplBase::help() const {
  ...
  log() << "Prints the properties for the objects currently selected in the "
           "MASTER\n"
           "selection or the specifed selection file via --selection (-c).\n\n"

           "Property values are output in column-separated (default) or JSON "
           "format.  By default, \n"
           "entries for 'name' and 'selected' values are included in the "
           "output. \n\n"
...
int QueryCommandImplBase::desc() const { return help(); }
...
class QueryCommandImpl : public QueryCommandImplBase {
 ...
 private:
  ...
  DB::Selection<DataObject> &_sel(Index i = 0) { return m_data.sel(i); }
...
template <typename DataObject>
void _update_query_dict(
    DataFormatterDictionary<QueryData<DataObject>> &query_dict) {}

template <>
void _update_query_dict<Configuration>(
    DataFormatterDictionary<QueryData<Configuration>> &query_dict) {
  ...
  query_dict.insert(
      QueryIO::permute_factor_group_op_desc<QueryData<DataObject>>());
...
QueryCommandImpl<DataObject>::QueryCommandImpl(const QueryCommand &cmd)
  ...
  for (auto const &formatter : m_data.dict()) {
    m_query_dict.insert(
        make_datum_formatter_adapter<QueryData<DataObject>, DataObject>(
            formatter));
  }
...
int QueryCommandImpl<DataObject>::_alias() const {
  ...
  std::string alias_name =
      boost::trim_copy(std::string(new_alias_str.cbegin(), it));
  std::string alias_command =
      boost::trim_copy(std::string(++it, new_alias_str.cend()));
...
void _query_equivalents(DataFormatter<QueryData<DataObject>> &formatter,
                        jsonParser &json, PrimClex const &primclex,
...
void _query_equivalents(DataFormatter<QueryData<Configuration>> &formatter,
  ...
  for (auto const &configuration : enumerator) {
    QueryData<Configuration> data{primclex, configuration, include_equivalents,
                                  equivalent_index, &enumerator.sym_op()};
...
template <typename DataObject>
void _query_equivalents(DataFormatter<QueryData<DataObject>> &formatter,
                        std::ostream &stream, PrimClex const &primclex,
                        DataObject const &object) {
  std::stringstream msg;
  msg << "Error in `casm query`: --include-equivalents not valid for type '"
      << traits<DataObject>::short_name << "'";
...
int QueryCommandImpl<DataObject>::_query() const {
  // WARNING: Valgrind has found some initialization/read errors in this block,
  // but unable to diagnose exact problem
  ...
  std::ostream &output_stream = make_ostream_if(
      _count("output"), log(), uniq_fout, _output_path(), _write_gz());
  output_stream << FormatFlag(output_stream).print_header(!_count("no-header"));

  // set status_log: where query settings and PrimClex initialization messages
  // are sent
  Log *status_log_ptr =
      (_output_path().string() == "STDOUT") ? &err_log() : &log();
  if (!status_log_ptr)
  ...
  // Print info
  status_log << "Print:"
             << std::endl;  // ***This line, from Valgrind: conditional depends
                            // on unitialized value
  for (int p = 0; p < _columns_vec().size(); p++) {
    status_log << "   - " << _columns_vec()[p]
               << std::endl;  // **This line, from Valgrind: invalid read
  }
  if (_count("output")) {
    if (_output_path().string() == "STDOUT") {
  ...
  DataFormatter<QueryData<DataObject>> formatter =
      _dict().parse(_all_columns());
...
std::vector<std::string> QueryCommandImpl<DataObject>::_all_columns() const {
  ...
  all_columns.insert(all_columns.end(), _columns_vec().begin(),
                     _columns_vec().end());
  return all_columns;
}
...
QueryCommand::QueryCommand(const CommandArgs &_args,
...
int QueryCommand::help() const { return impl().help(); }

int QueryCommand::desc() const { return impl().desc(); }

int QueryCommand::run() const { return impl().run(); }
  ...
  if (!m_impl) {
    if (in_project()) {
      ...
      DB::for_type_short(opt().db_type(),
                         DB::ConstructImpl<QueryCommand>(m_impl, *this));
    } else {
