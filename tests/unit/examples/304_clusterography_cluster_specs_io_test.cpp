...
// The InputParser template class helps parse JSON and collect errors and warning messages.
//
// The InputParser constructor calls an implementation function, `parse`, specialized by type, T,
// which indicates any extra input required to parse the JSON:
...
//
// When parsing is successful, the constructed value of type T will owned by the member `value`:
...
TEST(ExampleClusterographyClusterSpecsIO, ParseIntegralCluster) {
  ...
  CASM::InputParser<CASM::IntegralCluster> cluster_parser {cluster_json, *shared_prim};
  std::runtime_error cluster_error_if_invalid {"Failed to parse IntegralCluster JSON"};
  report_and_throw_if_invalid(cluster_parser, CASM::log(), cluster_error_if_invalid);
}

TEST(ExampleClusterographyClusterSpecsIO, ParseIntegralClusterOrbitGeneratorVec) {
  ...
  CASM::InputParser<std::vector<CASM::IntegralClusterOrbitGenerator>> clustergen_parser {clustergen_json, *shared_prim};
  std::runtime_error clustergen_error_if_invalid {"Failed to parse IntegralClusterOrbitGenerator JSON"};
  report_and_throw_if_invalid(clustergen_parser, CASM::log(), clustergen_error_if_invalid);
}
...
TEST(ExampleClusterographyClusterSpecsIO, PeriodicMaxLengthClusterSpecs) {
  ...
  // Get JSON input
  CASM::jsonParser cluster_specs_json = example::cluster_specs_json_test_valid();

  // Parsing ClusterSpecs requires a shared prim
  // - This uses the `parse` overload that chooses shared_prim->factor_group() automatically
  ...
  EXPECT_EQ(parser.value->periodicity_type(), CASM::CLUSTER_PERIODICITY_TYPE::PRIM_PERIODIC);
  CASM::ClusterSpecs::PeriodicOrbitVec orbits = parser.value->make_periodic_orbits(CASM::null_log());

  // Using check_orbits from example `302_clusterography_cluster_specs_test.cpp`
  example::check_orbits(shared_prim, orbits);
}

TEST(ExampleClusterographyClusterSpecsIO, InvalidPeriodicMaxLengthClusterSpecs) {
  // Example when JSON input is invalid

  // Construct a ZrO prim
  auto shared_prim = std::make_shared<CASM::Structure const>(test::ZrO_prim());

  // Get invalid JSON input (in this case 'Integral' is misspelled as 'IFntegral')
  CASM::jsonParser cluster_specs_json = example::cluster_specs_json_test_invalid();
  ...
  // Error to throw if parsing fails
  std::runtime_error error_if_invalid {"Failed to parse cluster specs JSON"};
  ASSERT_THROW((report_and_throw_if_invalid(parser, CASM::log(), error_if_invalid)), std::runtime_error);
