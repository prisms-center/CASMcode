...
/// \param input_state ConfigEnumInput, whose invariant group w.r.t.
/// [group_begin, group_end) is
...
VectorSpaceSymReport vector_space_sym_report(DoFSpace const &dof_space,
  ...
  if (!is_valid_dof_space(input_state.configuration(), dof_space)) {
    ...
    msg << "Error in vector_space_sym_report: DoFSpace is not valid for given "
           "input state.";
  ...
  if (!AnisoValTraits(dof_space.dof_key()).global()) {
    if (input_state.sites() != dof_space.sites()) {
      std::stringstream msg;
      msg << "Error in vector_space_sym_report: Mismatch between input state "
             "sites and dof_space sites.";
      throw std::runtime_error(msg.str());
    }
  }
  ...
  SupercellSymInfo const &sym_info =
      input_state.configuration().supercell().sym_info();
  xtal::BasicStructure const &prim_struc =
      input_state.configuration().prim().structure();
  std::vector<PermuteIterator> invariant_subgroup =
      make_invariant_subgroup(input_state, group_begin, group_end);
  ...
  if (prim_struc.global_dofs().count(dof_key)) {
    ...
    SymGroup pointgroup =
        make_point_group(invariant_subgroup, sym_info.supercell_lattice());
  ...
  // CASE 2: DoF is local
    ...
    auto group_and_ID = collective_dof_symrep(
        input_state.sites().begin(), input_state.sites().end(), sym_info,
        dof_key, invariant_subgroup);
