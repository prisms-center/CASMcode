...
namespace CASM {
  ...
  int ClusterInvariants::size() const {
    return m_size;
  }
  ...
  WithinScelClusterInvariants::WithinScelClusterInvariants(
    ...
    xtal::Lattice scel_lat = make_superlattice(cluster.prim().lattice(), transf_mat);
  ...
  /// \brief Number of elements in the cluster
  int WithinScelClusterInvariants::size() const {
    return m_size;
  }
  ...
  /// \brief Check if ClusterInvariants are equal
  bool almost_equal(
    WithinScelClusterInvariants const &A,
