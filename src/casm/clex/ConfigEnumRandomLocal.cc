...
ConfigEnumRandomLocalParams::ConfigEnumRandomLocalParams(
    MTRand &_mtrand, DoFKey _dof_key, Index _n_config, double _mag,
...
ConfigEnumRandomLocal::ConfigEnumRandomLocal(
    ...
    : ConfigEnumRandomLocal(_in_config, params.dof_key, params.n_config,
                            params.mag, params.normal_distribution,
                            params.mtrand) {}

ConfigEnumRandomLocal::ConfigEnumRandomLocal(ConfigEnumInput const &_in_config,
                                             DoFKey const &_dof_key,
                                             Index _n_config, double _mag,
  ...
  if (m_unit_length) m_normal = false;
...
std::string ConfigEnumRandomLocal::name() const { return enumerator_name; }

const std::string ConfigEnumRandomLocal::enumerator_name =
...
void ConfigEnumRandomLocal::randomize() {
  ...
  for (Index i = 0; i < m_site_selection.size(); ++i) {
    for (Index j = 0; j < m_dof_dims[i]; ++j) {
      m_dof_vals->site_value(m_site_selection[i])[j] =
          m_mtrand.randNorm(0., m_mag);
    }
    if (!m_normal) {
