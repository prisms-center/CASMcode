...
class MonteSettings : protected CASM::jsonParser {
 public:
  ...
  /// \brief Returns true if configname of configuration to use as starting
  /// motif has been specified
  ...
  /// \brief If dependent runs, start subsequent calculations with the final
  /// state
  ...
  /// \brief Given a settings jsonParser figure out the global tolerance
  /// (probably for == operator). Expects tolerance/value
  ...
  /// \brief Enumeration sample mode (default
...
class EquilibriumMonteSettings : public MonteSettings {
 public:
  ...
  /// \brief Returns true if explicit equilibration passes for the first run
  /// have been specified
  ...
  /// \brief Returns true if explicit equilibration passes for each run have
  /// been specified
  ...
  /// \brief Maximum number of steps, default
