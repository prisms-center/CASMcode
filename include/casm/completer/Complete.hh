...
/// Remove "--" or "-" from beginning of string if it exists, and return as new
/// string
 ...
 *  Argument: The argument you pass to the suboption, such as an integer or
 * filename
 *
 * The Suboption class consists of a string to identify the --long option, a
 * character to identify the -s(hort) version, and the type of arguments that
 * are expected.
 */
...
class Suboption {
 public:
  ...
  Suboption(const std::string &init_longname, std::string init_short,
  ...
  /// See if a provided string matches either the --long or -s(hort) tags.
  /// Expects leading '-' characters.
 ...
 * filename
 *
 * The Option class has a string that identifies it (e.g. "enum"), as well as a
 * list of Suboptions that it can take.
 */
...
class Option {
 ...
 public:
  ...
  Option(const std::string &init_tag,
  ...
  /// For a particular --suboption, get what kind of arguments are expected.
  /// suboption_tag should be pre-stripped.
 ...
 private:
  ...
  std::string m_tag;  // Make this const once the default constructors of Engine
                      // are gone?
 ...
 * things, but we don't seem to be using them much, so the completer is
 * currently limited to expecting only --long and -s(hort) (single character!)
 * options.
 */
...
class Engine {
 ...
 public:
  ...
  /// Construct by passing boost program options (eventually preferred so that
  /// it can update itself)
  // Engine(const std::vector<CASM::po::options_description>
  // &init_premade_descs);
  ...
  std::vector<std::string> probe_suboptions(
  ...
  /// Guess what should be returned based on the current word (probably not
  /// gonna make it that smart)
 ...
 private:
  /// List of all the available (presumably casm) options (e.g. "monte", "init",
  /// etc)
  std::vector<Option> m_avail_options;

  /// For a particular option with suboption, get what kind of arguments are
  /// expected
  ARG_TYPE _probe_argument_type(const std::string &option_tag,
                                const std::string &suboption_tag) const;

  /// Pointer to the project PrimClex, so the Engine can know the existing SCEL
  /// names
