...
void parse(InputParser<ClexBasisSpecs> &parser,
  ...
  auto bfuncs_subparser = parser.subparse<BasisFunctionSpecs>(
      "basis_function_specs", *shared_prim, dof_dict);

  auto cspecs_subparser =
      parser.subparse<ClusterSpecs>("cluster_specs", shared_prim);

  if (parser.valid()) {
    parser.value = notstd::make_unique<ClexBasisSpecs>(
        *bfuncs_subparser->value, std::move(cspecs_subparser->value));
  }
}
...
jsonParser &to_json(ClexBasisSpecs const &basis_set_specs, jsonParser &json,
                    ...
                    ParsingDictionary<DoFType::Traits> const *dof_dict) {
  to_json(basis_set_specs.basis_function_specs, json["basis_function_specs"],
          prim, dof_dict);
