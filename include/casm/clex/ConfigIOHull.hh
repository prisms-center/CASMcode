...
class BaseHull : public BaseValueFormatter<ValueType, Configuration> {
 public:
  ...
  BaseHull(
      ...
      double _singular_value_tol = 1e-8, double _bottom_facet_tol = 1e-8);
 ...
 private:
  // tol used to detect zero singular values during principal component analysis
  // preprocessing before hull calculation
...
/// \brief Returns a boolean indicating if a Configuration is a convex hull
/// vertex
///
/// Whether configuration is a vertex on the formation_energy convex hull (i.e.,
/// is a groundstate). Only one Configuration out of a set that have identical
/// or almost identical points in composition/energy space will return true.
///
...
class OnHull : public BaseHull<bool> {
 ...
 private:
  /// \brief Clone
  OnHull *_clone() const override { return new OnHull(*this); }
};

/// \brief Returns the distance, in eV, of a configuration's
...
class HullDist : public BaseHull<double> {
 ...
 private:
  /// \brief Clone
  HullDist *_clone() const override { return new HullDist(*this); }
};

/// \brief Returns a boolean indicating if a Configuration is a predicted convex
/// hull vertex
///
/// Whether configuration is a vertex on the *cluster-expanded* formation_energy
/// convex hull (i.e., is a *predicted* groundstate). Only one Configuration out
/// of a set that have identical or almost identical points in
...
class OnClexHull : public BaseHull<bool> {
 ...
 private:
  /// \brief Clone
  OnClexHull *_clone() const override { return new OnClexHull(*this); }
};

/// \brief Returns the distance, in eV, of a configuration's
/// clex(formation_energy_per_atom) above the predicted convex hull
///
/// Accepts arguments ($selection,$composition):
/// - $selection may be one of: <filename>, 'ALL', 'CALCULATED', 'MASTER'
/// <--default
...
class ClexHullDist : public BaseHull<double> {
 ...
 private:
  /// \brief Clone
  ClexHullDist *_clone() const override { return new ClexHullDist(*this); }
};
