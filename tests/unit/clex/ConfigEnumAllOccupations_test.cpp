...
namespace test {
  ...
  // This stores a histogram of composition (number of each type of molecule) -> # of occurances
  ...
  OccupationHistogram make_occ_histogram(ConfigIterator begin, ConfigIterator end) {
  ...
  std::ostream &print_histogram(std::ostream &sout, Eigen::VectorXi const &vector, std::vector<std::string> const &molecule_name) {
...
///
/// Notes:
/// - If enumerating on all sites in a supercell: guaranteed primitive & canonical, no duplicates
/// - If enumerating on selected sites and not all sites: guaranteed primitive, may be
...
protected:
  ...
  // allows checking number of configurations enumerated and histogram of composition (using number
  // of each type of occupant in the entire configuration)
  void check(ConfigEnumInput const &initial_state,
    ...
    std::vector<Configuration> configurations {enumerator.begin(), enumerator.end()};

    auto molecule_name = xtal::struc_molecule_name(*shared_prim);
    auto occ_histogram = test::make_occ_histogram(configurations.begin(), configurations.end());
...
TEST_F(ConfigEnumAllOccupationsTest, Test3) {
  ...
  // each entry: { {n_Zr, n_Va, n_O}, #occurances }
  test::OccupationHistogram expected_occ_histogram = {
    { to_VectorXi({16, 1, 15}), 1 },
    { to_VectorXi({16, 2, 14}), 2 },
    { to_VectorXi({16, 3, 13}), 9 },
    { to_VectorXi({16, 4, 12}), 20 },
    { to_VectorXi({16, 5, 11}), 43 },
    { to_VectorXi({16, 6, 10}), 70 },
    { to_VectorXi({16, 7, 9}), 95 },
    { to_VectorXi({16, 8, 8}), 105 },
    { to_VectorXi({16, 9, 7}), 95 },
    { to_VectorXi({16, 10, 6}), 70 },
    { to_VectorXi({16, 11, 5}), 43 },
    { to_VectorXi({16, 12, 4}), 20 },
    { to_VectorXi({16, 13, 3}), 9 },
    { to_VectorXi({16, 14, 2}), 2 },
...
TEST_F(ConfigEnumAllOccupationsTest, TestClusterSites1) {
  ...
  // only enumerate occupation on one site, site index=2 is fixed to occ value 0 (Va)
...
TEST_F(ConfigEnumAllOccupationsTest, TestClusterSites2) {
  ...
  // TODO: currently skips non-primitive, outputs non-canonical; change to output all? set via params?
...
//   std::vector<Index> number_expected {3, 3, 4, 3, 16, 10, 10, 10, 10, 18, 24, 24, 21, 15, 27, 27, 42, 24, 24, 21};
