...
CLIParse::CLIParse(int _argc, char **_argv)
    : m_argc(_argc), m_argv(_argv), m_parse_result(0), m_free_p(false) {}

/// Owning
CLIParse::CLIParse(std::string _args) : m_free_p(false) {
  ...
  if (m_parse_result) {
    ...
    switch (m_parse_result) {
      case 1: {
        err_log() << "Check for illegal unescaped characters: |, &, ;, <, >, "
                     "(, ), {, }"
                  << std::endl;
        break;
      }
...
void parse_args(Completer::OptionHandlerBase &opt, std::string args) {
  CLIParse parse(args);
  po::store(po::parse_command_line(parse.argc(), parse.argv(), opt.desc()),
            opt.vm());
  // gets default values
  po::notify(opt.vm());
}
