...
namespace CASM {
  ...
  Inserter select_cluster_sites(
    ...
    std::shared_ptr<Structure const> shared_prim = ref_config.supercell().shared_prim();
    SupercellSymInfo const &ref_supercell_sym_info = ref_config.supercell().sym_info();
    ...
    // Construct orbit generating elements for orbits with prim_ref_config.factor_group() symmetry
    // - prim_ref_supercell_sym_info.factor_group() -> prim_ref_config.factor_group() symmetry breaking
    ScelPeriodicSymCompare<IntegralCluster> prim_ref_supercell_sym_compare {
      shared_prim,
      prim_ref_supercell_sym_info.transformation_matrix_to_super(),
      shared_prim->lattice().tol()};
    std::vector<PermuteIterator> prim_ref_config_fg = prim_ref_config.factor_group();
    std::vector<IntegralCluster> prim_ref_config_orbit_generators;
    make_suborbit_generators(
      prim_ref_supercell_sym_info.permute_begin(),
      prim_ref_supercell_sym_info.permute_end(),
      prim_ref_config_fg.begin(),
      prim_ref_config_fg.end(),
      prim_ref_supercell_generators.begin(),
      prim_ref_supercell_generators.end(),
      prim_ref_supercell_sym_compare,
      std::back_inserter(prim_ref_config_orbit_generators));

    // Construct orbit generating elements for orbits with ref_config.factor_group() symmetry under
    // periodic boundary conditions
    // prim_ref_config.factor_group() -> ref_config.factor_group() w/ periodic boundar conditions
    std::vector<PermuteIterator> ref_config_fg = ref_config.factor_group();
    std::vector<Permutation> ref_config_inverse_permutations = make_inverse_permutations(
                                                                 ref_config_fg.begin(),
                                                                 ref_config_fg.end());
    auto ref_config_orbit_generators_pbc = make_orbit_generators_under_periodic_boundary_conditions(
                                             ref_supercell_sym_info,
                                             ref_config_inverse_permutations.begin(),
