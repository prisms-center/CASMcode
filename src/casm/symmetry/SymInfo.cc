...
SymInfo::SymInfo(const SymOp &op, const Lattice &lat, SymInfoOptions opt)
    : axis(lat), screw_glide_shift(lat), location(lat) {
...
void SymInfo::_set(const vector_type &_axis,
                   ...
                   const vector_type &_location, const Lattice &lat) {
...
/// notation
std::string sym_line(const Coordinate &axis, const Coordinate &point,
                     SymInfoOptions opt) {
  std::stringstream ss;
  double tol = opt.tol;
...
std::string sym_plane(const Coordinate &v1, const Coordinate &v2,
                      const Coordinate &point, SymInfoOptions opt) {
  std::stringstream ss;
  double tol = opt.tol;
...
/// notation
std::string sym_plane(const Coordinate &axis, const Coordinate &point,
                      SymInfoOptions opt) {
  double tol = opt.tol;
...
std::string description(const SymOp &op, const Lattice &lat,
                        SymInfoOptions opt) {
  return to_string(SymInfo(op, lat, opt), opt);
}

/// \brief Print SymGroup info
void description(Log &log, const SymGroup &g, const Lattice &lat,
                 SymInfoOptions opt) {
  Index i = 1;
  for (const auto &op : g) {
    log << log.indent_str() << i << ": (" << op.index() << ") "
        << description(op, lat, opt) << std::endl;
...
std::string brief_description(const SymOp &op, const Lattice &lat,
                              SymInfoOptions opt) {
  return to_brief_unicode(SymInfo(op, lat, opt), opt);
}

/// \brief Print SymGroup with brief strings
void brief_description(Log &log, const SymGroup &g, const Lattice &lat,
                       SymInfoOptions opt) {
  Index i = 1;
  for (const auto &op : g) {
    log << log.indent_str() << i << ": (" << op.index() << ") "
        << brief_description(op, lat, opt) << std::endl;
    if (opt.print_matrix_tau) {
      print_matrix_tau_col(log, op, opt.prec);
    }
    ++i;
