...
struct MagSpinDoFSpecs : public DoFSpecs {
  ...
  MagSpinDoFSpecs(std::string _flavor_name, Index _max_poly_order = -1)
      : max_poly_order(_max_poly_order), m_flavor_name(_flavor_name) {}
 ...
 private:
  ...
  std::string _name() const override { return m_flavor_name + "magspin"; }
};
...
/// max_poly_order)); \endcode
std::unique_ptr<DoFSpecs> magspin_specs(std::string const &flavor_name,
                                        Index max_poly_order);
}  // namespace DoFType
