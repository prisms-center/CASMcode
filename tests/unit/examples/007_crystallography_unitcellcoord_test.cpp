...
TEST(ExampleClusterographyUnitCellCoord, UnitCellCoordBasics) {
  ...
  // UnitCellCoord is a data structure representing crystal sites via integral
  // coordinates:
  // - the sublattice index, (b, index into Structure::basis)
  // - the unit cell coordinates, (i,j,k, multiples of the unit cell vectors,
  // stored as UnitCell object)
  ...
  // UnitCellCoord can be converted to Coordinate by referring to the reference
  // structure
  EXPECT_TRUE(uccoord_1.coordinate(prim).almost_equal(
      uccoord_1.unitcell().coordinate(prim.lattice()) +
      uccoord_1.sublattice_site(prim)));
}
