...
/// A RemoteHandle can be initialized with either a double or integer reference
/// and then passed to a DoF (or to a BasisFunction, which contains DoFs), in
/// order for the DoF to access the remote value
...
/// Base DoF class associates three pieces of data that identify the degree of
/// freedom
///   1. type_name:     The type of DoF (e.g., occupation ("occ"), displacement
///   ("disp"), etc.
///   2. variable_name: The name that will be associated with the individual
///   DoF.  This depends
///                     on usage case (e.g., "x", "y", or "z" for displacement)
///   3. ID:            Integer subscript that identifies variables of the same
///   name and type.
///                     For local DoF's, this is the site index; for global DoFs
///                     it denotes a subscript (e.g., for a strain component).
///
/// The type_name and variable_name must be set at construction. For local
/// DoF's, the ID can be updated as needed, but IDs of global DoFs persist
/// throughout their lifetime. Optionally, the ID can be 'locked' which prevents
...
class Base {
 ...
 private:
  ...
  /// dof_ID is a way to distinguish between DoFs with the same name but
  /// different identities dof_ID for now usually refers to the site index of a
  /// cluster (e.g., 0, 1, 2 of a triplet) or an index into the primitive cell
  /// neighbor list. Other usage cases may be introduced later
  Index m_dof_ID;
...
class OccupantDoF : public DiscreteDoF {
 public:
  ...
  /// \brief Returns true if (*this) is equivalent to RHS, @param compare_value
  /// specifies whether to compare occupation values
