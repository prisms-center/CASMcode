...
class EnumInterfaceBase : public notstd::Cloneable {
 ...
 public:
  /// Enumeration method description. What will be printed by `casm enum --desc
  /// MethodName`.
  ...
  ///        --scelnames as "scelnames": array of string (meaning depends on
  ///        method, if any)
  ///        --confignames as "confignames": array of string (meaning depends on
  ///        method, if any)
  ///        --verbosity as "verbosity": string (requested verbosity level)
  ///        --filter as "filter": array of string (query expression used filter
  ///        results)
  ///        --dry-run as "dry_run": bool (if true, do not commit database upon
  ///        completion)
  ///
  /// Notes:
...
/// Standardized string that can prepended to method output to make clear that
/// method results are not going to be committed
