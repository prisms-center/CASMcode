...
class GenericConfigCompare {
 public:
  explicit GenericConfigCompare(const IsEqualImpl &_eq) : m_eq(_eq) {}
  ...
  bool operator()(const PermuteIterator &A, const PermuteIterator &B, const
  ConfigType& other) const { if(m_eq(A, B, other)) { return false;
    }
    return m_eq.is_less();
  }
  */

  const IsEqualImpl &base() const { return m_eq; }
