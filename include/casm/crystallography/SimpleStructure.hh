...
namespace CASM {
  namespace xtal {
    ...
    public:
      ...
      /// If both contexts are present within the SimpleStructure, they are assumed to be different
      /// representation of the same crystal basis. The only difference is that the 'atomic'
      /// context separates each molecule into its constituent atoms.
      /// SimpleStructure DOES NOT TRACK how the molecular and atomic representations are related
      struct Info {
        ...
        /// \brief map of [property name, (m x names.size()) matrix] for all numerical site properties
        /// properties are assumed to be vectors of some property-specific dimension 'm'
        std::map<std::string, Eigen::MatrixXd> properties;

        /// \brief permutation that results in sites sorted alphabetically by species
        /// Guaranteed stable: will not change order for two sites with same species
        ...
        /// \brief Resize to hold N sites. All coordinates set to zero, all occupants set to "Va"
        void resize(Index N) {
          names.resize(N, "Va");
          coords.setZero(3, N);
          for(auto &el : properties)
            el.second.setZero(Eigen::NoChange, N);
        }
