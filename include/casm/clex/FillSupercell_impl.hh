...
namespace CASM {
  ...
  /// - For each, use ConfigEnumByPermutation to generate all equivalents (w.r.t. supercell factor group)
  ///   in "shared_supercell"
  ///
  /// Note:
  ...
  ConfigOutputIterator make_all_super_configurations(
    ...
    for(auto const &super_group_op : super_group) {
      if(generates_unique_prim_config_lattice_wrt_supercell_factor_group(super_group_op)) {
        auto it = find_supercell_factor_group_op_such_that_product_fills_supercell(super_group_op);
        if(it != supercell_sym_info.factor_group().end()) {
          Configuration initial_configuration = fill_supercell((*it) * super_group_op,
