...
class ConfigEnumInput {
 public:
  ...
  /// Construct using a Supercell's default Configuration and initial site
  /// selection
  ConfigEnumInput(Supercell const &_supercell,
                  std::set<Index> const &_site_index_selection);
  ...
  /// Const access selected sites, speficied by "site_index" (linear index into
  /// ConfigDoF)
...
/// Returns the subgroup of the configuration factor group that does not cause
/// any permutation between the set of selected and unselected sites of
/// "config_enum_input"
