...
namespace CASM {
  ...
  Configuration FillSupercell::operator()(SymOp const &symop,
    ...
    // In the first step, we transform the DoFs of motif by symop, without permuting their site indices
    ...
    // the starting supercell (*m_motif_supercell) and finding it in the resultant supercell (*m_supercell_ptr)
  ...
  SymOp const *FillSupercell::find_symop(Lattice const &motif_lattice) const {
    ...
    auto res = xtal::is_equivalent_superlattice(scel_lattice, motif_lattice, begin, end, motif_lattice.tol());
    if(res.first == end) {
      return nullptr;
    }
    return &(*res.first);
  ...
  void FillSupercell::_init(Supercell const &_motif_supercell) const {
    ...
    auto oriended_motif_lattice_points = xtal::make_lattice_points(oriented_motif_lat, m_supercell_ptr->lattice(), TOL);
    ...
    for(Index s = 0 ; s < m_motif_supercell->num_sites() ; s++) {
      ...
      UnitCellCoord oriented_uccoord = sym::copy_apply(*m_symop_ptr,
                                                       m_motif_supercell->uccoord(s),
                                                       prim.lattice(),
      ...
      for(const UnitCell &oriented_motif_uc : oriended_motif_lattice_points) {
        ...
        Index prim_motif_tile_ind = m_supercell_ptr->sym_info().unitcell_index_converter()(oriented_motif_uc_relative_to_prim);

        UnitCellCoord mc_uccoord(
          oriented_uccoord.sublattice(),
          m_supercell_ptr->sym_info().unitcell_index_converter()(prim_motif_tile_ind) + oriented_uccoord.unitcell());
