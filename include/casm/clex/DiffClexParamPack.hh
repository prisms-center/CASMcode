...
namespace CASM {
  ...
  /// \brief container struct for storing a standalone value as 2D arrays of both autodiff Scalars doubles
  struct DiffScalarContainer {
    using DiffScalar = DiffClexParamPack_impl::DiffScalar;

    /// \brief Name of the standalone value stored in this container. Should match name of associated key
    std::string m_name;

    /// \brief linear offset for start of value index in linear array
    /// FADBAD forward (backward) autodiff types reference independent (dependent) via a linear index
    /// For a particular entry (i,j) into the 2d container for a specific type (e.g., "occ_site_func"), the
    /// linear index is defined as l=lbegin+i*m_cache.cols()+j
    Index lbegin;

    /// \brief Flag, set to true if this container holds independent values (false if dependent)
    bool m_independent;
    ...
    /// \param _name Name of the standalone value stored in this container, should match name of associated key
    /// \param param_dim Dimension of vector parameter (e.g., 3 for "disp_var"), corresponds to columns of m_data, m_data_double, etc
    /// \param num_params Number of vector parameters (e.g., 1 for global DoF, number of sites for local DoF), corresponds to rows of m_data, m_data_double, etc
    ...
    /// \brief Gradient of dependent function index f wrt the independent variables contained in here
    /// \param f secondary identifier denoting linear index of function F(f) among the set F of all dependent variables
    /// \result matrix G, with G(i,j) = Grad[F(f)](l(i,j)), where 'l' is linear index of each (i,j) entry in m_data.
    ...
    /// \brief Hessian components of dependent function index f corresponding to independent variable index 'a' and all independent variables managed by this
    /// \param f secondary identifier denoting linear index of function F(f) among the set F of all dependent variables
    /// \param a secondary identifier denoting linear index of independent variable 'a' amont the set of all independent variables
    /// \result matrix H with M(i,j) = H[F(f)](a,l(i,j)), where 'l' is linear index of each (i,j) entry in m_data.
    ...
    /// \brief Must be called on each independent parameter before function evaluation if derivatives are desired.
    /// \param Total number of independent parameters managed by DiffClexParamPack
    ...
    /// \brief Must be called on each dependent parameter after function evaluation if derivatives are desired.
    /// \param Total number of dependent parameters managed by DiffClexParamPack
    ...
    /// \brief Copy values of autodiff scalars from m_data to m_data_double (matrix of doubles) in preparation for external access
    void eval_double() {
    ...
    /// \brief Returns one past end of max linear index range allowed for this parameter set
  ...
  /// \brief Class for managing the set of all dependent and independent paramters used/generated by clexulator
  ...
  public:
    ...
    /// \brief Default constructor initializes evaluation mode and zeros numbers of managed parameters
    DiffClexParamPack() :
    ...
    /// \brief Prepares all independent parameters before function evaluation if derivatives are desired
    void pre_eval() {
      if(eval_mode() == DIFF) {
    ...
    /// \brief Processes all dependent parameters after function evaluation if derivatives are desired
    void post_eval() {
      if(eval_mode() == DIFF) {
        for(DiffScalarContainer &datum : m_data) {
          if(!datum.m_independent) {
