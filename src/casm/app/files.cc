    ...
    try {
      ...
      if(!vm.count("help")) {
        ...
        if(!contains(allowed, settings)) {
          err_log() << "Error in 'casm files'. '" << settings << "' is not a valid option to --settings." << std::endl;
        ...
        for(auto it = calc.begin(); it != calc.end(); ++it) {
          if(!contains(allowed, *it)) {
            err_log() << "Error in 'casm files'. '" << *it << "' is not a valid option to --calc." << std::endl;
            err_log() << "  Valid options are: 'settings', 'status', 'all'" << std::endl;
            return ERR_INVALID_ARG;
          }
        }
      }
