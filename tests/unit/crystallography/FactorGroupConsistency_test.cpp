...
namespace {
  void print_poscar(const xtal::BasicStructure &printable, std::ostream &outstream) {
  ...
  xtal::SymOpVector make_factor_group_via_mapping(const xtal::BasicStructure &struc, double tol) {
    xtal::SimpleStructure sstruc = xtal::make_simple_structure(struc);
    xtal::StrucMapper mapper((xtal::SimpleStrucMapCalculator(sstruc)));
    auto sym_set =
      mapper.map_deformed_struc_impose_lattice(sstruc, xtal::Lattice(sstruc.lat_column_mat), 0, xtal::StrucMapping::big_inf(), tol);
...
protected:
  ...
  void SetUp() override {
    ...
    for(const xtal::BasicStructure &prim : primitive_structures) {
      ...
      for(const Eigen::Matrix3l &mat : prim_to_super_trasnformation_matrices) {
        superstructures.back().emplace_back(xtal::make_superstructure(prim, mat));
      }
    }
  ...
  void compare_factor_groups(const xtal::BasicStructure &struc) {
    xtal::SymOpVector mapping_factor_group = ::make_factor_group_via_mapping(struc, tol);
    ...
    for(const xtal::SymOp &mapping_op : mapping_factor_group) {
      UnaryCompare_f<xtal::SymOpPeriodicCompare_f> matches_mapping_op(mapping_op, struc.lattice(), tol);
      EXPECT_TRUE(std::find_if(factor_group.begin(), factor_group.end(), matches_mapping_op) != factor_group.end());
    }
  }
};
