...
///   - Access spatial transform that was used during most recent spatial
///   preparation of an
///     element
/// - Element Derived::representation_prepare_impl(const Element &A) const
///   - construct one canonical representation of Element amongst all equivalent
///   representations
///     (i.e. sort sites of a cluster), used for comparison
/// - std::unique_ptr<SymOpRepresentation>
/// Derived::canonical_transform_impl(Element const  &obj) const
...
/// - bool Derived::invariants_compare_impl(const Element &A, const Element &B)
/// const
...
///   - allows flexibility for working outside the typical
///   MasterSymGroup/SymGroupRep framework
///     or when symmetry application is ambiguous (like applying symmetry to a
///     Eigen::VectorXd)
///
...
class SymCompare : public Base {
 public:
  ...
  /// \brief Prepare an element for comparison via representation_prepare(),
  ...
  /// \brief type `InvariantsType` is used to screen elements for equivalence
  /// and sort orbits
  ...
  bool invariants_compare(const InvariantsType &A,
  ...
  bool inter_orbit_compare(const Element &A, const InvariantsType &A_invariants,
                           const Element &B,
                           const InvariantsType &B_invariants) const;

  /// \brief Check equivalence of prototypes in different orbit
  bool inter_orbit_equal(const Element &A, const InvariantsType &A_invariants,
                         const Element &B,
                         const InvariantsType &B_invariants) const;

  /// \brief Access transform that took cluster from unprepared to prepared
  /// state
  std::unique_ptr<SymOpRepresentation> canonical_transform(
 ...
 protected:
  /// \brief Orders orbit prototypes, breaking invariants_compare ties
  bool inter_orbit_compare_impl(const Element &A,
...
template <typename Base>
typename SymCompare<Base>::Element SymCompare<Base>::copy_apply(
    SymOp const &op, Element obj) const {
...
/// transformation
///
/// - For instance, translate a cluster so comparison may be performed more
/// efficiently.
/// - The transformation that is applied is stored and available as
/// `spatial_transform`
/// - The `spatial_transform` is also included in the symmetry operations stored
/// in the
///   Orbit equivalence_map (i.e. equivalence_map_element = spatial_transform *
///   generating_group_op)
/// - Returns pair such that pair.first = apply_sym(pair.second, obj)
///
template <typename Base>
typename SymCompare<Base>::Element SymCompare<Base>::spatial_prepare(
    Element obj) const {
  return derived().spatial_prepare_impl(obj);
}

/// \brief Prepare an element for comparison via transformation of its internal
/// representation
///
...
///
template <typename Base>
typename SymCompare<Base>::Element SymCompare<Base>::representation_prepare(
    Element obj) const {
...
///
template <typename Base>
typename SymCompare<Base>::Element SymCompare<Base>::prepare(
    Element obj) const {
...
template <typename Base>
typename SymCompare<Base>::InvariantsType SymCompare<Base>::make_invariants(
    const Element &obj) const {
...
///
/// - Returns 'true' to indicate A < B
/// - Equivalence is indicated by \code !invariants_compare(A,B) &&
/// !invariants_compare(B,A) \endcode
/// - Assumes elements are 'prepared' before being compared
template <typename Base>
bool SymCompare<Base>::invariants_compare(const InvariantsType &A,
                                          const InvariantsType &B) const {
...
template <typename Base>
bool SymCompare<Base>::invariants_equal(const InvariantsType &A,
                                        const InvariantsType &B) const {
...
///
/// - Returns 'true' to indicate A < B
/// - Equivalence is indicated by \code !inter_orbit_compare(A,B) &&
/// !inter_orbit_compare(B,A) \endcode
/// - Assumes elements are in canonical form
template <typename Base>
bool SymCompare<Base>::inter_orbit_compare(
    const Element &A, const InvariantsType &A_invariants, const Element &B,
    const InvariantsType &B_invariants) const {
...
/// \brief Check equivalence of prototypes in different orbit
///
/// \returns \code !inter_orbit_compare(A,B) && !inter_orbit_compare(B,A)
/// \endcode
///
/// - Assumes elements are in canonical form
template <typename Base>
...
template <typename Base>
std::unique_ptr<SymOpRepresentation> SymCompare<Base>::canonical_transform(
    Element const &obj) const {
