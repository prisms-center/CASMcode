...
class DatabaseSetIterator : public DatabaseIteratorBase<ValueType> {
 public:
  ...
  std::string name() const override { return m_it->name(); }
 ...
 private:
  ...
  DatabaseSetIterator(base_iterator _it) : m_it(_it) {}
  ...
  const ValueType &dereference() const override { return *m_it; }

  /*
  long distance_to(const DatabaseIteratorBase<ValueType> &other) const override
  { return std::distance(m_it, static_cast<const DatabaseSetIterator
  &>(other).m_it);
  }
  */
