...
TEST_F(ExampleEnumerationSimpleCubicConfigEnumStrain, Example1) {
  ...
  using enumeration_test_impl::global_dof_symrep;
  using global_dof_symrep::make_point_group;
  ...
  for(auto const &scel : primclex.db<Supercell>()) {
    ...
    // For this case, use the full space, so rank_of_subspace == dim
    Index rank_of_subspace = dim;

    min_val = Eigen::VectorXd::Constant(rank, 0.0);
    max_val = Eigen::VectorXd::Constant(rank, 1.0);
    increment_val = Eigen::VectorXd::Constant(rank, 0.1);

    CASM::ConfigEnumStrain enumerator {input, wedges, min_value, max_value, increment_value,
