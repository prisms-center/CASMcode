...
class ClexBasisBuilder {
 public:
  ...
  virtual std::vector<DoFKey> filter_dof_types(
      std::vector<DoFKey> const &_dof_types) {
    return _dof_types;
  }
...
/// \brief virtual base class for printing orbit functions of type specified by
/// implementation.
class OrbitFunctionTraits {
 public:
  static std::string class_desc() { return "Orbit Function Traits"; }

  OrbitFunctionTraits(std::string const &_name, std::string const &_short_desc,
                      std::string const &_long_desc)
      : m_name(_name), m_short_desc(_short_desc), m_long_desc(_long_desc) {}

  virtual ~OrbitFunctionTraits() {}

  std::string const &name() const { return m_name; }

  std::string const &short_desc() const { return m_short_desc; }

  std::string const &long_desc() const { return m_long_desc; }
  ...
  virtual std::string clexulator_point_prepare_string(
      ...
      PrimNeighborList &_nlist, std::string const &indent) const {
    return "";
  }
  ...
  virtual void print_typedefs(std::ostream &out, std::string const &class_name,
...
class InvariantPolyOrbitFunctionTraits : public OrbitFunctionTraits {
 public:
  InvariantPolyOrbitFunctionTraits()
      : OrbitFunctionTraits("invariant_poly", "Invariant Polynomials",
                            "Orbit functions are constructed as invariant "
                            "polynomials of site basis functions.") {}

  std::unique_ptr<ClexBasisBuilder> basis_builder() const override {
    return std::unique_ptr<ClexBasisBuilder>(
        new InvariantPolyBasisBuilder(name()));
  }
};
