...
void Proj::make() {
  if (find_casmroot(dir) != dir) {
    // build a project
    build_project(make_default_project_settings(prim, title, dir),
                  Structure{prim});
  }
...
/// \brief Check that 'casm init' runs without error and expected files are
/// created
...
void Proj::_check_symmetry(int lat_pg_op, int lat_pg_class, int xtal_pg_op,
                           int xtal_pg_class, int fg_op, int fg_class,
                           std::string lat_pg_name, std::string xtal_pg_name) {
  check_symgroup(jsonParser(m_dirs.lattice_point_group()), lat_pg_op,
                 lat_pg_class);
  check_symgroup(jsonParser(m_dirs.crystal_point_group()), xtal_pg_op,
                 xtal_pg_class);
  ...
  EXPECT_EQ(boost::regex_search(
                m_p.gets(), m_match,
                boost::regex(R"(Lattice point group is:\s+)" + lat_pg_name)),
            true);
  EXPECT_EQ(boost::regex_search(
                m_p.gets(), m_match,
                boost::regex(R"(Crystal point group is:\s+)" + xtal_pg_name)),
            true);
}
...
void Proj::check_enum() {
  m_p.popen(cd_and() + autotools::abs_ccasm_path() +
            " enum --method ScelEnum --max 2");
  EXPECT_EQ(m_p.exit_code(), 0);

  m_p.popen(cd_and() + autotools::abs_ccasm_path() +
            " enum --method ConfigEnumAllOccupations --all");
  EXPECT_EQ(m_p.exit_code(), 0);
}
