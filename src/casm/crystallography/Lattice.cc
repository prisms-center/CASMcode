...
namespace CASM {
  ...
  Lattice &Lattice::operator=(const Lattice &RHS) {
    if(this == &RHS)
      return *this;
    RHS.m_coord_trans_mat = RHS.m_coord_trans_mat;
  ...
  double Lattice::length(Index i) {
    // Calculates Lengths
    vec(i).length();
  }

  //********************************************************************
  double Lattice::angle(Index i) {
    double t_a = vec((i + 1) % 3).dot(vecs((i + 2) % 3)) / (length((i + 1) % 3) * length((i + 2) % 3));
...
void Lattice::generate_voronoi_table() const {
  ...
  Counter<Eigen::Vector3i > combo_count(Eigen::Vector3i(-1, -1, -1),
...
Lattice Lattice::get_lattice_in_plane(Eigen::Vector3i millers, int max_vol) const {  //John G 121030
  ...
  Array<Eigen::Vector3d > surface_cell(3);     //Holds new lattice vectors, two of which are in the surface plane
  Array<Eigen::Vector3d > last_surface_cell(3);
  ...
  //points that are on the plane, which we connect to get the second lattice vector
    ...
    Vector3<Eigen::Vector3d > templat;
    templat[0] = HK;
    templat[1] = KL;
    templat[2] = LH;
    ...
    for(int i = 0; i < 3; i++) {
      ...
      for(int j = maxval; j > 1; j--) {
        Eigen::Vector3d shortened = coord_trans_mat[CART] * templat[i] / j; //Shorten and convert to fractional
        bool combo = true;
