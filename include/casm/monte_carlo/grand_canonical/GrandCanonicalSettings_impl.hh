  ...
  try {
    for (auto it = t_measurements.cbegin(); it != t_measurements.cend(); it++) {
      ...
      std::vector<std::string> vector_possible = {
          "comp",           "comp_n",           "site_frac",
          "atom_frac",      "all_correlations", "non_zero_eci_correlations",
          "order_parameter"};
      ...
      if (std::find(vector_possible.cbegin(), vector_possible.cend(),
        ...
        else if (prop_name == "order_parameter") {
...
std::tuple<bool, double> GrandCanonicalSettings::_get_precision(
...
SamplerInsertIterator GrandCanonicalSettings::_make_comp_samplers(
  ...
  for (size_type i = 0;
       i < primclex.composition_axes().independent_compositions(); i++) {
    print_name =
        std::string("comp(") + std::string(1, (char)(i + ((int)'a'))) + ")";

    std::tie(must_converge, prec) = _get_precision(it);

    // if 'must converge'
    if (must_converge) {
      ptr = new CompMonteSampler(i, primclex.composition_axes(), print_name,
                                 prec, confidence(), data_maxlength);
    } else {
      ptr = new CompMonteSampler(i, primclex.composition_axes(), print_name,
...
SamplerInsertIterator GrandCanonicalSettings::_make_comp_n_samplers(
  ...
  for (size_type i = 0; i < primclex.composition_axes().components().size();
       i++) {
    prop_name = "comp_n";

    print_name = std::string("comp_n(") +
                 primclex.composition_axes().components()[i] + ")";
...
SamplerInsertIterator GrandCanonicalSettings::_make_site_frac_samplers(
  ...
  for (size_type i = 0; i < primclex.composition_axes().components().size();
    ...
    print_name = std::string("site_frac(") +
                 primclex.composition_axes().components()[i] + ")";
    ...
    // if 'must converge'
    if (must_converge) {
      ptr = new SiteFracMonteSampler(i, primclex.prim().basis().size(),
                                     print_name, prec, confidence(),
                                     data_maxlength);
    } else {
      ptr = new SiteFracMonteSampler(i, primclex.prim().basis().size(),
                                     print_name, confidence(), data_maxlength);
    }
...
SamplerInsertIterator GrandCanonicalSettings::_make_atom_frac_samplers(
  ...
  for (size_type i = 0; i < primclex.composition_axes().components().size();
    ...
    // sample for non-vacancy components
    if (!xtal::is_vacancy(primclex.composition_axes().components()[i])) {
      print_name = std::string("atom_frac(") +
                   primclex.composition_axes().components()[i] + ")";
      ...
      // if 'must converge'
      if (must_converge) {
        ptr = new AtomFracMonteSampler(i, vacancy_index, print_name, prec,
                                       confidence(), data_maxlength);
      } else {
        ptr = new AtomFracMonteSampler(i, vacancy_index, print_name,
                                       confidence(), data_maxlength);
      }
...
GrandCanonicalSettings::_make_non_zero_eci_correlations_samplers(
    const PrimClex &primclex, jsonParserIteratorType it,
    SamplerInsertIterator result) const {
  size_type data_maxlength = max_data_length();
  std::string prop_name;
...
template <typename jsonParserIteratorType, typename SamplerInsertIterator>
SamplerInsertIterator GrandCanonicalSettings::_make_order_parameter_samplers(
    const PrimClex &primclex, jsonParserIteratorType it,
    SamplerInsertIterator result) const {
  size_type data_maxlength = max_data_length();
  std::string print_name;
