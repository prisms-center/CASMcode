...
namespace CASM {
  ...
  template<typename T> class QueryCommandImpl;

  /// 'casm query' command
  ///
  /// This class wraps a pointer to an implementation class QueryCommandImpl<DataObject>
  /// which does most of the work. The implementation classes are defined in the
  /// source file.
  ///
  class QueryCommand :
    public APICommand<Completer::QueryOption> {

  public:

    using ImplBase = QueryCommandImplBase;
    template<typename ConfigType> using Impl = QueryCommandImpl<ConfigType>;
