...
namespace CASM {
  ...
  std::string SuperConfigEnum::name() const {
    ...
    for(Index i = 0; i < this->_unitcell_index_converter().total_sites(); ++i) {
      ...
      for(Index j = 0; j < _sub_supercell().num_sites(); ++j) {
        ...
        for(auto &pair : super_config.configdof().local_dofs()) {
          ...
          auto &local_dof_values = super_config.configdof().local_dof(dof_key).values();

          local_dof_values.col(m_index_map[i][j]) = sub_config_i.configdof().local_dof(dof_key).values().col(j);
        }
      }
    }
  }
