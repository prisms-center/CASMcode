...
class ClexBasisWriter {
 public:
  /// \brief Construct ClexBasisWriter, collecting requisite DoF info from
  /// '_prim'
  ClexBasisWriter(Structure const &_prim,
  ...
  void print_clexulator(std::string class_name, ClexBasis const &clex,
                        std::vector<OrbitType> const &_tree,
                        PrimNeighborList &_nlist, std::ostream &stream,
                        double xtal_tol);

 private:
  void _initialize(Structure const &_prim,
                   ParamPackMixIn const &parampack_mix_in);

  std::vector<std::unique_ptr<FunctionVisitor> > const &
  _site_function_visitors() const {
    return m_site_visitors;
  }

  std::vector<std::unique_ptr<FunctionVisitor> > const &
  _clust_function_visitors() const {
  ...
  template <typename OrbitType>
  void print_param_pack(std::string class_name, ClexBasis const &clex,
                        std::vector<OrbitType> const &_tree,
                        PrimNeighborList &_nlist, std::ostream &stream,
...
/// \brief Print the flower function formulae for orbit @param _clust_orbit
/// specified by BasisSet @param _bset_orbit The pivot of the flower is
/// specified by @param _sublat_index

/// The flower function of site @param _sublat_index and orbit @param
/// _clust_orbit is obtained by summing the contributions of all cluster
/// functions from @param _bset_orbit that 'touch' the site
/// (b,i,j,k)=(sublat_index,0,0,0), including functions that are found by
/// translations of equivalent clusters in @param _clust_orbit. Depending on the
/// orbit periodicity (i.e., Orbit::sym_compare()), not all translations of the
/// cluster that touch (sublat_index,0,0,0) are translationally equivalent.
/// Thus, the result is the std::map that associates UnitCell (i.e, translation)
/// to a set of formulae, (i.e., std::vector<std::string>), with one formula per
/// function in _clust_orbit[i] (some or all formulae may evaluate to zero, if
/// if @param _clust_orbit doesn't include site of type @param _sublat_index.

/// @param _bset_transfrom is a function/functor that applies a transformation
/// to each _bset_orbit[i].
/// @param _nlist is the PrimNeighborList, used to index sites in the
/// neighborhood
/// @param _labelers is a set of FunctionVisitors that can be used to control
/// formatting of the formulae
...
std::tuple<std::string, std::string> clexulator_dflower_function_strings(
    ...
    PrimNeighborList &_nlist,
    std::vector<std::unique_ptr<FunctionVisitor> > const &visitors,
    FunctionVisitor const &prefactor_labeler, std::string const &indent);
//*******************************************************************************************
...
std::set<UnitCellCoord> equiv_ucc(
    UCCIterType begin, UCCIterType end, UnitCellCoord const &_pivot,
    Structure const &_prim, IntegralClusterSymCompareType const &_sym_compare);
}  // namespace ClexBasisWriter_impl
}  // namespace CASM
