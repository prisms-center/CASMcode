...
class DatabaseHandler {
 public:
  ...
  template <typename T>
  void insert_props(std::string db_name, std::string calc_type,
  ...
  /// Access specified PropertiesDatabase
  template <typename T>
  const PropertiesDatabase &db_props(std::string db_name,
 ...
 private:
  ...
  typedef std::map<std::pair<std::string, std::string>,
                   std::unique_ptr<DatabaseBase> >
      map_type;

  class PropDBKey {
   public:
    PropDBKey(std::string _config_type, std::string _db_type,
              std::string _calc_type)
        : m_value(_config_type, _db_type, _calc_type) {}

    std::string &config_type() { return std::get<0>(m_value); }
    const std::string &config_type() const { return std::get<0>(m_value); }

    std::string &db_type() { return std::get<1>(m_value); }
    const std::string &db_type() const { return std::get<1>(m_value); }

    std::string &calc_type() { return std::get<2>(m_value); }
    const std::string &calc_type() const { return std::get<2>(m_value); }

    bool operator<(const PropDBKey &B) const { return m_value < B.m_value; }
  ...
  typedef std::map<PropDBKey, std::unique_ptr<PropertiesDatabase> >
      props_map_type;
  ...
  template <typename T>
  props_map_type::iterator _find_props(std::string db_name,
                                       std::string calc_type) const;
  ...
  template <typename T>
  void _no_props_database_error(std::string db_name,
                                std::string calc_type) const;
