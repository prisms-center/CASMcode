...
namespace Local {
// Takes set of subsystem compomponents, given as [set of indices], and set of
// sublattices, given as the pairs {allowed components [set of indeces],
// multiplicity [index]}, and total number of components in the larger system.
// Returns the set of extreme integer compositions for the subsystem
static std::vector<Eigen::VectorXi> _sub_extremes(
    std::set<Index> const &subcomponents,
    std::map<std::set<Index>, Index> const &sublats, Index dim) {
  ...
  for (Index ic = 0; ic < ncomb; ++ic) {
    ...
    for (Index c : combo) priority.push_back(compon[c]);
    ...
    do {
      ...
      // Keep unique extrema
      if (success && !contains(result, tend)) result.push_back(tend);
...
static std::map<std::set<Index>, std::map<std::set<Index>, Index> >
_chemical_subsystems(ParamComposition::AllowedOccupants const &_allowed_occs) {
  std::map<std::set<Index>, std::map<std::set<Index>, Index> > result;

  std::vector<std::string> compon =
      ParamComposition::string_components(_allowed_occs);

  // Convert _allowed_occs to occ_map, which has a pair for each unique
  // sublattice, consisting of the indices of its allowed components and its
  // multiplicity
  std::map<std::set<Index>, Index> occ_map;
  for (auto const &list : _allowed_occs) {
    std::set<Index> tocc;
    for (auto const &occ : list) tocc.insert(find_index(compon, occ));
  ...
  // Use flooding algorithm to partition unique sublattices into chemically
  // independent subsystems

  // sublattices that have been partitioned
  std::set<Index> visited;

  // A chemical subsystem is a graph with species as nodes. Each sublattice is a
  // fully connected subgraph. Traverse the edges implied by this assumption to
  // find the nodes belong to the connected subgraph associated with each
  // species. Loop over all species and record the sublattices that generate
  // each subgraph thus obtained
    ...
    while (!q.empty()) {
      ...
      if (!visited.count(j)) {
        ...
        for (auto const &list : occ_map) {
          // If searched species 'j' is allowed at this sublattice, add its
          // allowed species to the queue and to the set of connected nodes Add
          // the sublattice to the independent subsystem
...
std::vector<std::string> ParamComposition::string_components(
...
ParamComposition::ParamComposition(
    ...
    : m_allowed_occs(std::move(_allowed_occs)), m_prim_end_members(0, 0) {
...
ParamComposition::ParamComposition(
    ParamComposition::AllowedOccupants _allowed_occs,
    const Eigen::MatrixXd &transf_mat, const Eigen::VectorXd &_origin,
    const int &_rank_of_space, const int &COMP_TYPE)
...
void ParamComposition::generate_prim_end_members() {
  std::map<std::set<Index>, std::map<std::set<Index>, Index> > subsystems =
      Local::_chemical_subsystems(allowed_occs());

  std::vector<Eigen::VectorXi> tresult(
      1, Eigen::VectorXi::Zero(components().size()));

  for (auto const &subsystem : subsystems) {
    std::vector<Eigen::VectorXi> tsubs = Local::_sub_extremes(
...
void ParamComposition::generate_composition_space(bool verbose) {
  ...
  if (m_allowed_list.size() != 0) {
    std::cerr << "WARNING in ParamComposition::generate_composition_space, "
                 "your allowed_list is non-empty. If you are not careful,you "
                 "may end up with repeats of allowed composition axes"
              << std::endl;
  }
  ...
  if (verbose) log() << "Rank of space : " << m_rank_of_space << std::endl;

  // Count over K-combinations of end members, for each K-combination, select
  // one as origin and the rest as axes
  ...
  for (Index c = 0; c < ncomb; ++c) {
    ...
    if (c > 100000 && m_allowed_list.size() > 0) break;
    if (qr.compute(tmembers).rank() < K) continue;
    ...
    for (Index i_o = 0; i_o < K; ++i_o) {
      ...
      // initialize a ParamComposition object with these spanning vectors and
      // origin
      ParamComposition tcomp = calc_composition_object(torigin, tspanning);

      // loop through end members and see what the values of the compositions
      // works out to
      if (verbose) log() << "Calculated compositions:" << std::endl;
      ...
      for (Index j = 0; is_positive && j < m_prim_end_members.rows(); j++) {
        ...
        if (verbose)
          log() << m_prim_end_members.row(j) << "  :  " << test_comp.transpose()
                << std::endl;
...
//*************************************************************
// PRINT Routines
void ParamComposition::print_composition_formula(
    std::ostream &stream, const int &stream_width) const {
...
void ParamComposition::print_member_formula(const Eigen::VectorXd &member,
                                            std::ostream &stream,
...
void ParamComposition::print_origin_formula(std::ostream &stream,
                                            const int &stream_width) const {
...
void ParamComposition::print_composition_axes(std::ostream &stream) const {
  stream << "Number of choices of composition axes: " << m_allowed_list.size()
         << std::endl;
  // Print Header: ORIGIN, <COMPOUNDS AT THE ENDS OF DIFFERENT AXES> , GENERAL
  // FORMULA
...
Eigen::VectorXd ParamComposition::calc(const Eigen::VectorXd &tcomp,
                                       const int &MODE) {
  if (MODE == PARAM_COMP) {
    return m_origin + m_comp[PARAM_COMP] * tcomp;
  } else {
    return (m_comp[NUMBER_ATOMS] * (tcomp - m_origin))
        .head(m_rank_of_space - 1);
  }
}

//*************************************************************

Eigen::VectorXd ParamComposition::calc_param_composition(
    const Eigen::VectorXd &num_atoms_per_prim) const {
  return (m_comp[NUMBER_ATOMS] * (num_atoms_per_prim - m_origin))
      .head(m_rank_of_space - 1);
}

//*************************************************************

Eigen::VectorXd ParamComposition::calc_num_atoms(
...
// Given an origin and spanning vectors, returns a ParamComposition object that
// points to the same Prim as (*this)
ParamComposition ParamComposition::calc_composition_object(
  ...
  // std::cerr << "ERROR in ParamComposition::calc_composition_object the
  // spanning vectors are not as long as the number of "; std::cerr <<
  // "components in this system. I'm confused and recommend you quit and try
  // again. However, not going to force quit\n";
  //}
  ...
  if (tspanning.size() < (components().size())) {
    Eigen::FullPivHouseholderQR<Eigen::MatrixXd> qr(
        tmat.leftCols(tspanning.size()));
    Eigen::MatrixXd torthogonal = qr.matrixQ();
    tmat.rightCols((components().size() - tspanning.size())) =
        torthogonal.rightCols((components().size() - tspanning.size()));
    // copy the orthogonalized vectors into tmat. We now have a
    // complete spanning set in this component space
  }
  return ParamComposition(allowed_occs(), tmat, torigin, m_rank_of_space,
                          PARAM_COMP);
}
...
void ParamComposition::calc_transformation_matrices() {
  ...
  if (m_spanning_end_members.size() < components().size()) {
    Eigen::FullPivHouseholderQR<Eigen::MatrixXd> qr(
        tmat.leftCols(m_spanning_end_members.size()));
    Eigen::MatrixXd torthogonal = qr.matrixQ();
    tmat.rightCols((components().size() - m_spanning_end_members.size())) =
        torthogonal.rightCols(
            (components().size() - m_spanning_end_members.size()));
    // copy the orthogonalized vectors into tmat. We now have a
    // complete spanning set in this component space
  }
  if (m_comp.size() != 2) {
...
void ParamComposition::calc_spanning_end_members() {
  ...
  if (m_rank_of_space <= 0) {
    std::cerr << "WARNING something is wrong in "
                 "ParamComposition::spanning_end_members. The rank_of_space in "
                 "the ParamComposition object is <=0. I do not know how to "
                 "calculate the end_members in such a space"
              << std::endl;
...
void ParamComposition::select_composition_axes(const Index &choice) {
  ...
  if (m_allowed_list.size() < choice + 1) {
    std::cerr << "ERROR in ParamComposition::select_composition_axes. Your "
                 "value of choice is outside the range of allowed_list"
              << std::endl;
    exit(666);
  }
