...
class StructureMap<Configuration> {
 public:
  ...
  /// exist or be valid. \param req_properties, list of names of properties that
  /// are required for mapped data to be considered 'complete' \param hint
  /// std::unique_ptr<Configuration> to 'from' config for 'casm update', or
  /// 'end' if unknown as with 'casm import'. \param result Insert iterator of
  /// Result objects to output mapping results
  ///
  ...
  // TODO: get rid of req_properties, and have it checked on-the-fly at point
  // where it is needed
...
class Import<Configuration> : public ImportT<Configuration> {
 public:
  ...
  /// \brief Constructor
  Import(const PrimClex &primclex, const StructureMap<ConfigType> &mapper,
         ImportSettings const &_set, std::string const &report_dir);
...
class Update<Configuration> : public UpdateT<Configuration> {
 public:
  ...
  static const std::string desc;
  static int run(const PrimClex &primclex, const jsonParser &kwargs,
                 const Completer::UpdateOption &import_opt);
