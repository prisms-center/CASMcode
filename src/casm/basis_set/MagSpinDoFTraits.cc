...
/// site
std::vector<BasisSet> MagSpinDoFTraits::construct_site_bases(
  ...
  auto const &mag_spin_dof_specs =
      get<MagSpinDoFSpecs>(name(), _basis_function_specs);

  for (Index b = 0; b < _prim.basis().size(); b++) {
    if (!_prim.basis()[b].has_dof(name())) continue;
    BasisSet tresult;
    CASM::DoFSet adapted_dofset =
        adapter::Adapter<CASM::DoFSet, xtal::SiteDoFSet>()(
            _prim.basis()[b].dof(name()),
            _prim.site_dof_symrep_IDs()[b].at(name()), b);
    tresult.set_variable_basis(adapted_dofset);
    Array<BasisSet const *> tsubs(1, &tresult);
    result[b].construct_harmonic_polynomials(
        tsubs, mag_spin_dof_specs.max_poly_order, 1, false);
    result[b].get_symmetry_representation(_prim.factor_group());

    result[b].set_name(name() + "_site_func");
    // std::cout << "+:+:+:+Created variable set for site " << b << ", size " <<
    // result[b].size() << "\n";
  }
  return result;
}

void MagSpinDoFTraits::parse_dof_specs(InputParser<BasisFunctionSpecs> &parser,
...
void MagSpinDoFTraits::dof_specs_to_json(
    const BasisFunctionSpecs &basis_function_specs, jsonParser &json,
    Structure const &prim) const {
  MagSpinDoFSpecs const &magspin_dof_specs =
      get<MagSpinDoFSpecs>(this->name(), basis_function_specs);
...
std::unique_ptr<DoFSpecs> magspin_bfuncs(std::string const &flavor_name,
                                         Index max_poly_order = -1) {
  return notstd::make_unique<DoF_impl::MagSpinDoFSpecs>(flavor_name,
                                                        max_poly_order);
}
...
void parse(InputParser<DoF_impl::MagSpinDoFSpecs> &parser,
  ...
  if (parser.valid()) {
    parser.value = notstd::make_unique<DoF_impl::MagSpinDoFSpecs>(
        flavor_name, max_poly_order);
  }
}

void to_json(const DoF_impl::MagSpinDoFSpecs &magspin_dof_specs,
             jsonParser &json) {
  json.put_obj();
