...
struct EqMapRow {
  ...
  ///   tmp_element[b] == g[tmp_eq_map[b][j]]*g[a2proto]*tmp_element[a]; // for
  ///   all j

  EqMapRow(Index _a, Index _b,
           const std::vector<std::vector<Index>> &tmp_eq_map, const SymGroup &g)
      : a(_a), b(_b) {
    try {
      ...
      throw libcasm_runtime_error(
          "Error in Orbit constructor: Failed constructing EqMapRow.");
    }
  };
...
struct RelEqMap {
  /// generate eq_map row for tmp_element[b] relative to tmp_element[a]
  RelEqMap(Index _a, const std::vector<std::vector<Index>> &tmp_eq_map,
           const SymGroup &g)
      : a(_a) {
    try {
      ...
      throw libcasm_runtime_error(
          "Error in Orbit constructor: Failed constructing RelEqMap.");
    }
  }
  ...
  bool operator<(const RelEqMap &other) const { return this->map < other.map; }
};
...
/// \brief Construct an Orbit from a generating_element Element, using provided
/// Group
///
...
Orbit<_SymCompareType>::Orbit(Element generating_element,
  ...
  //  X   :   op(0)   op(1)   ...
  // proto:    0       ?       ?   (<-- proto.invariant_subgroup(g,
  // sym_compare)) el(1):  (the element generated by the first op that doesn't
  // result in proto) el(2):  (the element generated by the first op that
  // doesn't result in proto or el(1))
  //  ... :  ( etc. )
  ...
  // store ordered symop indices corresponding to transformations for each
  // equivalent
  std::map<Element, std::set<Index>, decltype(compare)> t_equiv(compare);
  try {
    for (Index i = 0; i < g.size(); i++) {
      t_equiv[m_sym_compare.prepare(
                  m_sym_compare.copy_apply(g[i], generating_element))]
          .insert(i);  // not g[i].index()!!;
    }
  } catch (const std::exception &e) {
    throw libcasm_runtime_error(
        "Error in Orbit constructor: Failed generating unique equivalents.");
  }
  ...
  try {
    ...
    for (Index j = 1; j < tmp_equivalence_map.size(); ++j) {
      if (tmp_equivalence_map[0].size() != tmp_equivalence_map[j].size()) {
        throw libcasm_runtime_error(
            "Error in Orbit constructor: equivalence map is not rectangular");
      }
    }
  } catch (const std::exception &e) {
    throw libcasm_runtime_error(
        std::string("Error in Orbit constructor: ") +
  ...
  /// ties are resolved by virtue of fact that 'tmp_element' is already sorted
  /// by m_sym_compare.compare
  Index best_a = 0;
  ...
  try {
    ...
    throw libcasm_runtime_error(
        "Error in Orbit constructor: Failed generating sorted equivalence "
        "map.");
  }
  ...
  try {
    ...
    for (const auto &row : best.map) {
      ...
      Element tequiv = m_sym_compare.prepare(
          m_sym_compare.copy_apply(g[proto2i], tmp_element[newproto_i]));
      SymOp ttrans = m_sym_compare.spatial_transform();

      m_element.push_back(m_sym_compare.copy_apply(ttrans * g[proto2i],
                                                   tmp_element[newproto_i]));
      ...
      for (const auto &cg_j : best.map.begin()->values) {
        ...
        tequiv = m_sym_compare.prepare(
            m_sym_compare.copy_apply(g[value], tmp_element[newproto_i]));
    ...
    throw libcasm_runtime_error(
        "Error in Orbit constructor: Failed copying sorted elements and "
        "equivalence map.");
  }

  if (m_equivalence_map[0][0].index() != 0) {
    throw libcasm_runtime_error(
        "Error in Orbit constructor: First equivalence map element is not "
        "identity.");
  }
}
...
Orbit<_SymCompareType> &Orbit<_SymCompareType>::apply_sym(const SymOp &op) {
  ...
  for (auto it = m_equivalence_map.begin(); it != m_equivalence_map.end();
       ++it) {
...
/// - Assume range of orbit is sorted according to
...
OrbitIterator find_orbit(OrbitIterator begin, OrbitIterator end, Element e) {
  ...
  auto _range =
      std::equal_range(invariants_iterator(begin), invariants_iterator(end),
                       e_invariants, compare);

  // find if any of the orbits in range [_range.first, _range.second) contain
  // equivalent
  auto contains = [&](const orbit_type &orbit) { return orbit.contains(e); };
