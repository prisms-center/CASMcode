...
class Function : public HierarchyID<Function> {
 ...
 protected:
  ...
  /// m_label_format sets the label format used to generate a label string for a
  /// Function object. It is specified as a string of the form (substr1 + "%a" +
  /// substr2 + "%b" + substr3 + ... ), where "%a" and "%b" are flags that
  /// specify object-specific values. The following flags are allowed:
  ...
  /// For objects where the flag does not uniquely specify a single value, it
  /// evaluates to a substring that concatenates multiple values in ascending
  /// order. Example: For a polynomial function that combines DoFs from the
  /// sites {8, 2, 4} of the neighborlist,
