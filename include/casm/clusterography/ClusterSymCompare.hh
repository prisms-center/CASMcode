...
namespace CASM {
  ...
  */
  ...
  /// \brief CRTP Base class for common cluster symmetry and comparison implementations
  ///
  /// ClusterSymCompare can be used for common implementations of SymCompareType methods for
  /// cluster orbits. To do so, it requires that a traits class (traits<SymCompareType>) is
  /// implemented with:
  ...
  ///
  /// The methods ClusterSymCompare implements are overriden if an implementation exists in
  /// final SymCompareType implementations. For example, this is done by WithinScelSymCompare to
  /// ensure all sites are within the supercell before finding the canonical transform permutation.
  ///
  ...
  public:
    ...
    using typename Base::Element; /// Element (of orbit) refers to Cluster, not element of Cluster
  ...
  ///
  /// Before doing a comparison, AperiodicSymCompare does not spatially transform the cluster in
  /// any way, as opposed to PrimPeriodSymCompare which translates the cluster so that the first
  /// site is within the origin unit cell.
  ///
  ...
  ///
  /// Before doing a comparison of cluster sites, PrimPeriodicSymCompare translates the cluster so
  /// that the first site is within the origin unit cell.
  ///
  /// Beyond ClusterSymCompare requirements, requires:
  /// - Element must be translatable (Element + UnitCell, Element - Unitcell are valid)
  /// - 'static xtal::UnitCellCoord traits<PrimPeriodicSymCompare<Element>>::position(
  ///     Element const&,
  ...
  /// \brief Comparisons of clusters using supercell periodic symmetry, but without periodic images
  ///
  /// Before doing a comparison, ScelPeriodicSymCompare translates the cluster so that the first
  /// site is within the supercell, as opposed to WithinScelSymCompare which moves all sites in the
  /// cluster within the supercell.
  ///
  /// ScelPeriodicSymCompare uses the direct distance between sites for orbit invariants without
  /// accounting for periodic images, while WithinScelSymCompare uses supercell periodic image
  /// minimum distance for the orbit invariants.
  ///
  /// Beyond ClusterSymCompare requirements, requires:
  /// - Element must be translatable (Element + UnitCell, Element - Unitcell are valid)
  /// - static xtal::UnitCellCoord traits<PrimPeriodicSymCompare<Element>>::position(
