...
TEST(ExampleClusterographyIntegralCluster, ClusterInvariants) {
  ...
  // Cluster invariants, properties of the cluster that invariant to application of symmetry
  //   are useful for understanding, comparing, filtering, etc. The particular properties that
  //   are invariant may be different in different contexts. For instance, when considering a
  //   system with or without periodic boundary conditions, or clusters in the infinite periodic
  //   crystal vs clusters around a particular local site, the number of sites in the cluster is
  //   always invariant, but the relevant site-to-site distance may be different.
  ...
  // The basic IntegralCluster invariants are cluster size and site-to-site distances
  ...
  CASM::WithinScelClusterInvariants within_scel_invariants {cluster, superlattice.transformation_matrix_to_super()};
