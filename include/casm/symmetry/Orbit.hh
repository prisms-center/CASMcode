...
/// includes any necessary tolerance for floating point comparison. See
/// `SymCompare` for how to implement the necessary methods.
///
/// The following relationships will be valid:
///    element(i) = sym_compare.copy_apply(equivalence_map()[i][j],
///    prototype()), for all j < equivalence_map()[i].size()
///    equivalence_map()[i][j] = t * g;
///      where g is a generating group element,
///      and t is the "spatial_transform" defined by the SymCompareType for
///      equivalent elements such that:
///    sym_compare.representation_prepare(element(i)) ==
///      copy_apply(t, sym_compare.representation_prepare(copy_apply(g,
///      prototype)))
///
...
class Orbit : public Comparisons<CRTPBase<Orbit<_SymCompareType>>> {
 public:
  ...
  ///
  /// \returns element(i) compares equivalent to
 ...
 private:
  ...
  /// \brief All symmetrically equivalent elements (excluding those that
  /// SymCompare equivalent)
  ///
  std::vector<Element> m_element;

  /// \brief element(i) compares equivalent to
  /// prototype().copy_apply(m_equivalence_map[i][j]) for all j
...
/// \brief Iterator over Generators (potential prototypes) and insert resulting
/// orbits into 'result' iterator
