...
TEST(ExampleCrystallographyLattice, LatticeVectors) {
  ...
  CASM::xtal::Lattice lattice{
      a,
      b,
      c,
  };

  // Check floating point equivalence with CASM::almost_equal, using default
  // tolerance
  ...
  // Access the lattice vectors as a column matrix via
  // 'Lattice::lat_column_mat() const'
...
TEST(ExampleCrystallographyLattice, LatticeConstructors) {
  ...
  // A tolerance used for crystallographic comparisons is included in Lattice.
  // By default the tolerance is set to CASM::TOL.
  ...
  // You can also construct a lattice with the lattice vectors as a column
  // matrix
  Eigen::Matrix3d M;
  M << 1., 0., 0., 0., 2., 0., 1., 0., 3.;
...
TEST(ExampleCrystallographyLattice, LatticeProperties) {
  ...
  // get lattice vector angles in degrees: lattice.angle(i) is the angle between
  // vectors (i+1)%3, (i+2)%3
  ...
  EXPECT_TRUE(almost_equal(reciprocal_lattice[0],
                           2 * M_PI * vec2.cross(vec3) / lattice.volume()));
  EXPECT_TRUE(almost_equal(reciprocal_lattice[1],
                           2 * M_PI * vec3.cross(vec1) / lattice.volume()));
  EXPECT_TRUE(almost_equal(reciprocal_lattice[2],
                           2 * M_PI * vec1.cross(vec2) / lattice.volume()));

  CASM::xtal::Lattice lattice_check = reciprocal_lattice.reciprocal();
  EXPECT_TRUE(
      almost_equal(lattice_check.lat_column_mat(), lattice.lat_column_mat()));
}
...
TEST(ExampleCrystallographyLattice, ReciprocalLattice) {
  ...
  EXPECT_TRUE(lattice_2.is_right_handed());
  EXPECT_TRUE(
      almost_equal(lattice_2.lat_column_mat(), lattice_1.lat_column_mat()));
}
