...
/** \defgroup ClusterOrbits
    ...
    "ClusterOrbits" (as opposed to IntegralClusterOrbits) has no dependency on
   casm/clusterography/ClusterSymCompare.
*/
...
struct IntegralClusterOrbitGenerator {
  IntegralClusterOrbitGenerator(const IntegralCluster &_prototype,
...
class OrbitBranchSpecs {
 public:
  ...
  /// \param _begin,_end Iterators over candidate sites that should be
  /// considered
  ///                    when determining prototype clusters
  /// \param _generating_grp Group used by the Orbit constructor
  /// \param _filter Function or functor implementing 'bool
  /// filter(ClusterType)', which
  ...
  ///                     comparison with canonicalization, taking into account
  ///                     symmetry
  ///
  template <typename SiteIterator>
  OrbitBranchSpecs(const PrimType &_prim, SiteIterator _begin,
                   SiteIterator _end, const SymGroup &_generating_grp,
                   Filter _filter, const SymCompareType &_sym_compare)
      : m_prim_ptr(&_prim),
  ...
  const SymGroup &generating_group() const { return *m_generating_grp; }
...
OutputIterator prim_periodic_orbit_neighborhood(
    const PrimPeriodicOrbit<IntegralCluster> &orbit, OutputIterator result);
...
/// This simply outputs all UnitCellCoord for clusters that include the origin
/// UnitCell, without any standard order. It uses all clusters that touch origin
/// unitcell, including translationally equivalent clusters. Respects
/// translational properties of local orbits, so can be used when translational
/// type is unknown.
///
///
/// \ingroup IntegralCluster
