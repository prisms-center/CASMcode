...
void k_best_mapping_test(xtal::SimpleStructure const &sstruc, double d) {
  ...
  {
    std::string comment("Check for all mappings better than the pure swap "
...
void sym_mapping_test(xtal::BasicStructure struc, Index N) {
  ...
  {
    std::string comment("Check that we find 8 perfect mapping for a Vol8 "
  ...
  {
    std::string comment("Check for perfect mappings using the best-1 calling "
  ...
  {
    std::string comment("Check for perfect mappings using the best-1000 "
...
TEST(SymInvariantMappingTest, Hcp) {
  ...
  for(double ca = ca_min; ca <= ca_max;) {
    ...
    use_child_sym ? child_fg : decltype(child_fg) {
      child_fg[0]
    });
