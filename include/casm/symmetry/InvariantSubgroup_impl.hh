...
namespace CASM {
  ...
  ///   if(sym_compare.equal(e, sym_compare.prepare(sym_compare.copy_apply(*it, e)))) {
  ...
  OpOutputIterator make_invariant_subgroup(Element const &element,
    ...
    Element e {sym_compare.prepare(element)};
    for(auto group_it = group_begin; group_it != group_end; ++group_it) {
      if(sym_compare.equal(e, sym_compare.prepare(sym_compare.copy_apply(*group_it, e)))) {
  ...
  ///   if(sym_compare.equal(e, sym_compare.prepare(sym_compare.copy_apply(permute_it->sym_op(), e)))) {
  ///     *result++ = permute_it;
  ///   }
  /// }
  /// return result;
  ...
  SymGroup make_invariant_subgroup(const Element &element,
    ...
    SymGroup result = generating_grp;
    result.clear();
    make_invariant_subgroup(element, generating_grp.begin(), generating_grp.end(), sym_compare, std::back_inserter(result));
    if(result[0].index() != 0) {
