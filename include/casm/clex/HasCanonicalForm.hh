...
class CanonicalForm : public Base {
 public:
  ...
  /// Check if canonical in Supercell, with respect to a subgroup of Supercell
  /// permutations
  template <typename PermuteIteratorIt>
  bool is_canonical(const Supercell &scel, PermuteIteratorIt begin,
                    PermuteIteratorIt end) const;

  /// Return canonical form in Supercell, with respect to a subgroup of
  /// Supercell permutations
  template <typename PermuteIteratorIt>
...
/// \brief Implements canonical form finding for Configuration and
/// DiffTransConfiguration
///
/// Requires MostDerived implements:
/// - Functor less() const;
///   - Functor is a `ConfigCompare` like functor that can compare A*config <
///   B*other
...
class SupercellCanonicalForm : public Base {
 public:
  ...
  ///   derived().lattice().is_equivalent(op, canonical_form().lattice())) ==
  ///   true,
