...
class MonteCarloDirectoryStructure {
 public:
  ...
  /// \brief "output_dir/"
  fs::path output_dir() const { return m_output_dir; }

  /// \brief Results summary: "output_dir/results.csv"
  fs::path results_csv() const { return m_output_dir / "results.csv"; }

  /// \brief Results summary: "output_dir/results.json"
  fs::path results_json() const { return m_output_dir / "results.json"; }

  /// \brief "output_dir/conditions.cond_index/"
  fs::path conditions_dir(int cond_index) const {
    return m_output_dir /
           (std::string("conditions.") + std::to_string(cond_index));
  }
  ...
  fs::path POSCAR_snapshot(int cond_index, size_type sample_index) {
    return trajectory_dir(cond_index) /
           (std::string("POSCAR.") + std::to_string(sample_index));
  }
...
/// \brief Will create new file or append to existing file results of the latest
/// run
template <typename MonteType>
