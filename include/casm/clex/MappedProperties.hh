...
namespace CASM {
  ...
  /// - "origin" (std::string), indicates the source of the properties. This can vary depending on
  ///   the context, but most typically it is a path to a `properties.calc.json` type file. In some
  ///   cases "prim:" is prepended to the file path to indicate that the "to" configuration is the
  ///   primitive of the structure read from the origin file.
  /// - "to" (std::string), the name of the Configuration that the properties are mapped to. It must
  ///   be a Configuration name.
  /// - "init_config" (std::string), used to indicate the Configuration that was calculated to
  ///   generate these properties. Typically, it is the name of the Configuration which was used
  ///   to generate the initial structure input to the calculation. The string "import" is typically
  ///   used when the properties were imported from outside the project and no initial ideal
  ///   Configuration is known.
  /// - "file_data" (FileData), used to store the last write time of the "origin" file
  ///
  /// Note: In the future, "origin", "to", "init_config", and "file_data" may be moved out of
  /// MappedProperties and stored as part of the PropertiesDatabase or as needed by `casm update`
  /// and `casm import`.
  ///
  struct MappedProperties {
    ...
    ///
    /// Typical usage: The path to the `properties.calc.json` type file containing the
    /// SimpleStructure with these properties.
    std::string origin;

    /// The name of the configuration that these properties are mapped to (the ending point of
    /// relaxation)
    std::string to;
