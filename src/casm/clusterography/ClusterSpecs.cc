...
namespace CASM {
  ...
  // }
  //
  // std::set<std::set<Index>> ClusterSpecs::make_within_scel_orbit_generators(std::ostream &status) const {
  ...
  // }
  //
  // std::set<std::set<Index>> ClusterSpecs::_make_within_scel_orbit_generators(std::ostream &status) const {
  //   throw std::runtime_error("Error: make_within_scel_orbit_generators not implemented for '" + name() + "'");
  // }
  ...
  //   sym_info(_sym_info),
  //   generating_group(_generating_group),
  //   inverse_permutations(make_inverse_permutations(generating_group.begin(), generating_group.end())),
  //   sym_compare(shared_prim, sym_info->transformation_matrix_to_super(), shared_prim->lattice().tol()),
  ...
  //       }
  //       else {
  //         candidate_sites_f = scel_neighborhood(sym_info->transformation_matrix_to_super());
  //       }
  //     }
  ...
  //   SymGroup generating_sym_group = make_sym_group(generating_group, sym_info->supercell_lattice());
  //
  //   return generate_orbits(generating_elements, generating_sym_group, sym_compare);
  // }
  //
  // ClusterSpecs::WithinScelOrbitVec WithinScelMaxLengthClusterSpecs::_make_within_scel_orbits(
  //   std::ostream &status) const {
  ...
  //
  //   for(int branch = 0; branch < max_length.size(); ++branch) {
  //     auto current_candidate_sites = candidate_sites[branch](*shared_prim, site_filter);
  //     specs.emplace_back(
  //       *shared_prim,
  ...
  // }
  //
  // std::set<std::set<Index>> WithinScelMaxLengthClusterSpecs::_make_within_scel_orbit_generators(
  // std::ostream &status) const {
  //
  //   // "orbit branch b" is all orbits with b sites per cluster
  //   //
  //   // This method creates orbits recursively by adding candidate sites to orbits in the previous
  ...
  //   std::copy(current_branch.begin(), current_branch.end(), std::inserter(orbit_generators, orbit_generators.end()));
  //
  ...
  //         Index site_index = sym_info->unitcellcoord_index_converter()(site_uccoord);
  ...
  //
  //         // check that the test cluster passes the filter (checks max_length, cutoff_radius)
  //         IntegralCluster test_cluster = make_cluster(test, *shared_prim, *sym_info);
  //         if(!current_cluster_filter(test_cluster)) {
  //           continue;
  //         }
  //
  //         // apply permutations to get the canonical site indices for this orbit
  //         std::set<Index> canonical = make_canonical_cluster_site_indices(
  ...
  //     std::set<Index> test = make_cluster_site_indices(custom.prototype, *sym_info);
  //     std::set<Index> canonical = make_canonical_cluster_site_indices(
  //       inverse_permutations.begin(),
  //       inverse_permutations.end(),
  //       test);
  ...
  //       while(it != end) {
  //         std::set<Index> subcluster_test = make_cluster_site_indices(*it++, *sym_info);
  //         std::set<Index> subcluster_canonical = make_canonical_cluster_site_indices(
  //           inverse_permutations.begin(),
  //           inverse_permutations.end(),
  //           subcluster_test);
