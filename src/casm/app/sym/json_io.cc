...
jsonParser &to_json(const Completer::SymOption &sym_opt, jsonParser &json) {
  ...
  if (vm.count("lattice-point-group")) {
    json["print_lattice_point_group"] =
        static_cast<bool>(vm.count("lattice-point-group"));  // bool
  }
  if (vm.count("factor-group")) {
    json["print_factor_group"] =
        static_cast<bool>(vm.count("factor-group"));  // bool
  }
  ...
  if (vm.count("dof-space-analysis")) {
    json["dof_space_analysis"] =
        static_cast<bool>(vm.count("dof-space-analysis"));  // bool
  }
