...
namespace CASM {
  ...
  ///
  xtal::ScelEnumProps jsonConstructor<xtal::ScelEnumProps>::from_json(const jsonParser &json) {
    jsonParser tjson {json};
    InputParser<xtal::ScelEnumProps> parser {tjson};

    std::runtime_error error_if_invalid {"Error reading xtal::ScelEnumProps from JSON"};
