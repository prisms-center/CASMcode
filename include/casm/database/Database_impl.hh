...
ValDatabase<ValueType>::set_alias(const std::string &name_or_alias,
...
template <typename ValueType>
std::string ValDatabase<ValueType>::name(
...
template <typename ValueType>
std::string ValDatabase<ValueType>::alias(
    const std::string &name_or_alias) const {
  auto it = m_alias_to_name.find(name_or_alias);
...
void ValDatabase<ValueType>::write_aliases() {
  if (!primclex().has_dir()) {
    throw std::runtime_error(
        "Error in write_aliases: CASM project has no root directory.");
  }
  fs::path p = primclex().dir().template aliases<ValueType>();
