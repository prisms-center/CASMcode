...
namespace CASM {
  ...
  // Note:
  // - The following formatters are all templated to expect a ConfigEnumDataType that follows the
  //   minimum interface defined by `ConfigEnumData`
  // - Besides the functions included here, you can also use `make_datum_formatter_adapter` to adapt
  //   DatumFormatters from ConfigIO or ScelIO for use with ConfigEnumDataType. Examples:
  //   - `make_datum_formatter_adapter<ConfigEnumDataType, Configuration>(ConfigIO::scelname())`
  //   - `make_datum_formatter_adapter<ConfigEnumDataType, Supercell>(ScelIO::TransfMat())`
  //   - Note that `make_datum_formatter_adapter` has overrides that can change the formatter name
  //     if that is appropriate.
  //   - Using `make_datum_formatter_adapter` requires defining an adapter::Adapter

  namespace adapter {

    template<typename EnumeratorType, typename InitialStateType>
    Configuration const &Adapter<Configuration, ConfigEnumData<EnumeratorType, InitialStateType>>::operator()(
    ConfigEnumData<EnumeratorType, InitialStateType> const &adaptable) const {
      return adaptable.configuration;
    }

    template<typename EnumeratorType, typename InitialStateType>
    Supercell const &Adapter<Supercell, ConfigEnumData<EnumeratorType, InitialStateType>>::operator()(
    ConfigEnumData<EnumeratorType, InitialStateType> const &adaptable) const {
  ...
  namespace ConfigEnumIO {
    ...
    GenericDatumFormatter<std::string, ConfigEnumDataType> name() {
      ...
      [](ConfigEnumDataType const & data) -> std::string {
        ...
        if(data.insert_result.canonical_it == data.primclex.template db<Configuration>().end()) {
          return "none";
        }
    ...
    GenericDatumFormatter<bool, ConfigEnumDataType> selected() {
      ...
      [](ConfigEnumDataType const & data) {
        ...
        return data.insert_result.canonical_it != data.primclex.template db<Configuration>().end();
      });
    }
    ...
    GenericDatumFormatter<bool, ConfigEnumDataType> is_new() {
      return GenericDatumFormatter<bool, ConfigEnumDataType>(
               "is_new",
               "True if canonical configuration did not exist before insertion. False if not checked or was existing before insertion.",
      [](ConfigEnumDataType const & data) {
        if(data.is_excluded_by_filter) {
          return false;
        }
    ...
    GenericDatumFormatter<bool, ConfigEnumDataType> is_existing() {
      return GenericDatumFormatter<bool, ConfigEnumDataType>(
               "is_existing",
               "True if canonical configuration did exist before insertion. False if not checked or not existing before insertion.",
      [](ConfigEnumDataType const & data) {
        if(data.is_excluded_by_filter) {
          return false;
        }
    ...
    GenericDatumFormatter<Index, ConfigEnumDataType> initial_state_index() {
      ...
      [](ConfigEnumDataType const & data) {
        return data.initial_state_index;
      });
    }
    ...
    GenericDatumFormatter<std::string, ConfigEnumDataType> initial_state_name() {
      return GenericDatumFormatter<std::string, ConfigEnumDataType>(
               "initial_state_name",
               "Initial state name. Typically the initital state configuration name plus description of selected sites.",
      [](ConfigEnumDataType const & data) {
        return data.initial_state_name;
      });
    }
    ...
    Generic1DDatumFormatter<std::vector<Index>, ConfigEnumDataType> selected_sites() {
      return Generic1DDatumFormatter<std::vector<Index>, ConfigEnumDataType>(
               "selected_sites",
               "Indices of selected sites in the initial state configuration. JSON output only.",
      [](ConfigEnumDataType const & data) {
    ...
    Generic1DDatumFormatter<Eigen::VectorXd, ConfigEnumDataType> normal_coordinate() {
      ...
      return Generic1DDatumFormatter<Eigen::VectorXd, ConfigEnumDataType>(
               "normal_coordinate",
               "Normal coordinate used to generate DoF value. Supported enumerators only.",
