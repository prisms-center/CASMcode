...
namespace CASM {
  ...
  ///   - Use this method to write symmetry files if the crystallography tolerance in project
  ...
  void write_prim_symmetry_impl(Structure const &prim,
    ...
    // Selects coordinate printing mode (CART vs FRAC) == coordtype until "C" goes out of scope
  ...
  void write_prim_symmetry(PrimClex &primclex,
    ...
    std::map<std::string, std::string> cli_to_combined_keys {
      {"print_lattice_point_group", "print_lattice_point_group"},    // --lattice-point-group
      {"print_factor_group", "print_factor_group"},                  // --factor-group
      {"print_crystal_point_group", "print_crystal_point_group"},    // --crystal-point-group
    ...
    parser.optional_else(print_lattice_point_group, "print_lattice_point_group", false);
