...
namespace CASM {
  namespace DataStream_impl {
    ...
    char DataStreamPromoter<char>::promote(double a) {
      throw std::runtime_error(std::string("No viable conversion to type 'char' from double ") + std::to_string(a) + "\n");
      return round(a);
    }
    ...
    bool DataStreamPromoter<bool>::promote(std::string a) {
      ...
      if(a == "false")
        return false;
      throw std::runtime_error("No viable convertion to type 'bool' from string '" + a + "\n");
    }
    ...
    bool DataStreamPromoter<bool>::promote(char a) {
      ...
      if(toupper(a) == 'T')
        return true;
      throw std::runtime_error("No viable convertion to type 'bool' from char '" + std::string(1, a) + "\n");
    }
