...
struct ConfigEnumStrainParams {
  ...
  /// The positive coordinate space defined by the SubWedge axes are a
  /// symmetrically unique portion of the strain space. It may require multiple
  /// SubWedge to fill strain space, hence the vector.
  ///
  /// Note:
  ...
  // subwedge axes columns.

  Eigen::VectorXd min_val;
  Eigen::VectorXd max_val;
  Eigen::VectorXd inc_val;

  // If true, set min_val[i]=-max_val[i], for strain components, i, with
  // multiplicity==1 Typically, set to true if using symmetry adapted axes
  bool auto_range = true;

  /// If true, fit an ellipsoid inside the grid defined by min_val/max_val and
  /// exclude grid points that lie outside the ellipsoid
...
class ConfigEnumStrain : public InputEnumeratorBase<Configuration> {
 ...
 private:
  ...
  const PermuteIterator &_perm_begin() { return m_perm_begin; }
  const PermuteIterator &_perm_end() { return m_perm_end; }
};
