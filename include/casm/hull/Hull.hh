...
/// \brief Generate and inspect the convex hull generated from a selection of
/// Configurations
///
/// - The underlying Qhull object may not be copy constructed or assigned, so
/// neither can this class. Instead use shared_ptr<Hull>.
class Hull {
 public:
  ...
  typedef std::pair<notstd::cloneable_ptr<CompCalculator>,
                    notstd::cloneable_ptr<EnergyCalculator> >
      CalculatorPair;
  ...
  Hull(const DB::Selection<Configuration> &_selection,
       ...
       const EnergyCalculator &_energy_calculator =
           ConfigIO::formation_energy_per_species(),
       double _singular_value_tol = 1e-14, double _bottom_facet_tol = 1e-14);

  /// \brief const Access the hull object directly
  const orgQhull::Qhull &data() const;

  /// \brief Orthogonal transformation matrix from a point in full comp/energy
  /// space to dimension-reduced comp/energy space
  const Eigen::MatrixXd &reduce() const;

  /// \brief Orthogonal transformation matrix from a point in the
  /// dimension-reduced comp/energy space to the full comp/energy space
  ...
  /// \brief Return the 0K ground state corresponding to the input chemical
  /// potential
  ...
  /// \brief Return a vector corresponding to the coordinate of a given
  /// configuration in full composition/energy space
  Eigen::VectorXd point(const Configuration &config) const;

  /// \brief Return a vector corresponding to the coordinate of a given
  /// configuration in the reduced composition/energy space
  Eigen::VectorXd reduced_point(const Configuration &config) const;

  /// \brief The distance a Configuration is above the hull along the energy
  /// axis
  double dist_to_hull(const Configuration &config) const;

  /// \brief The distance a point in the reduced composition/energy space is
  /// above the hull along the energy axis
 ...
 private:
  ...
  // transform full dimension comp/energy vector onto subspace
  // range(comp)/energy
  Eigen::MatrixXd m_reduce;

  // all 'bottom' facets, along with projection of 'm_bottom' along the unit
  // outward normal
