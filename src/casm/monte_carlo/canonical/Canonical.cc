...
namespace CASM {
  ...
  const Monte::ENSEMBLE Canonical::ensemble = Monte::ENSEMBLE::Canonical;
  ...
  Canonical::Canonical(PrimClex &primclex, const CanonicalSettings &settings, Log &log):
    ...
    m_occ_converter(primclex.get_prim(), primclex.orbitree(m_formation_energy_clex)),
    m_cand(primclex.get_prim(), primclex.orbitree(m_formation_energy_clex)),
    m_occ_loc(m_occ_converter, m_cand),
  ...
  const Canonical::EventType &Canonical::propose() {
    m_event.canonical_swap() = m_occ_loc->propose(m_canonical_swap, _mtrand());
  ...
  void Canonical::accept(const EventType &event) {
    ...
    // Apply occ mods:
    _configdof().occ(event.canonical_swap.mutating_site_a) =
      m_occ_converter.occ_index(event.canonical_swap.asym_unit_a, event.canonical_swap.species_index_b);
    _configdof().occ(event.canonical_swap.mutating_site_b) =
      m_occ_converter.occ_index(event.canonical_swap.asym_unit_b, event.canonical_swap.species_index_a);

    /// Update occ locations table
    m_occ_loc->update(event.canonical_swap);
  ...
  void Canonical::_set_nlist(Index l) {
  ...
  void Canonical::_set_dCorr(CanonicalEvent &event) const {
    ...
    const MCOccEvent &e = event.event();
    const MCOccTransform &fa = e.occ_transform[0];
    const MCOccTransform &fb = e.occ_transform[1];

    int curr_occ_a = _config().occ(f_a.l);
    int new_occ_a = m_convert.occ_index(f_a.asym, f_a.to_specie);
    int new_occ_b = m_convert.occ_index(f_b.asym, f_b.to_specie);
    ...
    _config().set_occ(f_a.l, new_occ_a);
    ...
    _config().set_occ(f_a.l, curr_occ_a);
  ...
  ///
  /// - Find the GrandCanonicalSwap that by applying minimizes:
  ///     (comp_n - conditions().mol_composition()).norm()
  /// - If cannot be improved, return end
  ///
  std::vector<GrandCanonicalSwap>::iterator
  Canonical::_find_grand_canonical_swap(
    const Configuration &config,
    std::vector<MCOccSwap>::iterator begin,
    std::vector<MCOccSwap>::iterator end) const {
    ;
    ...
    for(; it != end; ++it) {
      if(m_occ_loc->cand_size(swap.cand_a)) {
        Eigen::VectorXd tcomp_n = comp_n;
        tcomp_n[swap.cand_a.species_index] -= dn;
        tcomp_n[swap.cand_b.species_index] += dn;
  ...
  ///     (comp_n - conditions().mol_composition()).norm()
  ///
  ConfigDoF Canonical::_enforce_conditions(Config configdof) const {

    _log().custom("Enforce composition");
    Configuration tconfig(_supercell(), jsonParser(), configdof);
    m_occ_loc->initialize(tconfig);

    _log() << "initial   comp: " << jsonParser(CASM::comp(tconfig).transpose()) << std::endl;
    _log() << "initial comp_n: " << jsonParser(CASM::comp_n(tconfig)) << std::endl;

    int count = 0;
    while(true) {
      auto begin = m_cand.grand_canonical_swaps().begin();
      auto end = m_cand.grand_canonical_swaps().end();
      auto it = _find_grand_canonical_swap(tconfig, begin, end);

      if(it == end) {
        _log() << "applied " << count << " swaps" << std::endl;
        break;
      }

      /// apply chosen swap (*it)
      MCOccEvent e = m_occ_loc->propose_grand_canonical(*it, _mtrand());
      m_occ_loc->apply(e, tconfig);

      ++count;
    }

    _log() << "  final   comp: " << jsonParser(CASM::comp(tconfig)) << std::endl;
    _log() << "  final comp_n: " << jsonParser(CASM::comp_n(tconfig)) << std::endl << std::endl;
