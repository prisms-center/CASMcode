...
namespace CASM {
  ...
  SimpleOrbit<SymCompareType>::SimpleOrbit(
    ...
    std::set<Element, SimpleOrbitElementCompare<SymCompareType> > tmp {
      SimpleOrbitElementCompare{sym_compare}};
    Adapter<SymOp, typename GroupIterator::value_type> to_symop;
    for(auto it = group_begin; it != group_begin; ++it) {
      tmp.insert(sym_compare.prepare(sym_compare.copy_apply(to_symop(*it), e)));
    }

    std::copy(tmp.begin(), tmp.end(), std::back_inserter(m_elements));
  }

  /// Compare orbits, using SymCompareType::inter_orbit_compare
  template<typename SymCompareType>
  bool SimpleOrbit<SymCompareType>::operator<(const Orbit &B) const {
