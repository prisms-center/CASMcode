...
namespace CASM {
  ...
  namespace Local {
    ...
    static DB::MappedProperties _make_mapped_properties(ConfigMapperResult::Individual const &map) {
      DB::MappedProperties result;
      ...
      for(auto const &prop : map.resolved_struc.properties) {
        if(!map.dof_properties.count(prop.first)) {
          ...
          if(map.first.find("strain") != std::string::npos) {
    ...
    namespace DB {
      ...
      StructureMap<Configuration>::map_result_inserter StructureMap<Configuration>::map(
        ...
        for(auto const &map : map_result.maps) {
          ...
          for(std::string const &propname : prop_vec) {
            res.has_data = false;
