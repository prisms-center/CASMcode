...
template <>
inline std::string singleline_enum_help<DB::SELECTION_TYPE>() {
  return standard_singleline_enum_help<DB::SELECTION_TYPE>(
      to_string(DB::SELECTION_TYPE::MASTER), "filename");
}
...
class SelectionIterator
    : public boost::iterator_facade<SelectionIterator<ObjType, BaseIterator>,
                                    ObjType, std::bidirectional_iterator_tag,
                                    const ObjType &, long> {
 public:
  typedef typename CASM_TMP::ConstSwitch<
      std::is_same<BaseIterator,
                   std::map<std::string, bool>::const_iterator>::value,
      bool>
      bool_type;
 ...
 private:
  ...
  /// Construct iterator
  SelectionIterator(const Selection<ObjType> &_list, BaseIterator _it,
                    bool _selected_only);
...
class Selection {
 public:
  ...
  void set(const DataFormatterDictionary<ObjType> &dict,
           const std::string &criteria, bool selected);

  void read(std::istream &_input);

  void print(const DataFormatterDictionary<ObjType> &_dict, std::ostream &_out,
