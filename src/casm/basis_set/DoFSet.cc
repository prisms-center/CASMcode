...
DoFSet::DoFSet(AnisoValTraits const &_type, std::vector<std::string> components,
               DoFSetInfo _info,
               std::unordered_set<std::string> _excluded_occupants)
    : m_traits(_type),
      m_info(_info),
      m_excluded_occs(std::move(_excluded_occupants)) {
  if (components.empty()) {
  ...
  if (m_info.basis().rows() != traits().dim()) {
    throw std::runtime_error("Cannot construct DoFSet of type " + type_name() +
                             ", number of rows in basis matrix is " +
                             std::to_string(m_info.basis().rows()) +
                             " but must be " + std::to_string(traits().dim()));
  }
  if (m_info.basis().cols() != components.size()) {
    throw std::runtime_error("Cannot construct DoFSet of type " + type_name() +
                             ", number of columns in basis matrix is " +
                             std::to_string(m_info.basis().cols()) +
                             " but number of variable names is " +
...
void DoFSet::transform_basis(
    Eigen::Ref<const Eigen::MatrixXd> const &trans_mat) {
...
bool DoFSet::update_IDs(const std::vector<Index> &before_IDs,
                        const std::vector<Index> &after_IDs) {
  Index ID_ind;
  bool is_updated(false);
  for (Index i = 0; i < m_components.size(); i++) {
    // IMPORTANT: Do before_IDs.find(), NOT m_components().find() (if such a
    // thing existed)
    ...
    if (ID_ind < after_IDs.size() && !m_components[i].is_locked()) {
      ...
      // The new ID only changes the formula if the corresponding coeff is
      // nonzero if(!almost_zero(m_coeffs[i]))
      is_updated = true;
    }
  }
  return is_updated;
...
DoFSet DoFSet::make_default(DoFSet::BasicTraits const &_type) {
  DoFSet result(
      _type, _type.standard_var_names(),
