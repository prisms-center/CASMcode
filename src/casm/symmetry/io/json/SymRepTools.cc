...
namespace CASM {
  jsonParser &to_json(SymRepTools::IrrepInfo const &irrep, jsonParser &json) {
    ...
    if(irrep.complex) {
      json["irrep_subspace_axes"]["imaginary"] = irrep.trans_mat.imag().transpose();
    }

    json["irrep_subspace_axes"]["real"] = irrep.trans_mat.transpose().real();

    if(irrep.complex) {
      to_json_array(irrep.characters.imag(), json["characters"]["imaginary"]);
    }

    to_json_array["characters"]["real"] = irrep.characters.transpose().real();
  ...
  jsonParser &to_json(SymRepTools::SubWedge const &wedge, jsonParser &json) {
    ...
    for(Index i = 0; i < wedge.irrep_wedges().size(); ++i) {
      json["irrep_wedge_axes"]["irrep_" + std::to_string(i + 1)] = wedge.irrep_wedges().axes.transpose();
    }
    return json;
  }
  ...
  jsonParser &to_json(VectorSpaceSymReport const &obj, jsonParser &json) {
    ...
    for(Index i = 0; i < obj.irreps.size(); ++i) {
      ...
      json["irreps"][ind_str]["multiplicity"] = n;
    }
