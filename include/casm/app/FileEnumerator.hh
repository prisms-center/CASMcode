...
class FileEnumerator {
 public:
  ...
  FileEnumerator(const PrimClex &_primclex, bool _all_settings = false,
...
///
FileEnumerator::FileEnumerator(const PrimClex &_primclex, bool _all_settings,
                               bool _relative)
    :
...
/// output path if it exists
template <typename OutputIterator>
OutputIterator FileEnumerator::_if_exists(OutputIterator result,
                                          fs::path path) {
  if (fs::exists(path)) {
...
/// \brief Get all regular files that exist in directory 'location'
template <typename OutputIterator>
OutputIterator FileEnumerator::_all_that_exist(OutputIterator result,
                                               fs::path location) {
...
OutputIterator FileEnumerator::basic_files(OutputIterator result) {
  std::vector<fs::path> v{m_dir.prim(),  // m_dir.PRIM(),
                          m_dir.project_settings(),
                          m_dir.config_list(),
                          m_dir.SCEL(),
                          m_dir.lattice_point_group(),
                          m_dir.factor_group(),
                          m_dir.crystal_point_group()};
...
OutputIterator FileEnumerator::bset_files(OutputIterator result) {
  ...
  for (auto bset : m_all_bset) {
    ...
    result =
        _if_exists(result, m_dir.clexulator_src(m_set.project_name(), bset));
  }
  return result;
}
...
OutputIterator FileEnumerator::eci_files(OutputIterator result) {
  ...
  if (!m_all_settings) {
    result = _if_exists(
        result,
        m_dir.eci(m_set.default_clex().property, m_set.default_clex().calctype,
                  m_set.default_clex().ref, m_set.default_clex().bset,
                  m_set.default_clex().eci));
  } else {
        ...
        for (auto ref : all_ref) {
          for (auto bset : m_all_bset) {
            ...
            for (auto eci : all_eci) {
              result =
                  _if_exists(result, m_dir.eci(clex, calctype, ref, bset, eci));
            }
          }
        }
      }
...
OutputIterator FileEnumerator::calc_settings_files(OutputIterator result) {
  for (auto calctype : m_all_calctype) {
    ...
    for (const auto &scel : m_primclex.db<Supercell>()) {
      result = _all_that_exist(
          result, m_dir.supercell_calc_settings_dir(scel.name(), calctype));
    }

    // configuration level
    for (const auto &config : m_primclex.db<Configuration>()) {
      result = _all_that_exist(result, m_dir.configuration_calc_settings_dir(
                                           config.name(), calctype));
    }
  }
  return result;
}
...
OutputIterator FileEnumerator::calc_status_files(OutputIterator result) {
  for (auto calctype : m_all_calctype) {
    ...
    for (const auto &config : m_primclex.db<Configuration>()) {
      result = _if_exists(result,
                          m_dir.calculated_properties(config.name(), calctype));
    ...
    if (!m_all_settings) {
      ...
      if (fs::is_directory(*it)) {
        ...
        if (dir.substr(0, pattern.size()) == pattern) {
          if (dir.substr(pattern.size(), dir.size()) !=
              m_set.default_clex().calctype) {
            it.no_push();
          }
        }
      }
