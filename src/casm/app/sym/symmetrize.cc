...
namespace symmetrize_impl {
  ...
  ///
  /// Method:
  /// - Read a VASP POSCAR from "input_poscar_location" and, if not primitive, make its primitive
  ///   structure.
  /// - Construct a symmetrized lattice such that the point group of the lattice is equal to that of
  ///   the input structure's lattice when using "enforced_tol" to check for lattice equivalence.
  ///   The symmetrized lattice vectors are obtaining through a process of applying operations in
  ///   the enforced point group, averaging, and then rotating to match the original lattice
  ///   orientation.
  /// - Construct a symmetrized structure with factor group equal to that generated for the input
  ///   structure when using "enforced_tol" to check for structure equivalence. The symmetrized
  ///   structure's basis is constructed by setting basis site coordinates equal to the average
  ///   coordinate positions found after applying each operation in the enforced factor group to
  ///   the original basis sites.
  ...
  /// \param output_poscar_location Location to write symmetrized structure (or primitive of input
  ...
  void symmetrize_v2(
    ...
    Structure input_structure {xtal::BasicStructure::from_poscar_stream(infile, input_tol)};
    ...
    if(!input_structure_is_primitive) {
      ...
      log << "The primitive structure will be generated and symmetrized." << std::endl;
      input_structure = Structure {xtal::make_primitive(input_structure)};
    }

    // original method always printed a result, but only prints the resulting symmetrized structure
    // (called "test_structure" here) if its factor group is valid and has higher symmetry than the
    // original input structure

    // make the factor group to be enforced (this also symmetrizes the lattice), and symmetrize
    SymGroup enforced_factor_group = symmetrize_impl::make_enforced_factor_group(input_structure, enforced_tol);
    Structure test_structure = xtal::symmetrize(input_structure, enforced_factor_group);

    // check factor group of the result and compare to original
    bool test_factor_group_valid = test_structure.factor_group().is_group(enforced_tol);
    ...
    log << "Factor group size of input structure: " << input_factor_group_size << std::endl;
    log << "Factor group size with enforced tolerance: " << test_factor_group_size << std::endl;

    // Set to point to the structure that should be printed
    Structure const *output_structure = nullptr;
    if(test_factor_group_valid && (test_factor_group_size > input_factor_group_size)) {
      log << "Writing symmetrized structure: " << output_poscar_location << std::endl;
      output_structure = &test_structure;
    }
    else {
      log << "Higher symmetry not found the specified tolerance." << std::endl;
      if(input_structure_is_primitive) {
        log << "Writing input structure, unchanged: " << output_poscar_location << std::endl;
      }
      else {
        log << "Writing primitive of input structure: " << output_poscar_location << std::endl;
      }
...
namespace CASM {
  ...
  std::string symmetrize_desc() {
    std::string description =
      ...
      "  The --symmetrize option implements a method to adjust a structure's lattice \n"
      "  and basis to increase factor group symmetry.                                \n\n"

      "  Method: \n"
      "  - Read a VASP POSCAR file from POSCAR_LOCATION and, if not primitive, make  \n"
      ...
      "    structure equivalence. The symmetrized structure's basis is constructed   \n"
      "    by setting basis site coordinates equal to the average coordinate         \n"
      "    positions found after applying each operation in the enforced factor group\n"
  ...
  void symmetrize(APICommandBase const &cmd,
    ...
    combine_json_options(cli_to_combined_keys,
                         cli_options_as_json,
                         json_combined);
    ...
    std::runtime_error error_if_invalid {"Error reading `casm sym --symmetrize` input"};
