...
namespace CASM {
  ...
  FormattedDataFile<DataObject>::FormattedDataFile(FormattedDataFileOptions const &options):
    FormattedDataFile(options.file_path,
                      options.json_output,
                      options.json_arrays,
                      options.compress) {}

  template<typename DataObject>
  FormattedDataFile<DataObject>::FormattedDataFile(fs::path file_path, bool json_output, bool json_arrays, bool compress):
    m_initialized(false), m_json_output(json_output), m_json_arrays(json_arrays) {
    if(!compress) {
  ...
  template<typename DataObject>
  void FormattedDataFile<DataObject>::operator()(DataFormatter<DataObject> const &formatter, DataObject const &object) {
    if(!m_initialized) {
      if(!m_json_output) {
