...
class MCData {
 public:
  ...
  MCData(size_type count)
      : m_observation(Eigen::VectorXd::Zero(count)), m_size(0) {}
 ...
 private:
  /// \brief vector of all observations (includes m_size observations, and the
...
class MCDataEquilibration {
 public:
  ...
  bool is_equilibrated() const { return m_is_equilibrated; }

  size_type equilibration_samples() const { return m_equil_samples; }
...
class MCDataConvergence {
 public:
  ...
  ///
  /// \seealso MCDataConvergence(const Eigen::VectorXd& observations, double
  /// conf)
  ///
  bool is_converged(double prec) const { return m_calculated_prec <= prec; }

  /// \brief <X>
  double mean() const { return m_mean; }

  /// \brief <X*X>
  double squared_norm() const { return m_squared_norm; }
  ...
  /// - Var(<X>), the variance in the mean of X, is calculated as described by
  /// MCDataConvergence
  ///
  double calculated_precision() const { return m_calculated_prec; }

 private:
  /// \brief Try to find rho = pow(2.0, -1.0/i), using min i such that
  /// CoVar[i]/CoVar[0] <= 0.5
  std::tuple<bool, double, double> _calc_rho(
