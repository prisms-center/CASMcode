...
std::string pull_short(const po::option_description &single_boost_option) {
  ...
  possible_short = single_boost_option.canonical_display_name(
      po::command_line_style::allow_dash_for_short);
  if (possible_short.size() > 2 || possible_short[0] != '-' ||
      possible_short[1] == '-') {
    return "-_";
  } else {
...
std::string pull_long(const po::option_description &single_boost_option) {
  return single_boost_option.canonical_display_name(
      po::command_line_style::allow_long);
}
}  // namespace Suboption_impl

Suboption::Suboption(const std::string &init_longname, std::string init_short,
...
void Suboption::_sanity_throw() const {
  if (m_long.size() < 3 || m_short.size() != 2) {
    throw std::runtime_error(
        "--long option must be at least 3 characters long and -s(hort) must be "
        "exactly 2!");
  }
  if (m_long[0] != '-' || m_long[1] != '-' || m_short[0] != '-') {
    throw std::runtime_error(
        "Suboption --long and -s(hort) tags must include leading '-' "
        "characters!");
  }
}

std::string Suboption::long_tag() const { return m_long; }

std::string Suboption::short_tag() const { return m_short; }
...
bool Suboption::matches(const std::string &test_tag) const {
  ...
  throw std::runtime_error(
      "The impossible has occurred. Stepped through an if/else block and hit "
      "neither");
}

ARG_TYPE Suboption::argument_type() const { return m_expected_arg; }

//***************************************************************************************************//

Option::Option(const std::string &init_tag,
               const std::vector<Suboption> &init_allowed_subopts)
    : m_tag(init_tag), m_avail_suboptions(init_allowed_subopts) {}
...
Option::Option(const std::string &init_tag,
  ...
  for (auto it = init_premade_descs.options().begin();
       it != init_premade_descs.options().end(); ++it) {
    m_avail_suboptions.push_back(Suboption(**it));
  }
}

std::string Option::tag() const { return m_tag; }
...
std::vector<std::string> Option::probe_suboptions() const {
  ...
  for (auto it = m_avail_suboptions.begin(); it != m_avail_suboptions.end();
       ++it) {
...
/**
 * Runs through all the available Suboptions trying to match the provided
 * suboption_tag. If the suboption_tag matches, then return the expected type of
 * arguments of that suboption, if the provided tag doesn't match anything, then
 * default to returning VOID, i.e. no completion should be done by bash.
 */
...
/**
 * Runs through all the available Options and generates a list of all their
 * tags. This list is then returned.
 */
...
/**
 * Run through all the Options and try to match the provided option_tag. If one
 * of the Options has that tag, then return all the Suboptions it has as a list
 * of --long formatted strings. If the proved tag doesn't match any options,
 * return an empty list.
 */

std::vector<std::string> Engine::probe_suboptions(
    const std::string &option_tag) const {
  for (auto it = m_avail_options.begin(); it != m_avail_options.end(); ++it) {
...
/**
 * Determine what kind of argument should follow the provided suboption and
 * return either a bash completion suggestion, or a keyword the bash completion
 * script can identify to make the completion itself (e.g. completing paths or
 * available executables)
 */

std::vector<std::string> Engine::probe_arguments(
    const std::string &option_tag, const std::string &suboption_tag) const {
...
/**
 * First run through all the Options and see if you can find one that matches
 * the provided option_tag. If one is found, then run through all the available
 * Suboptions of said tag and see if the provided Suboption_tag can be matched.
 * If it can be matched, return the expected argument type of that Suboption.
 *
 * If either tag can't be found the routine returns VOID, i.e. bash completion
 * shouldn't make any suggestions.
 */

ARG_TYPE Engine::_probe_argument_type(const std::string &option_tag,
                                      const std::string &suboption_tag) const {
  for (auto it = m_avail_options.begin(); it != m_avail_options.end(); ++it) {
    if (it->matches(option_tag)) {
