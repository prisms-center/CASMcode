...
// An "orbit" is the set of equivalent elements that can be generated under
// application of
//   symmetry operations in a group. In CASM, an Orbit instance is constructed
//   from:
//   - a "generating element", one element which will be transformed by
//   operations in a symmetry
//     group generating all the equivalent elements in the orbit
//   - a "generating group", the symmetry group to be applied
//   - a "SymCompare" functor, which customizes the orbit constructing algorithm
//   to be able to
...
//     - SymCompare::spatial_prepare: transform an element to a standard spatial
//     position
//       (i.e. translate into the unit cell) for efficient comparison
//     - SymCompare::representation_prepare: transform an element to a standard
//     representation for
//       efficient comparison (i.e. sort cluster elements)
//     - SymCompare::prepare: prepare an element for comparison via
//     representation_prepare,
//       followed by spatial_prepare
//     - SymCompare::compare: order prepared elements
//     - SymCompare::make_invariants: construct orbit invariants from one
//     element in the orbit,
//       for efficient comparison (i.e. cluster size, site-to-site distances,
//       etc.)
//     - SymCompare::compare_invariants: order orbits by invariants
//     - SymCompare::inter_orbit_compare: order orbits, breaking
//     invariants_compare ties
//     - SymCompare::canonical_transform: access the transform that took an
//     element from
//       the unprepared to prepared state
//     - SymCompare::spatial_transform: access the spatial transform that was
//     used during most
//       recent spatial preparation of an element
//
// CASM provides SymCompare functors for several standard cluster orbit uses
// cases:
// - PrimPeriodicSymCompare: For comparing elements with the translation
// symmetry of the "prim"
//   structure's lattice. This is the appropriate method for generating the
//   cluster expansion basis for crystal properties.
// - ScelPeriodicSymCompare: For comparing elements with the translation
// symmetry of a superlattice
...
// - AperiodicSymCompare (or alias LocalSymCompare): For comparing elements
// without translation
//   symmetry. This is the appropriate method for generating the cluster
//   expansion basis for properties local to a site or cluster of sites in a
//   crystal.
// - WithScelSymCompare: For comparing elements by placing all sites "within" a
// supercell and
//   then comparing. When using this method there are a finite number of orbits
//   that may possibly be generated rather than an infinite number. For example,
//   sites in a pair cluster cannot get separated farther than the half the
//   maximum periodic image distance because they are returned "within" the
//   supercell. This is the appropriate method for generating the unique
//   clusters in a supercell with periodic boundary conditions.
//
...
// - an "equivalence map", a table of SymOp such that equivalence_map()[i][j],
// for all j, acts
...
//       sym_compare.copy_apply(t,
//       sym_compare.representation_prepare(sym_compare.copy_apply(g,
//       prototype)))
//   The first row of the equivalence map is the subgroup of the generating
//   group that leaves the prototype invariant.
...
void expect_for_any_orbit(
  ...
  EXPECT_TRUE(sym_compare.equal(sym_compare.prepare(orbit.prototype()),
                                orbit.prototype()))
  ...
  for (int i = 0; i < orbit.equivalence_map().size(); ++i) {
    ...
    EXPECT_EQ(orbit.equivalence_map()[i].size(),
              orbit.generating_group().size() / orbit.size())
    ...
    for (int j = 0; j < orbit.equivalence_map()[i].size(); ++j) {
      ...
      auto cluster = CASM::sym::copy_apply(orbit.equivalence_map()[i][j],
                                           orbit.prototype(), *shared_prim);

      // for first column only, applying equivalence_map symop to prototype
      // yields element(i) directly
      if (j == 0) {
        EXPECT_TRUE(sym_compare.equal(orbit.element(i), cluster))
            << test_case +
                   ": not equivalent after applying first equivalence map "
                   "symop";
      }

      // for other columns, applying equivalence_map symop may result in a
      // different representation
      auto rep_prepared_element_i =
          sym_compare.representation_prepare(orbit.element(i));
      auto rep_prepared_cluster = sym_compare.representation_prepare(cluster);
      EXPECT_TRUE(
          sym_compare.equal(rep_prepared_element_i, rep_prepared_cluster))
...
TEST(ExampleClusterographyClusterOrbits, OrbitConstructorTernaryFCC) {
...

  {
    ...
    typedef CASM::PrimPeriodicSymCompare<CASM::IntegralCluster>
        sym_compare_type;
    typedef CASM::Orbit<sym_compare_type> orbit_type;
    orbit_type orbit{cluster, shared_prim->factor_group(),
                     sym_compare_type{shared_prim, xtal_tol}};
  ...
  {
    // Construct a pair cluster of two [A, B, C] sites, the conventional cubic
    // cell edges
    //   There are 3 equivalent clusters or this type, corresponding to x, y, z
    //   axes
    CASM::IntegralCluster cluster{*shared_prim};
    cluster.elements().emplace_back(0, 0, 0, 0);
...
TEST(ExampleClusterographyClusterOrbits, OrbitConstructorZrO) {
  ...
  // Construct the prim structure: HCP Zr, with [Va, O] octahedral interstitial
  // sites
  // - Lattice:
  //   - a: [3.234, -1.617, 0.000]
  //   - b: [0.000,  2.801, 0.000]
  ...
  {
    // Construct a pair cluster of two [Va, O] sites, the closest neighbors
    // along the c-axis
...

  {
    // Construct a pair cluster of two [Va, O] sites, the closest neighbors
    // along the a-axis
    //   There are 6 equivalent clusters of this type, 3 each involving basis
    //   site 2 and basis site 3
    CASM::IntegralCluster cluster{*shared_prim};
    cluster.elements().emplace_back(2, 0, 0, 0);
...
TEST(ExampleClusterographyClusterOrbits, OrbitGenerators) {
  // Multiple orbits may be generated using the OrbitGenerators class to
  // identify a set of unique
  ...
  // Construct the OrbitGenerators instance with the generating group and
  // sym_compare
  CASM::OrbitGenerators<orbit_type> generators{shared_prim->factor_group(),
                                               sym_compare};

  // Generate orbits for all single site clusters in the prim unit cell:

  // First, insert all single site clusters into generators, which will keep
  // unique clusters
  ...
  for (int b = 0; b < shared_prim->basis().size(); ++b) {
    ...
    // The `insert` member will use the sym_compare object to transform
    // test_cluster into its
    //   "prepared" form and insert it into a set of unique generating elements
    // There also exists an `insert_canonical` member which can be used if the
    // cluster to be
    //   inserted has already been "prepared" externally.
    generators.insert(test_cluster);
  }

  // there are two unique clusters in the ZrO prim: a [Zr] basis site, and a
  // [Va, O] basis site
...
TEST(ExampleClusterographyClusterOrbits, AsymmetricUnitOrbits) {
  // The asymmetric unit is the set of symmetrically unique sites in the unit
  // cell of the prim.
  //
  // The asymmetric unit is important for structural analysis and for
  // constructing the cluster
  //   expansion basis. Site basis functions must be constructed for each
  //   prototype site in the asymmetric unit, and then each symmetrically
  //   equivalent site must have symmetrically equivalent site basis functions.
  ...
  // A function with signature `bool (xtal::Site)` which returns True for sites
  // that should be
  //   included in the generated orbits, and False for sites that will be
  //   excluded. Options:
  ...
  //     - If dofs is empty (default), include sites that have any continuous
  //     DoF or >1 allowed
  //       occupant DoF
  //     - If dofs is not empty, include sites that have any of the DoF types
  //     included. Use "occ"
  ...
  // Orbit generation can be slow depending on the generating criteria, so orbit
  // generating functions
  //   in CASM typically take a `status` argument of type `std::ostream&` where
  //   status info can be printed while the orbit generation method is running.
  //   Asymmetric units are generally small and quick, so here we pass the
  //   CASM::null_log() and nothing will be printed.
...

  {
    ...
    make_prim_periodic_asymmetric_unit(shared_prim, site_filter, xtal_tol,
                                       inserter, status);
