...
class OccupantFunction : public Function,
                         public DerivedID<OccupantFunction, Function> {
 public:
  OccupantFunction(const DiscreteDoF &init_var,
                   const Eigen::VectorXd &init_eval, int _occ_func_ind,
                   int _basis_ind, SymGroupRepID _sym_rep_ID)
      : m_var(init_var.clone()),
        m_eval_table(init_eval),
        m_sym_rep_ID(_sym_rep_ID),
        m_occ_func_ind(_occ_func_ind),
        m_basis_ind(_basis_ind) {}

  static int sclass_ID();
  int class_ID() const override;

  std::string type_name() const override { return "OccupantFunction"; }

  Index occ_func_ind() const { return m_occ_func_ind; }

  Index basis_ind() const { return m_basis_ind; }

  void set_basis_ind(int new_ind) { m_basis_ind = new_ind; }

  const DiscreteDoF &dof() const { return *m_var; }
  ...
  const Eigen::VectorXd &eval_table() const { return m_eval_table; }
