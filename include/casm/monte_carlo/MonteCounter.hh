...
/// \brief Track the number of passes, steps and samples taken in a Monte Carlo
/// calculation.
///
/// It's a convenience for figuring out which pass you're on by only having to
/// think of incrementing after every step.
///
/// It also has information about the period at which certain routines should be
/// called, such as how often to take a snapshot or write out the state of the
/// run to a file.
///
class MonteCounter {
 public:
  ...
  /// \brief Increments the number of samples taken and resets counter until the
  ...
  /// \brief Returns true if based on period and current number of steps it is
 ...
 private:
  /// One pass is equal to the number of sites with variable degrees of freedom
  /// in the supercell
  size_type m_pass = 0;
  ...
  /// How often to sample data. In terms of m_sample_mode. If == 1 sample every
  /// pass/step.
  int m_sample_period;

  /// Number of steps per pass is equal to the number of sites with variable
  /// degrees of freedom in the supercell
