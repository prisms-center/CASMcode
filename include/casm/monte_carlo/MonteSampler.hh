...
///
/// - Derived classes "know" how to sample a particular property via
/// implementation
///   of 'virtual void sample(const MonteCarlo &mc, const MonteCounter &counter)
///   = 0;'
/// - Optionally may require and check for convergence to some level of
/// precision
...
class MonteSampler {
 public:
  ...
  MonteSampler(const std::string &print_name, double data_confidence,
               size_type data_initsize);

  /// \brief Construct sampler that must converge
  MonteSampler(const std::string &print_name, double data_prec,
               double data_confidence, size_type data_initsize);
  ...
  /// \brief Returns pair(true, equil_steps) if equilibration has occured to
  /// required precision
  ///
  ...
  std::pair<bool, size_type> is_equilibrated() const {
    ...
    return std::make_pair(m_equilibration.is_equilibrated(),
                          m_equilibration.equilibration_samples());
  }

  /// \brief Returns true if convergence criteria must be met for Monte Carlo
  /// calculation to be complete
  bool must_converge() const { return m_must_converge; }

  /// \brief Returns requested precision on the mean
  double requested_precision() const { return m_prec; }

  /// \brief Returns true if convergence criteria have been met for data sampled
  /// in range [equil_samples, end)
  ///
  /// - If must_converge() == false, always returns false
  bool is_converged(size_type equil_samples) const {
    ...
    // if not calculated, or calculated for a different range of data,
    // re-calculate
    if (!m_convergence_uptodate ||
  ...
  /// \brief Returns calculated precision on the mean for data sampled in range
  /// [equil_samples, end)
  ...
  const MCData &data() const { return m_data; }
 ...
 private:
  virtual MonteSampler *_clone() const { return new MonteSampler(*this); }

  void _check_convergence(size_type equil_samples) const {
    m_convergence_start_sample = equil_samples;
    m_convergence = MCDataConvergence(
        m_data.observations().segment(
            equil_samples, m_data.observations().size() - equil_samples),
        m_conf);
    m_convergence_uptodate = true;
  }

  /// \brief If false, data observations are recorded but shouldn't be used to
  /// determine if Monte Carlo run has converged
...
class ScalarMonteSampler : public MonteSampler {
 public:
  /// \brief Construct sampler that does not need to converge
  ScalarMonteSampler(std::string _property_name, std::string print_name,
                     double data_confidence, size_type data_initsize);

  /// \brief Construct sampler that must converge
  ScalarMonteSampler(std::string _property_name, std::string print_name,
                     double data_prec, double data_confidence,
                     size_type data_initsize);
...
class VectorMonteSampler : public MonteSampler {
 public:
  /// \brief Construct sampler that does not need to converge
  VectorMonteSampler(std::string _property_name, size_type _index,
                     std::string print_name, double data_confidence,
                     size_type data_initsize);

  /// \brief Construct sampler that must converge
  VectorMonteSampler(std::string _property_name, size_type _index,
                     std::string print_name, double data_prec,
                     double data_confidence, size_type data_initsize);
 ...
 public:
  ...
  class Formatter {
   public:
    ...
    DataFormatter<Configuration> &get() { return m_formatter; }

    const DataFormatter<Configuration> &get() const { return m_formatter; }

    /// \brief Evaluate datum formatters, if necessary, and return result
    const Eigen::VectorXd &sample(const MonteCarlo &mc,
  ...
  QueryMonteSampler(std::shared_ptr<QueryMonteSampler::Formatter> formatter,
                    size_type _index, std::string print_name,
                    double data_confidence, size_type data_initsize);

  /// \brief Construct sampler that must converge
  QueryMonteSampler(std::shared_ptr<QueryMonteSampler::Formatter> formatter,
                    size_type _index, std::string print_name, double data_prec,
                    double data_confidence, size_type data_initsize);
...
class SiteFracMonteSampler : public MonteSampler {
 public:
  /// \brief Construct sampler that does not need to converge
  SiteFracMonteSampler(size_type _index, size_type _basis_size,
                       std::string print_name, double data_confidence,
                       size_type data_initsize);
...
class AtomFracMonteSampler : public MonteSampler {
 public:
  /// \brief Construct sampler that does not need to converge
  AtomFracMonteSampler(size_type _index, size_type _vacancy_index,
                       std::string print_name, double data_confidence,
                       size_type data_initsize);
