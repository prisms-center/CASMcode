...
const std::string &FilesOption::settings_str() const { return m_settings_str; }
...
void FilesOption::initialize() {
  ...
  m_desc.add_options()(
      ...
      ("calc", po::value<std::vector<std::string> >(&m_calc_vec)->multitoken(),
       "Which calculation files to include. "
       "May be zero (default) or more of: 'settings', 'status', 'all'.")  // TODO:
                                                                          // Include
                                                                          // these
                                                                          // in
                                                                          // ArgHandler

      ("relative,R", "Print relative path from project root directory.");
}
}  // namespace Completer
...
int files_command(const CommandArgs &args) {
  ...
  std::vector<std::string>
      calc;  // These variable names are bad and you should feel bad
  bool gz_flag;
  ...
  try {
    po::store(
        po::parse_command_line(args.argc(), args.argv(), files_opt.desc()),
        vm);  // can throw
    ...
    if (!vm.count("help")) {
      ...
      if (!contains(allowed, settings)) {
        err_log() << "Error in 'casm files'. '" << settings
                  << "' is not a valid option to --settings." << std::endl;
        err_log() << "  Valid options are: 'curr', 'all'" << std::endl;
      ...
      for (auto it = calc.begin(); it != calc.end(); ++it) {
        if (!contains(allowed, *it)) {
          err_log() << "Error in 'casm files'. '" << *it
                    << "' is not a valid option to --calc." << std::endl;
          err_log() << "  Valid options are: 'settings', 'status', 'all'"
                    << std::endl;
          return ERR_INVALID_ARG;
        }
      }
    ...
    if (vm.count("desc")) {
      ...
      log()
          << "DESCRIPTION  \n"
             ...
             "      - Replace 'destination' with '-e ssh "
    ...
    err_log() << files_opt.desc() << std::endl;
  ...
  Log &status_log = (out_path.string() == "STDOUT") ? err_log() : log();
  auto logging = notstd::make_unique<ScopedLogging>(status_log, err_log());
  ...
  std::ostream &output_stream =
      make_ostream_if(vm.count("output"), log(), uniq_fout, out_path, gz_flag);
