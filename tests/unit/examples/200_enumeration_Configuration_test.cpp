...
// -------------
//
// A Configuration represents a particular periodic perturbation of the infinite
// crystal within
...
// continuous)
//   "within" the supercell (i.e. the translationally unique perturbation), plus
//   the values of the global DoF.
// - calculated properties (std::map<std::string, MappedProperties>), the values
// of properties
//   (i.e. energy, relaxation displacements, relaxation strain, etc.) that are
//   dependent on the values of the DoF. Properties are stored in a map with key
//   "calctype" to allow for different values of the properties depending on the
//   calculation method.
//
// Supercell
// ---------
...
//   - SupercellSymInfo, which provides symmetry representations which transform
//   site indices,
//     and values of global and site DoF given the invariance of the supercell
//     lattice vectors.
//   - a SuperNeighborList, which specifies, for each unit cell in the
//   supercell, the indices of the
//     translationally equivalent neighboring sites in a local region surronding
//     the reference cell. This provides fast access of neighboring site DoF
//     values when evaluating cluster expansion basis functions for all
//     Configuration with the same Supercell. SuperNeighborList is described
//     more completely in example 401_clex_neighbor_list_test.cpp.
//
//     SupercellSymInfo has:
//     - a xtal::Superlattice,
//     - the supercell factor group (SymGroup), the subgroup of the prim factor
//     group that
//       leaves the super lattice vectors invariant.
//     - the translation permutations ("translation_permutations",
//     std::vector<Permutation>), which
//       describe the unique ways primitive structure lattice vector
//       translations permute DoF values in a ConfigDoF associated with the
//       Supercell. The number of translation permutations is equal the to
//       volume of the supercell w.r.t. the primitive structure unit cell.
//     - keys (SymGroupRepID) used to access symmetry group representations
//     (SymGroupRep) that
//       encode how discrete DoF values and local and global continuous DoF
//       values transform due to supercell factor group operations.
//     - converters between UnitCellCood site indices and the corresponding
//     linear index into the
//       ConfigDoF occupation value vector, or column index in the continuous
//       site DoF value matrix
//
//
// ConfigDoF
// ---------
...
TEST(ExampleEnumerationSupercell, SupercellConstructor) {
  // Construct the "prim" Structure, representing the infinite crystal and
...
TEST(ExampleEnumerationConfiguration, ConfigurationConstructor) {
  // Construct the (shared) "prim" Structure, representing the infinite crystal
  // and allowed perturbations.
  auto shared_prim = std::make_shared<CASM::Structure const>(test::ZrO_prim());
