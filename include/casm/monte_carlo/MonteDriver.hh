...
/**
 * MonteDriver consists of a specialized MonteCarlo object and a list of
 * conditions to equilibrate at. The first condition in the list requires a
 * starting configuration (read from the setting), while subsequent conditions
 * are calculated using the final state of the previous condition.
 *
 * The different kinds of drive modes the user can specify are:
 * INCREMENTAL:   Given a delta in condition values, increment the conditions by
 * the delta after each point CUSTOM:        Calculate for a list of condition
 * values
 */
...
class MonteDriver {
 public:
  ...
  MonteDriver(const PrimClex &primclex, const SettingsType &settings, Log &_log,
              Log &_err_log);
 ...
 private:
  ...
  /// Return the appropriate std::vector of conditions to visit based from
  /// settings. Use for construction.
  std::vector<CondType> make_conditions_list(const PrimClex &primclex,
  ...
  /// Copy of initial settings given at construction. Will expand to have
  /// MonteCarlo states dumped into it.
  ...
  /// List of specialized conditions to visit in sequential order. Does not
