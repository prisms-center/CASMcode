...
std::string Remove<Supercell>::desc() {
  std::string res =
      "Remove a supercell, including all enumerated configurations and "
      "calculation results: \n\n"
      ...
      "    --selection options.\n"
      "  - Use without additional options to remove all enumerated "
      "configurations\n"
      "    that do not have any associated files or data for each specified "
      "suprecell.\n"
      "    If no data or files, erase supercell.\n"
      "  - Use --data (-d) to remove all configuration data, but not "
      "enumerated \n"
      "    configurations, for each specified supercell. \n"
      "  - Use --force (-f) to remove specified supercells including all data "
      "and \n"
...
struct EraseScelConfigsBase {
  EraseScelConfigsBase(Remove<Supercell> &_remover, std::string _scelname,
                       bool _dry_run)
      : primclex(_remover.primclex()),
        remover(_remover),
        scelname(_scelname),
        dry_run(_dry_run),
        remaining(0) {}
  ...
  void count_remaining() {
    if (!dry_run) {
      remaining +=
          boost::distance(remover.primclex().db<T>().scel_range(scelname));
    } else {
...
/// Helper struct that calls Remove<ConfigType>::erase for all configs in a
/// supercell
struct EraseScelConfigs : public EraseScelConfigsBase {
  EraseScelConfigs(Remove<Supercell> &_remover, std::string _scelname,
                   bool _dry_run)
...
/// Helper struct that calls Remove<ConfigType>::erase_data for all configs in a
/// supercell
struct EraseDataScelConfigs : public EraseScelConfigsBase {
  EraseDataScelConfigs(Remove<Supercell> &_remover, std::string _scelname,
                       bool _dry_run)
      : EraseScelConfigsBase(_remover, _scelname, _dry_run) {}
...
/// Helper struct that calls Remove<ConfigType>::erase_all for all configs in a
/// supercell
struct EraseAllScelConfigs : public EraseScelConfigsBase {
  EraseAllScelConfigs(Remove<Supercell> &_remover, std::string _scelname,
                      bool _dry_run)
      : EraseScelConfigsBase(_remover, _scelname, _dry_run) {}
...
/// supercell
void Remove<Supercell>::erase(const DB::Selection<Supercell> &selection,
                              bool dry_run) {
  ...
  for (; it != end; ++it) {
    ...
    if (!f.remaining) {
      ...
      log() << "skipping " << it.name() << ": has " << f.remaining
...
/// \brief Erase all enumerated configs that have no data; if no data, erase
/// supercell
void Remove<Supercell>::erase_data(const DB::Selection<Supercell> &selection,
                                   bool dry_run) {
  // call Remove<ConfigType>::erase for each ConfigType
  auto it = selection.selected().begin();
  auto end = selection.selected().end();
...
/// \brief Erase all enumerated configs that have no data; if no data, erase
/// supercell
void Remove<Supercell>::erase_all(const DB::Selection<Supercell> &selection,
                                  bool dry_run) {
  // call Remove<ConfigType>::erase for each ConfigType
  auto it = selection.selected().begin();
  auto end = selection.selected().end();
...
int Remove<Supercell>::run(const PrimClex &primclex,
...
const PrimClex &Remove<Supercell>::primclex() const { return m_primclex; }

std::string Remove<Supercell>::report_dir() const { return m_report_dir; }
