...
jsonParser expected_asym_unit(OrbitIterator begin, OrbitIterator end) {
  jsonParser json = jsonParser::object();
  json["orbits"] =
      jsonParser::array(std::distance(begin, end), jsonParser::object());
...
TEST(ClusterographyTest, MultiCaseTest) {
  ...
  for (auto test_it = tests.begin(); test_it != tests.end(); ++test_it) {
    ...
    EXPECT_TRUE(j.contains("cluster_specs"))
        << "test case data 'cluster_specs' is required";
    ...
    // generate a one site orbit, prim periodic
    {
      ...
      PrimPeriodicOrbit<IntegralCluster> orbit(
          clust, prim.factor_group(),
          PrimPeriodicSymCompare<IntegralCluster>(shared_prim,
                                                  crystallography_tol));
      EXPECT_TRUE(orbit.prototype().size() == 1) << "orbit generated";

      check("first_prim_periodic_orbit", j,
            expected_first_prim_periodic_orbit(orbit), test_cases_path, quiet);
    }

    // generate asym unit
    {
      std::vector<PrimPeriodicOrbit<IntegralCluster>> asym_unit;
      make_prim_periodic_asymmetric_unit(shared_prim, alloy_sites_filter,
                                         crystallography_tol,
                                         std::back_inserter(asym_unit), log);

      // run checks:
      check("asym_unit", j,
            expected_asym_unit(asym_unit.begin(), asym_unit.end()),
            test_cases_path, quiet);
    }
    ...
    // generate cluster orbits
    {
      ...
      // run checks:
      check("Nclusters", j, expected_Nclusters(orbits.begin(), orbits.end()),
            test_cases_path, quiet);
    }
