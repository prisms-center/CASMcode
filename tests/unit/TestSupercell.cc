...
namespace test {
  ...
  TestSupercell::TestSupercell(const PrimClex &primclex, const Eigen::Matrix3i &T) :
    ...
    scel_sym_compare(scel.primclex().shared_prim(), scel.prim_grid(), scel.crystallography_tol()),
    within_scel_sym_compare(scel.primclex().shared_prim(), scel.prim_grid(), scel.crystallography_tol()) {}
