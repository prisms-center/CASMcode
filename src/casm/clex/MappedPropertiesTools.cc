...
namespace CASM {
  MappedProperties copy_apply(PermuteIterator const &op, MappedProperties const &props) {
    ...
    for(auto it = props.global.begin(); it != props.global.end(); ++it) {
      ...
      result.global[it->first] = traits.symop_to_matrix(symop.matrix(), symop.tau(), symop.time_reversal()) * it->second;
    }

    Permutation tperm(op.combined_permute());
    for(auto it = props.site.begin(); it != props.site.end(); ++it) {
      AnisoValTraits traits(AnisoValTraits::name_suffix(it->first));
      Eigen::MatrixXd new_matrix = traits.symop_to_matrix(symop.matrix(), symop.tau(), symop.time_reversal()) * it->second;
      auto it2 = result.site.emplace(std::make_pair(it->first, Eigen::MatrixXd(new_matrix.rows(), new_matrix.cols()))).first;
