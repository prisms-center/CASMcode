...
QueryHandler<DataObject>::QueryHandler(const ProjectSettings &set)
  ...
  *m_dict = make_dictionary<DataObject>();
  ...
  // add plugins
  load_query_plugins(set, std::inserter(*m_dict, m_dict->end()),
...
template <typename DataObject>
const DataFormatterDictionary<DataObject> &QueryHandler<DataObject>::dict()
    const {
  return *m_dict;
}
...
template <typename DataObject>
void QueryHandler<DataObject>::set_selected(
    const DB::Selected<DataObject> &selection) {
  if (m_dict->find("selected") != m_dict->end()) {
    m_dict->erase("selected");
  }
  m_dict->insert(datum_formatter_alias(
      "selected", selection,
...
template <typename DataObject>
void QueryHandler<DataObject>::set_selected(
    const DB::Selection<DataObject> &selection) {
...
///   to file
template <typename DataObject>
void QueryHandler<DataObject>::add_alias(const std::string &alias_name,
                                         const std::string &alias_command) {
  auto new_formatter =
      datum_formatter_alias<DataObject>(alias_name, alias_command, *m_dict);
  ...
  // else do not add, throw error
    ...
    std::stringstream ss;
    ss << "Error: Attempted to over-write standard CASM query name with user "
          "alias.\n";
...
std::pair<DataFormatterDictInserter, RuntimeLibInserter> load_query_plugins(
  ...
  if (fs::is_directory(dir.query_plugins<DataObject>())) {
    ...
    for (auto &entry : boost::make_iterator_range(
             fs::directory_iterator(dir.query_plugins<DataObject>()), {})) {
      ...
      if (fs::is_regular_file(p) &&
        ...
        auto lib_ptr = log_make_shared_runtime_lib(
            ...
            set.compile_options() + " " +
                include_path(dir.query_plugins<DataObject>()),
