...
namespace CASM {
  ...
  // final evaluated functions. ClexParamPack allows the underlying Scalar value type to be obscured,
  // in order to implement special functionality, such as automatic differentiation
  // Clexulators may implement their own ClexParamPack, or utilitize a pre-defined ClexParamPack specialization
  class ClexParamPack;

  // ClexParamKey is used externally to Clexulator and ClexParamPack to access stored and derived values from
  ...
  namespace ClexParamPack_impl {
    ...
    /// Hides implementation-specific access attributes through an abstract interface.
    /// A ClexParamKey object manages access for a particular type of data valuess.
    /// Additional specification of referenced data can be provided via linear or pair indices as necessary
    class BaseKey {
    public:

      /// \brief Must be constructed with at least a name
      /// \param _name { unique name that describes managed data }
      /// \param _standalone { true if key refers directly to managed data, false if refers to value derived from managed data }
      /// \param _offset { list of constant offsets for each supplemental linear identifier index }
      /// \param _stride { list of strides to convert supplemental pair identifier indices to linear indices}
      BaseKey(std::string const &_name,
              bool _standalone,
      ...
      /// \brief Name of this Key, should be descriptive of managed data. Ex.: "disp_var" of "magspin_site_basis"
      std::string const &name() const {
        return m_name;
      }

      /// \brief Specify supplemental identifiers, either via linear indices or pair indices.
      /// Internally, identifiers are always passed as linear indices. The number and specification of identifiers
      /// depends on derived ClexParamKey implementation. Linear identifiers should be passed as an integral value (Index, int, etc)
      /// pair index identifiers must be passed as std::pair<Index,Index>
      ...
      /// \brief Returns true if key refers directly to managed data, false if it refers to values derived from managed data
  ...
  /// \brief ParamPackMixIn is interface class to control ClexParamPack portion of Clexulator printing
  /// Used primarily by ClexBasisWriter
  ...
  public:
    ...
    /// \brief returns string with c++ definitions to be written before Clexulator.
  ...
  public:
    ...
    ///  Eigen::MatrixXd my_params = pp_ptr->read(key(i, std::pair<Index,Index>(j,k)));
    /// \endcode
    template<typename ...Args>
  ...
  /// Parameters are assumed to be stored as, or at least sliceable into, 2D or 1D arrays
  /// in order to standardize read/write interface via Eigen types
  /// Arrays are layed out along two dimensions to form (m x N) or (m x 1) matrices
  /// where 'N' is the number of sites in the neighborhood of a unit cell,
  /// and 'm' is the intrinsic dimension of a particular parameter.
  /// Macroscopic or homogeneous properties of the crystal, such as evaluated crystal basis functions
  /// or homogeneous strain, which are not associated with individual sites, are
  /// represented as a (m x 1) array. The length of the 'm' dimension of a parameter array
  /// is referred do as 'dim' and the length of the N' dimension is referred to as 'size'
  class ClexParamPack {
  public:
    ...
    /// \brief Obtain registry of all keys for data blocks managed by this ClexParamPack
    ...
    /// \brief 'N' dimension of parameter array (either '1', or size of unit cell neighborhood)
    /// returns number of columns in parameter array
    ...
    /// - "read" (User fixes values via ClexParamPack::write and Clexulator reads provided values)
    /// - "custom" (Clexulator utilizes user-provided function handles to evaluate parameters)
    virtual std::string eval_mode(ClexParamKey const &_key) const = 0;

    /// \brief Returns const reference to parameter array for parameter specified by \param _key
    /// NOTE: Take care in constructing pointers or references from the return value.
    /// If _key accepts additional index identifiers, (because the parameter has more than 2 dimensions)
    /// ClexParamPack::read() may be implemented using  an internally cached matrix holding the
    /// current data slice. As such, subsequent requests to ClexParamPack::read() for a different slice
    /// may alter the internally cached values while returning a reference for the same block of memory
    virtual Eigen::MatrixXd const &read(ClexParamKey  const &_key) const = 0;

    /// \brief Returns const reference to element of 1D parameter array for parameter specified by \param _key
    virtual double const &read(ClexParamKey  const &_key, size_type _i) const = 0;

    /// \brief Returns const reference to element of 2D parameter array for parameter specified by \param _key
    /// \param _i {row index} and \param _j {column index}
    ...
    /// \brief Write element to 1D parameter array for parameter specified by \param _key
    virtual void write(ClexParamKey const &_key, size_type _i, double val) = 0;

    /// \brief Write element to 2D parameter array for parameter specified by \param _key
