...
void prim2_read_test(BasicStructure &struc) {
  ...
  /*         } */
  /*         /1* EXPECT_EQ(struc.basis()[i].occupant_dof().value(), check_value[i]); *1/ */
  /*     } */
...
protected:
  void SetUp() override {
    ...
    xtal::BasicStructure hcp_stack_structure = this->manual_construction();
  ...
  xtal::BasicStructure manual_construction() const {
    Eigen::Matrix3d hcp_3stack_row_matrix;
    hcp_3stack_row_matrix << 1.59609453, 2.76451683, 0.00000000, -1.59609453, 2.76451683, 0.00000000, 0.00000000, 0.00000000,
                          15.55205871;

    xtal::Lattice hcp_3stack_lat(hcp_3stack_row_matrix.transpose());
    xtal::BasicStructure hcp_3stack_struc(hcp_3stack_lat);

    std::vector<Eigen::Vector3d> frac_coords;
    frac_coords.emplace_back(0.66666670,  0.66666670, 0.25000000);
    frac_coords.emplace_back(0.66666670,  0.66666670, 0.58333330);
    frac_coords.emplace_back(0.66666670,  0.66666670, 0.91666670);
    frac_coords.emplace_back(0.33333330,  0.33333330, 0.08333330);
    frac_coords.emplace_back(0.33333330,  0.33333330, 0.41666670);
    frac_coords.emplace_back(0.33333330,  0.33333330, 0.75000000);

    std::vector<xtal::Site> basis;
    for(const auto &frac : frac_coords) {
      xtal::Coordinate as_coord(frac, hcp_3stack_struc.lattice(), FRAC);
      basis.emplace_back(as_coord, "Mg");
    }

    hcp_3stack_struc.set_basis(basis);
    return hcp_3stack_struc;
  }
...
TEST_F(HexagonalSuperStructureTest, MakePrimitive) {
  ...
  std::cout << hcp_3stack_ptr->basis().size() << std::endl;
  std::cout << hcp_primitive_ptr->basis().size() << std::endl;
