...
///
/// Note:
/// - The Supercell default Configuration is given by
...
ConfigEnumInput::ConfigEnumInput(
    Configuration const &_configuration,
    std::set<Index> const &_site_index_selection = {})
    : m_site_index_selection(_site_index_selection),
      m_configuration(_configuration) {}

/// Construct using a Supercell's default Configuration and specified sites
/// selected
///
/// Note:
/// - The Supercell default Configuration is given by
...
void ConfigEnumInput::clear_sites() { m_site_index_selection.clear(); }
...
void ConfigEnumInput::select_site(UnitCellCoord const &site_uccord) {
  auto const &converter =
      configuration().supercell().sym_info().unitcellcoord_index_converter();
...
/// Returns the subgroup of the configuration factor group that not cause any
...
std::vector<PermuteIterator> make_invariant_subgroup(
  ...
  std::vector<PermuteIterator> factor_group =
      config_enum_input.configuration().factor_group();
  return make_invariant_subgroup(config_enum_input, factor_group.begin(),
                                 factor_group.end());
}
