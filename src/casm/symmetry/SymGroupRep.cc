...
namespace CASM {
  ...
  ReturnArray<Array< Eigen::VectorXd> > SymGroupRep::calc_special_total_directions(const SymGroup &head_group, std::vector<Index> &subspaces)const {
    Array<Index> irrep_IDs = get_irrep_IDs(head_group);
    subspaces.clear();
    Array<Array<Eigen::VectorXd> > sdirs;
  ...
  //*******************************************************************************************
  Eigen::MatrixXd SymGroupRep::irreducible_wedge(const SymGroup &head_group, std::vector<Index> &multiplicities, std::vector<Index> &subspaces)const {
    Index dim=get_MatrixXd(head_group[0])->cols();
    Array<Array< Eigen::VectorXd> > sdirs=calc_special_total_directions(head_group,subspaces);
    Eigen::MatrixXd wedge(Eigen::MatrixXd::Zero(dim,dim));
    ...
    for(Index i=0; i<sdirs.size(); i++){
      ...
      for(Index j=1; j<sdirs[i].size(); j++){
        tproj=(wedge.transpose()*sdirs[i][j]).sum();
        if(tproj>best_proj){
          best_proj=tproj;
          j_best=j;
        }
      }
      multiplicities.push_back(sdirs[i].size());
      wedge.col(i)=sdirs[i][j_best];
    }
    return wedge;
  }
  //*******************************************************************************************
  ...
  ReturnArray<Array< Eigen::MatrixXd> > SymGroupRep::calc_special_subspaces(const SymGroup &head_group)const {
    ...
    const Array<Array<Array<Index> > > &sg_op_inds(head_group.get_large_subgroups());
  ...
  ReturnArray<Index> SymGroupRep::num_each_irrep(const SymGroup &head_group, bool verbose) const {
    ...
    const Array<Array<std::complex<double> > > &char_table(head_group.get_character_table());
    ...
    for(Index i = 0; i < conj_class.size(); i++) {
      ...
      repchar[i] = at(rep_index)->get_character();
  ...
  bool SymGroupRep::is_irrep() const {
    ...
    for(Index i = 0; i < size(); i++) {
      tvalue += (at(i)->get_character()) * (at(i)->get_character());
    }
  ...
  bool SymGroupRep::is_irrep(const SymGroup &head_group) const {
    ...
    for(Index i = 0; i < head_group.size(); i++) {
      tvalue += (at(head_group[i].index())->get_character()) * (at(head_group[i].index())->get_character());
    }
  ...
  Eigen::MatrixXd SymGroupRep::get_irrep_trans_mat(const SymGroup &head_group, std::vector< std::vector<Index> > &subspaces) const {
    ...
    const Array<bool > &complex_irrep(head_group.get_complex_irrep_list());
    //const Array<Array<std::complex<double> > > &char_table(master_group().get_character_table());
    const Array<Array<std::complex<double> > > &char_table(head_group.get_character_table());
