...
namespace {
  ...
  Configuration _make_super_configuration_L12(long N, Configuration const &_configuration_L12);

  /// Check how many clusters have a matching number of each type of occupant on the selected sites
  Index count_selected(Configuration const &configuration,
...
protected:
  ...
  Configuration superconfiguration_L12;    // superconfiguration of configuration_L12
  ...
  ClusterSitesSelectorTest():
    ...
    superconfiguration_L12(_make_super_configuration_L12(5, configuration_L12)) {
...
TEST_F(ClusterSitesSelectorTest, Test1) {
  ...
  /// Orbits based on primitive configuration factor group, under periodic boundary conditions
  auto unitconfig_fg = configuration_L12.factor_group();
  auto unitsupercell_sym_info = configuration_L12.supercell().sym_info();
  std::vector<Permutation> unit_config_inverse_permutations = make_inverse_permutations(
                                                                unitconfig_fg.begin(),
                                                                unitconfig_fg.end());

  std::set<std::set<Index>, ClusterSiteIndicesCompare> orbit_generators_unitconfig_pbc =
    make_orbit_generators_under_periodic_boundary_conditions(
      unitsupercell_sym_info,
      unit_config_inverse_permutations.begin(),
  ...
  EXPECT_EQ(count_selected(configuration_L12, orbit_generators_unitconfig_pbc, {0, 0, 0}), 1);
  EXPECT_EQ(count_selected(configuration_L12, orbit_generators_unitconfig_pbc, {1, 0, 0}), 1);
  EXPECT_EQ(count_selected(configuration_L12, orbit_generators_unitconfig_pbc, {0, 1, 0}), 1);
  EXPECT_EQ(count_selected(configuration_L12, orbit_generators_unitconfig_pbc, {1, 1, 0}), 1);
  EXPECT_EQ(count_selected(configuration_L12, orbit_generators_unitconfig_pbc, {2, 0, 0}), 1);
  EXPECT_EQ(count_selected(configuration_L12, orbit_generators_unitconfig_pbc, {2, 1, 0}), 1);
  EXPECT_EQ(count_selected(configuration_L12, orbit_generators_unitconfig_pbc, {3, 0, 0}), 1);
  ...
  std::set<std::set<Index>, ClusterSiteIndicesCompare> orbit_generators_superconfig_pbc =
    make_orbit_generators_under_periodic_boundary_conditions(
  ...
  // Uncomment to print superconfiguration L12 orbit generators (cluster site indices as std::set<Index>)
  {
    Index i = 0;
  ...
  // when the supercell is large enough, pbc do not cause aliasing, and the number of generators
  // is unchanged
  EXPECT_EQ(orbit_generators_superconfig_pbc.size(), orbit_generators_prim_config.size());
}
...
namespace {
  ...
  /// NxNxN super configuration of configuration_L12
  Configuration _make_super_configuration_L12(long N, Configuration const &_configuration_L12) {
    auto conventional_fcc_supercell = std::make_shared<Supercell>(
  ...
  /// - Count how many generators have 1 site occupied by an A atom, 2 sites occupied by a B atom,
