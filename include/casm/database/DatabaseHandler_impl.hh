...
/// Insert a Database
template <typename T>
void DatabaseHandler::insert(std::string db_name,
                             std::unique_ptr<DatabaseBase> &&value) {
  m_db.emplace(std::make_pair(traits<T>::name, db_name), std::move(value));
}

/// Insert a PropertiesDatabase
template <typename T>
void DatabaseHandler::insert_props(
    std::string db_name, std::string calc_type,
    std::unique_ptr<PropertiesDatabase> &&value) {
  m_db_props.emplace(PropDBKey(traits<T>::name, db_name, calc_type),
                     std::move(value));
}
...
/// Access default PropertiesDatabase
template <typename T>
const PropertiesDatabase &DatabaseHandler::db_props(
...
template <typename T>
const PropertiesDatabase &DatabaseHandler::db_props(
    std::string db_name, std::string calc_type) const {
  auto res = _find_props<T>(db_name, calc_type);
  return static_cast<PropertiesDatabase &>(res->second->open());
}
...
template <typename T>
DatabaseHandler::map_type::iterator DatabaseHandler::_find(
...
template <typename T>
DatabaseHandler::props_map_type::iterator DatabaseHandler::_find_props(
    std::string db_name, std::string calc_type) const {
...
template <typename T>
void DatabaseHandler::_no_props_database_error(std::string db_name,
                                               std::string calc_type) const {
  std::stringstream ss;
  ss << "Value: " << traits<T>::name;
  ss << "  Database: " << db_name;
  ss << "  CalcType: " << calc_type;
  throw std::runtime_error("Requested properties database not found: " +
                           ss.str());
}
