...
//****************************************************
jsonParser &to_json(AtomPosition const &apos, jsonParser &json,
...
AtomPosition jsonConstructor<AtomPosition>::from_json(
  ...
  if (json.is_obj()) {
    ...
    if (json.contains("attributes")) {
      ...
      for (; it != end_it; ++it) {
        auto result_pair = attr_map.emplace(
            it.name(), _modules.aniso_val_dict().lookup(it.name()));
...
jsonParser &to_json(Molecule const &mol, jsonParser &json,
                    Eigen::Ref<const Eigen::Matrix3d> const &c2f_mat) {
  json.put_obj();
...
Molecule jsonConstructor<Molecule>::from_json(
...
Site jsonConstructor<Site>::from_json(
    const jsonParser &json, Lattice const &_home, COORD_TYPE coordtype,
...
void from_json(Site &site, const jsonParser &json, Lattice const &_home,
  ...
  if (json.contains("dofs")) {
    ...
    for (; it != end_it; ++it) {
      ...
      try {
         ...
         * it->get<xtal::DoFSet>(_modules.aniso_val_dict().lookup(it.name()))));
         */
        _dof_map.emplace(std::make_pair(
            it.name(), it->get<xtal::SiteDoFSet>(
                           _modules.aniso_val_dict().lookup(it.name()))));
        /* _dof_map.emplace(std::make_pair(it.name(),
  ...
  if (t_occ.empty()) t_occ.push_back(Molecule::make_unknown());
...
BasicStructure read_prim(const jsonParser &json, double xtal_tol,
                         HamiltonianModules const *_modules) {
  HamiltonianModules default_module;
  if (_modules == nullptr) _modules = &default_module;
...
ChemicalReference read_chemical_reference(fs::path filename,
  ...
  try {
    ...
    std::cerr << "Error reading chemical reference states from " << filename
              << std::endl;
    /// re-throw exceptions
    throw;
  }
...
ChemicalReference read_chemical_reference(jsonParser const &json,
  ...
  return jsonConstructor<ChemicalReference>::from_json(
      json["chemical_reference"], prim, tol);
}
...
CompositionAxes::CompositionAxes(fs::path _filename) { read(_filename); }

/// \brief Read CompositionAxes from file
///
/// - Stores err_code and err_message for non-fatal errors
/// - Catches and throws any exceptions
CompositionAxes::CompositionAxes(const jsonParser &json) { read(json); }
  ...
  try {
    *this = CompositionAxes();
    ...
    if (json.contains("standard_axes")) {
      ...
      for (auto const &el : standard) enumerated.insert(el.first);
...
void CompositionAxes::write(jsonParser &json) const {
  ...
  if (enumerated.size()) json["enumerated"] = enumerated;
}
...
#define ORBIT_CONTAINER_INST(ITERATOR, INSERTER, ORBIT)                        \
  ...
  template void print_clust<ITERATOR>(ITERATOR begin, ITERATOR end, Log & out, \
