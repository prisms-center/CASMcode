...
/// \brief Class for checking equivalence of two DoFSet objects, with respect to
...
class DoFIsEquivalent {
 public:
  DoFIsEquivalent(DoFSet const &_dof, double tol = TOL)
      : m_dof(_dof), m_tol(tol) {}

  /// returns true if m_dof and _other have matching labels, and m_dof.basis() =
  /// _other.basis()*U
  ...
  /// return transformation matrix U calculated during last successful
  /// comparison
 ...
 private:
  ...
  /// unique solution U stores solution in m_U
  bool _vector_equiv(
...
/// - if compare_occupant is set to TRUE, the current occupant of dofA is
/// equivalent to the current occupant of dofB
///
/// \ingroup IsEquivalent
///
