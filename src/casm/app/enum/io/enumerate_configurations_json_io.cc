...
jsonParser combine_configuration_enum_json_options(
  ...
  ///   "scelnames": <array of string, list of supercell names, context
  ///   dependent usage>, "confignames": <array of string, list of config names,
  ///   context dependent usage>, "all": <bool, use all existing supercells for
  ///   enumeration, ignore other inputs>, "filter": <array of string, filter
  ///   query/select expression, save configs if evaluates true>, "verbosity":
  ///   <string, to be read by Log::verbosity_level>, "dry_run": <bool,
  ///   print/return method results but do not save results>
...
void parse(InputParser<ConfigEnumOptions> &parser, std::string method_name,
  ...
  if (!options.filter_expression.empty()) {
    options.filter =
        make_data_formatter_filter(options.filter_expression, dict);
  }

  parser.optional_else(options.output_configurations, "output_configurations",
                       false);
  ...
  if (options.output_configurations) {
    ...
    parser.optional_else(output_filtered_configurations,
                         base / "output_filtered_configurations", false);
