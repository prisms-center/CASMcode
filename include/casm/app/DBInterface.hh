...
template <typename CommandType,
          template <typename> class ImplType = CommandType::template Impl>
...
class InterfaceData {
 public:
  ...
  const DataFormatterDictionary<DataObject> &dict() const { return *m_dict; }

  std::string sel_str() const { return m_ss.str(); }

  double sel_size() const { return m_sel.size(); }

  Selection<DataObject> &sel(Index i = 0) { return *m_sel[i]; }

  const Selection<DataObject> &sel(Index i = 0) const { return *m_sel[i]; }
