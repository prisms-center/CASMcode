...
std::string ProjectSettings::project_name() const { return m_project_name; }

bool ProjectSettings::has_dir() const { return bool{m_dir}; }

DirectoryStructure const &ProjectSettings::dir() const {
  if (!m_dir) {
    throw std::runtime_error(
        "Error accessing DirectoryStructure from ProjectSettings: Does not "
        "exist.");
  }
  return *m_dir;
}
...
fs::path ProjectSettings::root_dir() const { return dir().root_dir(); }

ProjectSettings::required_properties_map_type const &
ProjectSettings::required_properties() const {
...
std::vector<std::string> const &ProjectSettings::required_properties(
...
std::map<std::string, ClexDescription> const &
ProjectSettings::cluster_expansions() const {
  return m_clex;
}
...
std::pair<std::string, std::string> ProjectSettings::cxxflags() const {
  return m_cxxflags.first.empty() ? RuntimeLibrary::default_cxxflags()
                                  : m_cxxflags;
}

std::pair<std::string, std::string> ProjectSettings::soflags() const {
  return m_soflags.first.empty() ? RuntimeLibrary::default_soflags()
                                 : m_soflags;
}

std::pair<fs::path, std::string> ProjectSettings::casm_includedir() const {
  return m_casm_includedir.first.empty()
             ? RuntimeLibrary::default_casm_includedir()
             : m_casm_includedir;
}

std::pair<fs::path, std::string> ProjectSettings::casm_libdir() const {
  return m_casm_libdir.first.empty() ? RuntimeLibrary::default_casm_libdir()
                                     : m_casm_libdir;
}

std::pair<fs::path, std::string> ProjectSettings::boost_includedir() const {
  return m_boost_includedir.first.empty()
             ? RuntimeLibrary::default_boost_includedir()
             : m_boost_includedir;
}

std::pair<fs::path, std::string> ProjectSettings::boost_libdir() const {
  return m_boost_libdir.first.empty() ? RuntimeLibrary::default_boost_libdir()
                                      : m_boost_libdir;
}
...
std::string ProjectSettings::so_options() const {
  ...
  return cxx().first + " " + soflags().first + " " +
         link_path(boost_libdir().first) + " " + link_path(casm_libdir().first);
}

std::string ProjectSettings::view_command() const { return m_view_command; }
...
double ProjectSettings::lin_alg_tol() const { return m_lin_alg_tol; }
...
void ProjectSettings::set_default_database_name(
    std::string _default_database_name) {
...
ProjectSettings::query_alias_map_type const &ProjectSettings::query_alias()
    const {
  return m_query_alias;
}

void ProjectSettings::set_query_alias(
    query_alias_map_type const &_query_alias) {
  m_query_handler.clear();
  m_query_alias = _query_alias;
}

void ProjectSettings::set_query_alias(std::string type_name,
...
QueryHandler<DataObject> &ProjectSettings::query_handler() {
  ...
  res = m_query_handler
            .insert(std::make_pair(traits<DataObject>::name,
                                   notstd::cloneable_ptr<notstd::Cloneable>(
                                       new QueryHandler<DataObject>(*this))))
            .first;

  QueryHandler<DataObject> &q =
      static_cast<QueryHandler<DataObject> &>(*res->second);
...
void print_summary(ProjectSettings const &set, Log &log) {
  ...
  for (int i = 0; i < all_calctype.size(); i++) {
    ...
    for (int j = 0; j < all_ref.size(); j++) {
      ...
      if (all_calctype[i] == default_desc.calctype &&
          all_ref[j] == default_desc.ref) {
        log << "*";
      }
      log << "\n";
    }
  ...
  // print training data (calctype) & references
  std::vector<std::string> all_eci =
      set.dir().all_eci(default_desc.property, default_desc.calctype,
                        default_desc.ref, default_desc.bset);
  ...
  log << "*: indicates the default settings used by CASM whenever particular \n"
         "settings are not explicitly specified (i.e. the basis set to "
         "evaluate \n"
...
jsonParser &to_json(const ProjectSettings &set, jsonParser &json) {
  ...
  // only write compiler & linker settings if stored in project settings, not if
  // obtained from default values or environment variables

  auto _write_str_if =
      [&](std::string name,
          std::pair<std::string, std::string> value_and_source) {
        if (value_and_source.second == "project_settings") {
          json[name] = value_and_source.first;
        };
      };

  auto _write_path_if = [&](std::string name,
                            std::pair<fs::path, std::string> value_and_source) {
    if (value_and_source.second == "project_settings") {
...
ProjectSettings jsonConstructor<ProjectSettings>::from_json(
  ...
  try {
    ...
    if (json.contains("required_properties") &&
        json["required_properties"].size()) {
    ...
    if (json.contains("cluster_expansions") &&
        json["cluster_expansions"].size()) {
      ...
      if (json.contains("default_clex")) {
        ...
        if (settings.has_clex("formation_energy")) {
          ...
          settings.set_default_clex_name(
              settings.cluster_expansions().begin()->first);
        }
      }
    }
    ...
    if (json.contains("nlist_weight_matrix")) {
      settings.set_nlist_weight_matrix(
          json["nlist_weight_matrix"].get<Eigen::Matrix3l>());
    }
    if (json.contains("nlist_sublat_indices")) {
      settings.set_nlist_sublat_indices(
          json["nlist_sublat_indices"].get<std::set<int>>());
    }
...
ProjectSettings read_project_settings(fs::path project_settings_path) {
  try {
    ...
    err_log() << "Error reading ProjectSettings from: '"
              << project_settings_path << "'" << std::endl;
    throw e;
  }
}
