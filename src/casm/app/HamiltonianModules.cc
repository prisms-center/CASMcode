...
make_parsing_dictionary<SymRepBuilderInterface>() {
  ...
  dict.insert(SymRepBuilder::Identity(), SymRepBuilder::Cartesian(),
              SymRepBuilder::TimeReversal(), SymRepBuilder::AngularMomentum(),
              SymRepBuilder::Rank2Tensor(), SymRepBuilder::dOrbitalOccupation(),
...
HamiltonianModules::HamiltonianModules(ProjectSettings const *_set)
    : m_dof_dict(make_parsing_dictionary<DoFDictionary::value_type>().clone()),
      m_aniso_val_dict(
          make_parsing_dictionary<AnisoValDictionary::value_type>().clone()),
      m_symrep_builder_dict(
          make_parsing_dictionary<SymRepBuilderDictionary::value_type>()
              .clone()) {
  // std::cout << "Inside HamiltonianModules Constructor\n";
  // add DoF plugins
  if (_set) {
    load_dof_plugins(*_set, std::inserter(*m_dof_dict, m_dof_dict->end()),
...
HamiltonianModules::AnisoValDictionary const &
HamiltonianModules::aniso_val_dict() const {
  return *m_aniso_val_dict;
}

HamiltonianModules::SymRepBuilderDictionary &
HamiltonianModules::symrep_builder_dict() {
  return *m_symrep_builder_dict;
}

HamiltonianModules::SymRepBuilderDictionary const &
HamiltonianModules::symrep_builder_dict() const {
  return *m_symrep_builder_dict;
}
