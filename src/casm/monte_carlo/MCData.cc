...
///
///  We consider the calculation equilibrated at start1 if mean of elements in
///  first and second partition are approximately equal to the desired precsion:
///  (std::abs(mean1 - mean2) < prec).
///
///  Additionally, we increment start1 as much as needed to ensure that the
///  equilibriation stage has observations on either side of the total mean.
///
