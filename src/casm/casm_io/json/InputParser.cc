...
namespace CASM {
  ...
  KwargsParser::KwargsParser(jsonParser const &_input, fs::path _path, bool _required) :
    input(_input),
    path(_path),
  ...
  void KwargsParser::insert(fs::path path, const std::shared_ptr<KwargsParser> &subparser) {
    m_subparsers.emplace(path, subparser);
  }

  void KwargsParser::insert_error(fs::path option, std::string message) {
    auto subparser = std::make_shared<KwargsParser>(this->input, this->relpath(option), false);
