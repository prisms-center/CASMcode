...
///   where ConfigType = Configuration, const Configuration, Transition, or
///   const Transition
...
class FilteredConfigIterator
 ...
 public:
  ...
  FilteredConfigIterator(const IteratorType &_end)
      : m_it(_end),
        m_end(_end),
        m_filter(ConstantValueFormatter<bool, DataObject>("invalid", false)) {}
...
FilteredConfigIterator<IteratorType> filter_begin(
    const IteratorType &it, const IteratorType &it_end,
    const std::vector<std::string> &filter_expr,
    const DataFormatterDictionary<DataObject> &_dict) {
...
FilteredConfigIterator<IteratorType> filter_end(const IteratorType &it_end) {
  // Maybe this constructor should be private and make filter_end() a friend of
  // FilteredConfigIterator
