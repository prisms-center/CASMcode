...
namespace CASM {
  ...
  template<typename T> class SelectCommandImpl;

  /// 'casm select' command
  ///
  /// This class wraps a pointer to an implementation class SelectCommandImpl<DataObject>
  /// which does most of the work. The implementation classes are defined in the
  /// source file.
  ///
  class SelectCommand :
    public APICommand<Completer::SelectOption> {

  public:

    using ImplBase = SelectCommandImplBase;
    template<typename ConfigType> using Impl = SelectCommandImpl<ConfigType>;
