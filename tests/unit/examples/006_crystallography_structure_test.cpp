...
TEST(ExampleCrystallographyStructure, StructureConstructor) {
  ...
  //
  // In most contexts it is stored in a shared_ptr so that the BasicStructure and factor group can
  //   be shared and used in various contexts.

  CASM::xtal::BasicStructure ZrO_basic_structure = test::ZrO_prim();
  auto ZrO_shared_prim = std::make_shared<const CASM::Structure>(ZrO_basic_structure);
