...
// This introduces how to use CASM to analyze and sample the space of global
// continuous degrees of freedom (DoF), using strain as an example.

// A brief review of how CASM represents an infinite crystal and the space of
// allowed perturbations (the DoF space):
//
//   Each type of continuous DoF has:
//   - AnisoValTraits, which provides the DoF type name, a standard coordinate
//   system (the "standard
//     basis"), and specifies how values transform under application of
//     symmetry. Examples of the "standard basis" specified by AnisoValTraits
//     include:
//     - "disp" -> (dx, dy, dz) -> displacement components relative to fixed
//     laboratory frame
//     - "strain" -> (e_xx, e_yy, e_zz, sqrt(2)*e_yz, sqrt(2)*e_xz,
//     sqrt(2)*e_xy) -> tensor elements
//
//   A BasicStructure represents an infinite crystal and its allowed
//   perturbations by specifying one unit cell. BasicStructure has:
//   - a Lattice
...
//
//     Note: DoFKey is a typedef for std::string, matching
//     AnisoValTraits::name() for a particular
//           type of DoF.
//
//   Each xtal::DoFSet or xtal::SiteDoFSet has:
//   - a "DoF basis" specifying a basis which is a linear combination of the
//   standard basis defined
//     by AnisoValTraits.
//
//     Note: The DoF basis may be a subspace of the standard basis. This allows
//     specifying that for
//           a particular crystal only a subset of strain perturbations are
//           allowed, or for particular sites only particular displacements are
//           allowed. These subspaces are taken into account when determining
//           the symmetry group of the BasicStructure.
//
//   A Structure is an object that holds a (shared, const) BasicStructure and
//   its symmetry information. A Structure has:
...
//     structure" or "prim",
//           especially in the context where it is generally expected to the
//           primitive unit cell.
//
//
// The DoF Space
// -------------
//
// The total crystal degree of freedom space corresponding to a particular prim
// structure is the combination of the global continuous DoFs, local discrete
// DoF, and local continuous DoFs.
//
// - Essential symmetry information (irreducible representations) can be
// obtained for various
//   subspaces of the total crystal degree of freedom space and subgroups of the
//   prim structure's factor group.
// - The degree of freedom subspaces are primarily specified by a combination of
// DoF type (e.g.,
//   "disp", "GLstrain", "occ") and, for local DoF, which sites to include.
// - The symmetry of a degree of freedom subspace can not be higher than that of
// the prim factor
//   group, but it may have lower symmetry. It may have lower translational
//   symmetry corresponding to a particular supercell lattice periodicity. It
//   may have lower symmetry corresponding to order/disorder of local degrees of
//   freedom within a supercell. Thus, this symmetry constraint may be specified
//   by using the factor group of a particular Configuration.
//
// The DoFSpace class is used to specify a particular degree of freedom space
// and the symmetry constraints on it. DoFSpace have:
//   - dof_key (DoFKey): A string indicating which DoF type (e.g., "disp",
//   "Hstrain", "occ")
//   - config_region (ConfigEnumInput): Specifies both the symmetry group for
//   the DoFSpace (from
//     "config_region.group()") and, for local DoF, which DoF to include (DoF on
//     sites listed "config_region.sites()"). The symmetry group
//     "config_region.group()" is the factor group of the ConfigEnumInput's
//     configuration, excluding any operation that causes permutation between
//     those sites included in "config_region.sites()" and those sites excluded
//     from "config_region.sites()".
//   - dof_subspace (Eigen::MatrixXd): Allows specifying a subspace of the space
//   determined from
...
//         each of the 4 sites. Then, dof_subspace is a matrix with 12 rows and
//         12 or fewer columns.
//
//       For dof_key=="occ", and config_region.sites().size()==4, with the
//       particular sites selected having allowed occupations ["A", "B"], ["A",
//       "B", "C"], ["A, B"], and ["A", "B", "C"] :
...
//         GLstrain components. Then, dof_subspace is a matrix with 6 rows and 6
//         or fewer columns.
//
//     By default, dof_subspace is the full DoF space (identity matrix with
//     dimensions matching the full space for the particular combination of
//     config_region and dof_key).
//
//
// Sampling the DoF Space
// ----------------------
//
// For sampling strains within the full GLstrain space, one way to proceed is to
// find symmetrically unique "wedges", symmetrically unique portions of space
// that fall between high symmetry directions. Note that wedges represent an
// infinite volume of space. For example, in strain space a wedge begins at the
// origin, representing zero strain, and comprises the region of space between
// high symmetry strain directions at any magnitude.
//
// There are multiple types of wedges:
// - The SymRepTools::IrrepWedge class represents a wedge in an irreducible
// vector space (a subspace
//   of the full DoF space that is unchanged under transformation by the
//   relevant symmetry group).
// - The SymRepTools::SubWedge class represents a vector of
// SymRepTools::IrrepWedge, one from each
//   irreducible subspace of the full DoF space. Together, the IrrepWedge
//   comprising a Subwedge span the full DoF space. While its axes do span the
//   full space, the orbit of a SubWedge may not fill the full DoF space.
// - The VectorSpaceSymReport::irreducible_wedge is a vector of SubWedge whose
// orbits combined
//   together fill the full DoF space.
//
// Sampling of degree of freedom values in the full DoF space can be performed
// by sampling, in turn, values in each SubWedge comprising the irreducible
// wedge for the full space. The standard approach in CASM to enumerate degree
// of freedom values in a SubWedge is to construct a grid of points aligned with
// the subwedge axes. Especially for subwedges whose axes form acute angles,
// this grid may include many unwanted points with large magnitudes. So CASM
// also provides a "trim_corners" option which excludes any points lying outside
// the ellipsoid that passes through the maximum enumerated value lying on each
// wedge axis.
//
// The following examples show how to specify a DoF space, create a
// VectorSpaceSymReport to analyze it, and construct and use wedges to sample
// the DoF space.
...
SymGroupRep const &global_dof_symrep(Structure const &prim,
                                     DoFKey global_dof_key);

SymGroupRep const &global_dof_symrep(ConfigEnumInput const &config_input,
                                     DoFKey global_dof_key);

SymGroup make_point_group(ConfigEnumInput const &config_input);

// check if there is a solution X, for:  B = A * X
bool is_equivalent_column_space(Eigen::MatrixXcd const &A,
                                Eigen::MatrixXcd const &B, double tol);
}  // namespace enumeration_test_impl
...
TEST_F(ExampleEnumerationSimpleCubicConfigEnumStrain, VectorSpaceSymReport) {
  // This example demonstrates specifying a DoFSpace and constructing its
  // VectorSpaceSymReport

  using namespace enumeration_test_impl;  // for global_dof_symrep,
                                          // make_point_group

  // The VectorSpaceSymReport provides symmetry information about a particular
  // crystal "degree of freedom space" under particular symmetry constraints.
  // VectorSpaceSymReport has:
...
TEST_F(ExampleEnumerationSimpleCubicConfigEnumStrain, FullSpaceWedgesEnum) {
  ...
  Eigen::VectorXd max_value = Eigen::VectorXd::Constant(
      dim, _min + (n_increment - 1) * _incr + _incr / 10.);
  ...
  CASM::ConfigEnumStrain enumerator{config_input, subwedges,       min_value,
                                    max_value,    increment_value, dof_key,
                                    auto_range,   trim_corners};
  ...
  // - Each subwedge has a grid with n_increment points along each dimension, so
  // the total number
...
TEST_F(ExampleEnumerationSimpleCubicConfigEnumStrain, SubSpaceIrreps) {
  ...
  for (int i = 0; i < expected_irrep_subspace.size(); ++i) {
    // Construct subspace consisting of a single direction in an irreducible
    // subspace
  ...
  for (int i = 0; i < expected_irrep_subspace.size(); ++i) {
    for (int j = 0; j < i; ++j) {
      // Construct subspace consisting of one direction from each of two
      // irreducible subspaces
...
TEST_F(ExampleEnumerationSimpleCubicConfigEnumStrain, SubSpaceWedgesEnum) {
  ...
  for (int i = 0; i < expected_irrep_subspace.size(); ++i) {
    ...
    // Construct subspace consisting of a single direction from the irreducible
    // subspace
    ...
    // - Using min=0.2, max=1.21, increment=0.5 results in a sampling grid with
    // three points along
...
TEST_F(ExampleEnumerationSimpleCubicConfigEnumStrain,
       FullSpaceDirectGridStrainEnum) {
  // It is also possible to use ConfigEnumStrain to directly sample GLstrain on
  // any user-specified
  //   grid. This is done by creating a "dummy subwedge" which specifies the
  ...
  // To sample strains in the entire GLstrain space, we want the "dummy
  // subwedge" to span the entire
  //   6-dimensional GLstrain space. For any global DoF we can get the
  //   dimensionality of its space from the SymGroupRep.
  SymGroupRep const &strain_symrep = global_dof_symrep(config_input, dof_key);
  int dim = strain_symrep.dim();

  // To sample GLstrain on a grid spanning the entire GLstrain space, the
  // sampled_space_rank is the
  //   full dimensionality of the GLstrain space.
  int sampled_space_rank = dim;

  // In this example, use the reference GLstrain space axes for the sampling
  // grid axes. To rotate
  //   the sampling grid, change "axes" so that sampling grid axes are column
  //   vectors:
  //
  ...
  Eigen::VectorXd min_value =
      Eigen::VectorXd::Constant(sampled_space_rank, _min);
  Eigen::VectorXd max_value = Eigen::VectorXd::Constant(
      sampled_space_rank, _min + (n_increment - 1) * _incr + _incr / 10.);
  Eigen::VectorXd increment_value =
      Eigen::VectorXd::Constant(sampled_space_rank, _incr);
  ...
  EXPECT_EQ(configurations.size(),
            pow(n_increment, sampled_space_rank) * subwedges.size());
};
...
SymGroupRep const &global_dof_symrep(Structure const &prim,
                                     DoFKey global_dof_key) {
  return prim.factor_group().representation(
      prim.global_dof_symrep_ID(global_dof_key));
}

SymGroupRep const &global_dof_symrep(ConfigEnumInput const &config_input,
                                     DoFKey global_dof_key) {
  return global_dof_symrep(config_input.configuration().supercell().prim(),
                           global_dof_key);
}
...
ExampleEnumerationSimpleCubicConfigEnumStrain::
    ...
    : title("ExampleEnumerationSimpleCubicConfigEnumStrain"),
      shared_prim(std::make_shared<CASM::Structure const>(
          test::SimpleCubic_GLstrain_prim())),
  ...
  // "q1"
  Q.col(0) << 0.577350269190, 0.577350269190, 0.577350269190, 0.000000000000,
      0.000000000000, 0.000000000000;
  expected_irrep_subspace.push_back(Q.block(0, 0, 6, 1));

  // "q2, q3"
  Q.col(1) << -0.408248290464, -0.408248290464, 0.816496580928, 0.000000000000,
      0.000000000000, 0.000000000000;
  Q.col(2) << 0.707106781187, -0.707106781187, -0.000000000000, 0.000000000000,
      0.000000000000, 0.000000000000;
  expected_irrep_subspace.push_back(Q.block(0, 1, 6, 2));

  // "q4, q5, q6"
  Q.col(3) << 0.000000000000, 0.000000000000, 0.000000000000, 0.000000000000,
      0.000000000000, 1.000000000000;
  Q.col(4) << 0.000000000000, 0.000000000000, 0.000000000000, -1.000000000000,
      0.000000000000, 0.000000000000;
  Q.col(5) << 0.000000000000, 0.000000000000, 0.000000000000, 0.000000000000,
...
void ExampleEnumerationSimpleCubicConfigEnumStrain::check_irrep(
  ...
  // in the original
  //   reference space and transforms it into irreducible representation vector
  //   space:
  //
  //      v_irrep_space = irrep.trans_mat * v_reference_space
  //
  // The reference space has dimensions (vector_dim  x vector_dim)
  // An irreducible representation vector space has dimensions (vector_dim  x
  // irrep_dim) The IrrepInfo::trans_mat has dimensions (irrep_dim x vector_dim)
  //
  // For this example, GLstrain has vector_dim=6, and the irrep_dim depends on
  // the particular
  //   irreducible representation. The irreducible representations are named
  //   with two indices:
  //
  //      irrep name: irrep_<index>_<index>  // TODO: explain mult/indices
  //
  //   irrep_1_1: irrep_dim=1, irrep_axes=["q1"]
  //     q1 = 0.577350269190, 0.577350269190, 0.577350269190, 0.000000000000,
  //     0.000000000000, 0.000000000000
  //   irrep_2_1: irrep_dim=1, irrep_axes=["q2", "q3"]
  //     q2 = -0.408248290464, -0.408248290464, 0.816496580928, 0.000000000000,
  //     0.000000000000, 0.000000000000 q3 = 0.707106781187, -0.707106781187,
  //     -0.000000000000, 0.000000000000, 0.000000000000, 0.000000000000
  //   irrep_2_1: irrep_dim=1, irrep_axes=["q4", "q5", "q6"]
  //     q4 = 0.000000000000, 0.000000000000, 0.000000000000, 0.000000000000,
  //     0.000000000000, 1.000000000000 q5 = 0.000000000000, 0.000000000000,
  //     0.000000000000, -1.000000000000, 0.000000000000, 0.000000000000 q6 =
  ...
  if (irrep.irrep_dim() == 1) {  // irrep_1_1: ["q1"]
    ...
    EXPECT_EQ(is_equivalent_column_space(irrep.trans_mat.transpose(),
                                         expected_irrep_subspace[0], tol),
              true);
    ...
    EXPECT_EQ(irrep.index, 0);
    EXPECT_EQ(is_equivalent_column_space(irrep.trans_mat.transpose(),
                                         expected_irrep_subspace[2], tol),
              true);
    // TODO: IrrepInfo::directions, not currently checked
    // TODO: IrrepInfo::characters, not currently checked
  } else {
    EXPECT_EQ(true, false) << " for SimpleCubic GLstrain space, no irrep "
                              "should have irrep_dim > 3";
  }
}
