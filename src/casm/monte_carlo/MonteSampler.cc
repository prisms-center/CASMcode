...
MonteSampler::MonteSampler(const std::string &print_name, double data_prec,
                           double data_confidence, size_type data_initsize)
...
/// \brief Construct sampler that does not need to converge
///
/// \param _property_name Name of scalar property to sample, ex:
/// "formation_energy" \param print_name Name to be printed, ex:
/// "formation_energy" \param data_initsize For constructing MCData object
///
...
/// \param _property_name Name of scalar property to sample, ex:
/// "formation_energy" \param print_name Name to be printed, ex:
/// "formation_energy" \param data_prec Required precision level \param
/// data_confidence Required confidence level \param data_initsize For
/// constructing MCData object
///
ScalarMonteSampler::ScalarMonteSampler(std::string _property_name,
...
/// \param _index Index of individual element of vector property to sample, ex:
/// 10 \param print_name Name to be printed, ex: "corr(10)" \param data_initsize
/// For constructing MCData object
///
...
/// \param _property_name Name of vector property to sample, ex: "corr"
/// \param _index Index of individual element of vector property to sample, ex:
/// 10 \param print_name Name to be printed, ex: "corr(10)" \param data_prec
/// Required precision level \param data_confidence Required confidence level
...
/// \brief Construct sampler that does not need to converge
QueryMonteSampler::Formatter::Formatter(
    const DataFormatter<Configuration> &formatter)
    : m_formatter(formatter),
      m_last_sample(std::numeric_limits<MonteCounter::size_type>::max(),
                    std::numeric_limits<MonteCounter::size_type>::max()) {}

/// \brief Evaluate datum formatters, if necessary, and return result
const Eigen::VectorXd &QueryMonteSampler::Formatter::sample(
    const MonteCarlo &mc, const MonteCounter &counter) {
...
QueryMonteSampler::QueryMonteSampler(
    std::shared_ptr<QueryMonteSampler::Formatter> formatter, size_type _index,
    std::string print_name, double data_confidence, size_type data_initsize)
    : MonteSampler(print_name, data_confidence, data_initsize),
      m_index(_index),
      m_formatter(formatter) {}
...
QueryMonteSampler::QueryMonteSampler(
    std::shared_ptr<QueryMonteSampler::Formatter> formatter, size_type _index,
    std::string print_name, double data_prec, double data_confidence,
    size_type data_initsize)
    : MonteSampler(print_name, data_prec, data_confidence, data_initsize),
      m_index(_index),
      m_formatter(formatter) {}
...
/// \brief Sample data from a MonteCarlo calculation
void QueryMonteSampler::sample(const MonteCarlo &mc,
                               const MonteCounter &counter) {
...
/// \param data_initsize For constructing MCData object
///
SiteFracMonteSampler::SiteFracMonteSampler(
    size_type _index, size_type _basis_size, std::string print_name,
    double data_prec, double data_confidence, size_type data_initsize)
    : MonteSampler(print_name, data_prec, data_confidence, data_initsize),
      m_index(_index),
      m_basis_size(_basis_size) {}
...
/// \param data_initsize For constructing MCData object
///
AtomFracMonteSampler::AtomFracMonteSampler(
    size_type _index, size_type _vacancy_index, std::string print_name,
    double data_prec, double data_confidence, size_type data_initsize)
    : MonteSampler(print_name, data_prec, data_confidence, data_initsize),
      m_index(_index),
      m_vacancy_index(_vacancy_index) {}
