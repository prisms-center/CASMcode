...
///
/// This method enumerates configurations given a rage of starting values,
/// typically a range of supercells or initial configurations, possibly with
/// particular sites selected for enumeration, represented using
/// `ConfigEnumInput`.
///
...
///
/// Note:
/// - To avoid unnecessary configuration canonicalization, specialize the
/// following method:
...
/// \param make_enumerator_f A function that constructs a Configuration
/// enumerator of type
///        EnumeratorType for each value in the range `[name_value_pairs_begin,
///        name_value_pairs_end)`. Requires signature:
///            EnumeratorType make_enumerator_f(Index initial_state_index,
///                                             std::string initial_state_name,
///                                             InitialStateType const
///                                             &initial_state);
/// \param name_value_pairs_begin, name_value_pairs_end A range of pairs of name
/// (for printing
///        progress) to `InitialStateType` value (typically, but not necessarily
///        `InputType = ConfigEnumInput`). The value `InputType` is used as the
///        argument to `make_enumerator_f` to construct a series of enumerators
///        which are executed in turn.
/// \param formatter DataFormatter used to record enumerated configurations. See
/// ConfigEnumOptions
///        for output options. The ConfigEnumDataType should follow the example
///        of `ConfigEnumData` which provides the minimum required interface.
///
/// Note:
...
void enumerate_configurations(
  ...
  for (; it != name_value_pairs_end; ++it) {
    ...
    auto enumerator =
        make_enumerator_f(initial_state_index, input_name_value_pair.first,
                          input_name_value_pair.second);

    for (Configuration const &configuration : enumerator) {
      ConfigEnumDataType data{primclex,
                              initial_state_index,
      ...
      if (options.filter && !options.filter(configuration)) {
        ...
        // configuration can be directly inserted, else makes canonical before
        // inserting
        data.insert_result =
            make_canonical_and_insert(enumerator, configuration, supercell_db,
                                      configuration_db, options.primitive_only);
      }

      if (data_out_ptr && (options.output_filtered_configurations ||
                           !data.is_excluded_by_filter)) {
    ...
    log << dry_run_msg << count << " configurations"
        << " (" << (num_after - num_before) << " new, " << count_filtered
  ...
  if (data_out_ptr) {
    log << "Write enumerated configurations: "
        << options.output_options.file_path << std::endl;
    data_out_ptr.reset();
  }
  log.end_section();
}
