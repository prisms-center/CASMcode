...
TEST(ConfigDoFTest, ConstructorFail) {
  ...
  // fail construction: shared prim structure with no basis sites: cannot construct supercell or configdof

  auto shared_prim = std::make_shared<Structure const>(test::no_basis_prim());
  ASSERT_ANY_THROW(std::make_shared<Supercell const>(shared_prim, Eigen::Matrix3l::Identity()));

}

TEST(ConfigDoFTest, Constructor0) {

  // fail construction: shared prim structure with basis sites but no occupant DoF
  ...
  EXPECT_EQ(configdof.has_occupation(), true);  // TODO: note this is inconsisten with Site having no occupant DoF
...
TEST(ConfigDoFTest, Constructor1) {
  ...
  EXPECT_EQ(configdof.occupation(), Eigen::VectorXi::Zero(shared_supercell->num_sites()));
  EXPECT_EQ(configdof.global_dofs().size(), 0);
  EXPECT_EQ(configdof.local_dofs().size(), 0);
...
TEST(ConfigDoFTest, Constructor2) {
  ...
  auto shared_prim = std::make_shared<Structure const>(test::SimpleCubic_GLstrain_prim());
  auto shared_supercell = std::make_shared<Supercell const>(shared_prim, Eigen::Matrix3l::Identity());
...
TEST(ConfigDoFTest, Constructor4) {
  ...
  auto shared_prim = std::make_shared<Structure const>(test::SimpleCubic_disp_prim());
  auto shared_supercell = std::make_shared<Supercell const>(shared_prim, Eigen::Matrix3l::Identity());
...
TEST(ConfigDoFTest, Constructor5) {
  ...
  // construction: shared prim structure with ternary occupation, GLstrain, and disp DoF

  auto shared_prim = std::make_shared<Structure const>(test::FCC_ternary_strain_disp_prim());
  auto shared_supercell = std::make_shared<Supercell const>(shared_prim, Eigen::Matrix3l::Identity());
