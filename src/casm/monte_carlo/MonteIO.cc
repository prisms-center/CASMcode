...
namespace CASM {
  ...
  namespace Monte {
    ...
    GenericDatumFormatter<size_type, ConstMonteCarloPtr> MonteCarloNEquilSamplesFormatter() {
      ...
      return GenericDatumFormatter<size_type, ConstMonteCarloPtr>("N_equil_samples", "N_equil_samples", evaluator);
    }
    ...
    GenericDatumFormatter<size_type, ConstMonteCarloPtr> MonteCarloNAvgSamplesFormatter() {
      ...
      return GenericDatumFormatter<size_type, ConstMonteCarloPtr>("N_avg_samples", "N_avg_samples", evaluator);
    }

    /// \brief Print Pass number of observation
    GenericDatumFormatter<MonteCounter::size_type, std::pair<ConstMonteCarloPtr, size_type> > MonteCarloPassFormatter() {

      auto evaluator = [ = ](const std::pair<ConstMonteCarloPtr, size_type> &obs) {
        return obs.first->sample_times()[obs.second].first;
      };

      return GenericDatumFormatter<size_type, std::pair<ConstMonteCarloPtr, size_type> >("Pass", "Pass", evaluator);
    }
    ...
    GenericDatumFormatter<size_type, std::pair<ConstMonteCarloPtr, size_type> > MonteCarloStepFormatter() {
      ...
      return GenericDatumFormatter<size_type, std::pair<ConstMonteCarloPtr, size_type> >("Step", "Step", evaluator);
    }
    ...
    GenericDatumFormatter<double, std::pair<ConstMonteCarloPtr, size_type> > MonteCarloObservationFormatter(std::string prop_name) {
      ...
      return GenericDatumFormatter<double, std::pair<ConstMonteCarloPtr, size_type> >(prop_name, prop_name, evaluator);
    }
    ...
    GenericDatumFormatter<int, std::pair<ConstMonteCarloPtr, size_type> > MonteCarloOccFormatter(size_type occ_index) {
      ...
      auto evaluator = [ = ](const std::pair<ConstMonteCarloPtr, size_type> &site)->int {
        return site.first->trajectory()[site.second].occ(occ_index);
      };

      std::string header = std::string("occ(") + std::to_string(occ_index) + ")";

      return GenericDatumFormatter<int, std::pair<ConstMonteCarloPtr, size_type> >(header, header, evaluator);
    }
    ...
    void write_POSCAR_initial(const MonteCarlo &mc, size_type cond_index, Log &_log) {
      ...
      if(!fs::exists(dir.initial_state_json(cond_index))) {
        throw std::runtime_error(
          std::string("ERROR in 'write_POSCAR_initial(const MonteCarlo &mc, size_type cond_index)'\n") +
      ...
      VaspIO::PrintPOSCAR p(xtal::to_simple_structure(mc.supercell(), config_dof));
      p.sort();
      p.print(sout);
      return;
    }
    ...
    void write_POSCAR_final(const MonteCarlo &mc, size_type cond_index, Log &_log) {
      ...
      if(!fs::exists(dir.final_state_json(cond_index))) {
        throw std::runtime_error(
          std::string("ERROR in 'write_POSCAR_final(const MonteCarlo &mc, size_type cond_index)'\n") +
    ...
    void write_POSCAR_trajectory(const MonteCarlo &mc, size_type cond_index, Log &_log) {
      ...
      if(mc.settings().write_json()) {
        ...
        if(!fs::exists(filename)) {
          throw std::runtime_error(
            std::string("ERROR in 'write_POSCAR_trajectory(const MonteCarlo &mc, size_type cond_index)'\n") +
      ...
      for(size_type i = 0; i < trajectory.size(); i++) {
        ...
        VaspIO::PrintPOSCAR p(xtal::to_simple_structure(mc.supercell(), trajectory[i]));
        p.set_title(ss.str());
        p.sort();
        p.print(sout);
        sout.close();
