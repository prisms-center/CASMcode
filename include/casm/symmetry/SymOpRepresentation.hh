...
class SymOpRepresentation {
 public:
  ...
  SymOpRepresentation(const MasterSymGroup &_master_group,
                      SymGroupRepID _rep_ID, Index _op_index)
      : SymOpRepresentation(&_master_group, _rep_ID, _op_index) {}

  /// SymOpRepresentation specifies how a symmetry operation acts on a certain
  /// type of object. It is a virtual class, since we don't need to know the
  /// specifics of its behavior at higher levels of abstraction
  ...
  virtual double character() const { return NAN; }

  virtual Permutation const *permutation() const { return nullptr; }

  virtual Eigen::MatrixXd const *MatrixXd() const { return nullptr; }

  virtual SymBasisPermute const *ucc_permutation() const { return nullptr; }
  ...
  /// get array of pointers to matrix representations for representations
  /// corresponding to _rep_IDs
  Array<Eigen::MatrixXd const *> get_matrix_reps(
  ...
  void set_identifiers(const MasterSymGroup &new_group,
                       SymGroupRepID new_rep_ID);

  /// Set m_master_group, _rep_ID, and op_index
  void set_identifiers(const MasterSymGroup &new_group,
                       SymGroupRepID new_rep_ID, Index new_op_index);
  ...
  void invalidate_index() { m_op_index = -1; }

  /// Index of this operation within the master_group (equivalent to
  /// master_group_index)
  Index index() const { return m_op_index; }

  /// Index of this operation within the master_group
  Index master_group_index() const { return m_op_index; }

  /// ID of representation that this operation belongs to within the
  /// master_group
  SymGroupRepID rep_ID() const { return m_rep_ID; }

  /// Get the operation index of the inverse of this operation, using the
  /// master_group's multiplication table
  Index ind_inverse() const;

  /// Get the operation index of operation that is the product of this operation
  /// and @param RHS, using the master_group's multiplication table
 ...
 protected:
  ...
  SymOpRepresentation(MasterSymGroup const *_master_group_ptr,
                      SymGroupRepID _rep_ID, Index _op_index)
      : m_master_group(_master_group_ptr),
        m_rep_ID(_rep_ID),
        m_op_index(_op_index) {
    _set_integral_tau();
  }

  virtual SymOpRepresentation *inverse_impl() const {
    throw std::runtime_error(
        "Attempting to use unimplemented SymOpRepresentation::inverse_impl()");
    return nullptr;
  }

  virtual void _set_integral_tau() { return; }

  /// Pointer to the MasterSymGroup where prototype of this SymOp lives
  MasterSymGroup const *m_master_group;

  /// ID of this representation within the master_group.  Default is
  /// uninitialized.
...
struct SymRepIndexCompare {
  ...
  bool operator()(const SymOpRepresentation &A,
                  const SymOpRepresentation &B) const {
