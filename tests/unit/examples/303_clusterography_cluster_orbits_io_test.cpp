...
class ExampleClusterographyClusterOrbitsIO : public testing::Test {
 protected:
  void SetUp() override {
    ...
    auto shared_prim =
        std::make_shared<CASM::Structure const>(test::ZrO_prim());

    // Use the complete factor group as the generating group
    CASM::SymGroup const &generating_group = shared_prim->factor_group();

    // Cluster "max_length" cutoff values: max_length[b] is cutoff value for
    // orbit branch b
    ...
    cluster_specs = notstd::make_unique<CASM::PeriodicMaxLengthClusterSpecs>(
        ...
        CASM::alloy_sites_filter,  // filter to include sites with >1 occupant
                                   // in cluster orbits
...
// SymInfoOptions and OrbitPrinterOptions allow controlling some aspects of what
// is printed and how:
//
// // Controls printing of symmetry operations
// struct SymInfoOptions {
//   COORD_TYPE coord_type = FRAC;   // how symmetry operation coordinates are
//   printed double tol = CASM::TOL; Index prec = 7; bool print_matrix_tau =
//   false;  // print symmetry operations matrix and translation vector
// };
//
...
//   bool print_equivalence_map = false;    // print symmetry operations in the
//   equivalence map bool print_invariant_group = false;      // print symmetry
//   operations in the cluster invariant group
// };
...
TEST_F(ExampleClusterographyClusterOrbitsIO, PrintToStream) {
  ...
  // The "print_clust" function prints a range of orbits to a stream, using the
  // provided options
...
TEST_F(ExampleClusterographyClusterOrbitsIO, ReadWriteJSON) {
  ...
  // The "write_clust" function writes a range of orbits to JSON, using the
  // provided options
  ...
  // To read the orbits back in from JSON we need the prototypes, prim,
  // generating group,
  //   and sym_compare functor
  std::vector<CASM::PrimPeriodicIntegralClusterOrbit> orbits_in;
  read_clust(std::back_inserter(orbits_in), json, *cluster_specs->shared_prim,
             cluster_specs->shared_prim->factor_group(),
             cluster_specs->sym_compare);

  EXPECT_EQ(orbits_in.size(), 6);

  // The full orbit JSON object can be read in similarly (only the prototypes
  // get used)
  std::vector<CASM::PrimPeriodicIntegralClusterOrbit> full_orbit_in;
  read_clust(std::back_inserter(full_orbit_in), full_orbit_json,
