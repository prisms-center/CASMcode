...
///   at step 'm_it_step' by calling 'goto_step'. Then the enumerator's
///   'current' object reference can be safely returned.
/// - Inherits from InputEnumIteratorBase so that it can be held by
/// InputEnumIterator
///
template <typename ValueType, bool IsConst = true>
class RandomAccessEnumIteratorBase
    : public InputEnumIteratorBase<ValueType, IsConst> {
 ...
 public:
  ...
  RandomAccessEnumIteratorBase(
      RandomAccessEnumeratorBase<ValueType, IsConst> &enumerator, bool is_end)
  ...
  step_type step() const override { return m_it_step; }
 ...
 private:
  ...
  /// boost::iterator_facade implementation
  void increment() override { ++m_it_step; }

  /// boost::iterator_facade implementation
  void decrement() { --m_it_step; }

  /// boost::iterator_facade implementation
  void advance(step_type n) { m_it_step += n; }
  ...
  RandomAccessEnumeratorBase<ValueType, IsConst> &_enum() {
    return static_cast<RandomAccessEnumeratorBase<ValueType, IsConst> &>(
        *_enum_ptr());
  }
...
class RandomAccessEnumIterator :
    ...
    public boost::iterator_facade<RandomAccessEnumIterator<ValueType, IsConst>,
                                  ValueType, boost::random_access_traversal_tag,
                                  CASM_TMP::ConstSwitch<IsConst, ValueType> &,
                                  long> {
 public:
  typedef typename RandomAccessEnumIteratorBase<ValueType, IsConst>::step_type
      step_type;
  typedef ValueType value_type;
  typedef typename RandomAccessEnumIteratorBase<ValueType, IsConst>::reference
      reference;
  typedef step_type difference_type;
  ...
  /// Construct iterator
  RandomAccessEnumIterator(
      const RandomAccessEnumIteratorBase<ValueType, IsConst> &it)
      : m_ptr(notstd::clone(it)) {}

  step_type step() const { return m_ptr->step(); }

  jsonParser source() const { return m_ptr->source(); }
 ...
 private:
  ...
  reference dereference() const { return m_ptr->dereference(); }

  void increment() { m_ptr->increment(); }

  void decrement() { m_ptr->decrement(); }

  void advance(step_type n) { m_ptr->advance(n); }

  difference_type distance_to(const RandomAccessEnumIterator &B) const {
    return m_ptr->distance_to(*(B.m_ptr));
  }

  notstd::cloneable_ptr<RandomAccessEnumIteratorBase<ValueType, IsConst> >
      m_ptr;
};

/// \brief Base class for implementing specialized random access enumerators
///
/// InputEnumerator allow random access iteration over some objects of
/// ValueType. The specialized class should document any guaranteed properties
/// of the enumerated objects. For instance, some enumerators may guarantee that
...
/// - constructors
///   - Must call _initialize or _set_current_ptr to set pointer to first valid
///   object
...
///     - const std::string traits<MyDerivedEnumClass>::name =
///     "MyDerivedEnumClass";
...
///     - \code
///       const std::string traits<MyDerivedEnumClass>::name =
///       "MyDerivedEnumClass"; const std::string
///       traits<MyDerivedEnumClass>::help =
///         "MyDerivedEnumClass: \n\n"
///
...
///         "  ... \n";
///
///       int EnumInterface<MyDerivedEnumClass>::run(PrimClex &primclex, const
///       jsonParser &kwargs) const {
///         ...
///         implementation to run a MyDerivedEnumClass enumerator constructed
///         using the JSON input in 'kwargs' and store & save results in
///         primclex
///         ...
///         return (returncode);
///       }
...
template <typename ValueType, bool IsConst = true>
class RandomAccessEnumeratorBase
    : public InputEnumeratorBase<ValueType, IsConst> {
 ...
 public:
  ...
  RandomAccessEnumeratorBase(size_type size = 0)
      : InputEnumeratorBase<ValueType, IsConst>(), m_size(size) {}
  ...
  /// Reference to first element in enumerator
  reference initial() { return front(); }

  /// Reference to last element in enumerator
  reference final() { return back(); }
 ...
 protected:
  ...
  /// Set size value
  void _set_size(size_type val) { m_size = val; }
 ...
 private:
  ...
  void increment() override { goto_step(step() + 1); }
...
class RandomAccessEnumerator {
 public:
  typedef typename RandomAccessEnumeratorBase<ValueType, IsConst>::step_type
      step_type;
  typedef ValueType value_type;
  typedef typename RandomAccessEnumeratorBase<ValueType, IsConst>::reference
      reference;
  typedef RandomAccessEnumIterator<ValueType, IsConst> iterator;
  typedef std::reverse_iterator<iterator> reverse_iterator;
  typedef typename RandomAccessEnumeratorBase<ValueType, IsConst>::size_type
      size_type;

  RandomAccessEnumerator() {}

  RandomAccessEnumerator(
      std::unique_ptr<RandomAccessEnumeratorBase<ValueType, IsConst> > e)
      : ptr(e.release()) {}
  ...
  reference current() const { return ptr->current(); }

  /// Increments with each enumerated object
  step_type step() const { return ptr->step(); }

  /// Returns false if enumeration is complete
  bool valid() const { return ptr->valid(); }

  jsonParser source(step_type step) const { return ptr->source(step); }
