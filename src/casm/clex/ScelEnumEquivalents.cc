...
struct MakeInvariantSubgroup {
  ...
  SymOpOutputIterator operator()(const Supercell &scel, SymOpIterator begin,
                                 SymOpIterator end,
                                 SymOpOutputIterator result) {
    return sym::invariant_subgroup(std::vector<SymOp>(begin, end),
                                   scel.lattice(), result);
  }
};
...
ScelEnumEquivalents::ScelEnumEquivalents(const Supercell &scel)
    : EnumEquivalents<Supercell, std::vector<SymOp>::const_iterator, SymOp,
                      SymRepIndexCompare>(
          scel.canonical_form(), scel.prim().point_group().begin(),
          scel.prim().point_group().end(), MakeInvariantSubgroup()) {}
