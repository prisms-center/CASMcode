...
/// \brief Print Temperature for any class MonteType with valid 'double
/// MonteType::conditions().temperature()'
...
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloTFormatter() {
  ...
  return GenericDatumFormatter<double, ConstMonteCarloPtr>("T", "Temperature",
                                                           evaluator);
}

/// \brief Print Beta for any class MonteType with valid 'double
/// MonteType::conditions().beta()'
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloBetaFormatter() {
  ...
  return GenericDatumFormatter<double, ConstMonteCarloPtr>("Beta", "Beta",
                                                           evaluator);
}

/// \brief Print param_chem_pot(x) for any class MonteType with valid 'double
/// MonteType::conditions().param_chem_pot()(index)'
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr>
MonteCarloParamChemPotFormatter(const MonteType &mc, int index) {
  auto evaluator = [=](const ConstMonteCarloPtr &mc) {
    ConstMonteCarloPtr ptr = mc;
    return static_cast<const MonteType *>(ptr)->conditions().param_chem_pot()(
        index);
  };
  std::string header = std::string("param_chem_pot(") +
                       CompositionConverter::comp_var(index) + ")";
  return GenericDatumFormatter<double, ConstMonteCarloPtr>(header, header,
                                                           evaluator);
}

/// \brief Print chem_pot(N) for any class MonteType with valid 'double
/// MonteType::conditions().chem_pot(index)'
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloChemPotFormatter(
    const MonteType &mc, int index) {
  auto evaluator = [=](const ConstMonteCarloPtr &mc) {
    ConstMonteCarloPtr ptr = mc;
    return static_cast<const MonteType *>(ptr)->conditions().chem_pot(index);
  };
  std::string header = std::string("chem_pot(") +
                       struc_molecule_name(mc.primclex().prim())[index] + ")";
  return GenericDatumFormatter<double, ConstMonteCarloPtr>(header, header,
                                                           evaluator);
}

/// \brief Print comp(x) for any class MonteType with valid 'double
/// MonteType::conditions().param_composition()(index)'
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloCompFormatter(
    const MonteType &mc, int index) {
  auto evaluator = [=](const ConstMonteCarloPtr &mc) {
    ConstMonteCarloPtr ptr = mc;
    return static_cast<const MonteType *>(ptr)
        ->conditions()
        .param_composition()(index);
  };
  std::string header =
      std::string("comp(") + CompositionConverter::comp_var(index) + ")";
  return GenericDatumFormatter<double, ConstMonteCarloPtr>(header, header,
                                                           evaluator);
}

/// \brief Print comp(x) for any class MonteType with valid 'double
/// MonteType::conditions().mol_composition()(index)'
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloCompNFormatter(
    const MonteType &mc, int index) {
  auto evaluator = [=](const ConstMonteCarloPtr &mc) {
    ConstMonteCarloPtr ptr = mc;
    return static_cast<const MonteType *>(ptr)->conditions().mol_composition()(
        index);
  };
  std::string header = std::string("comp_n(") +
                       xtal::struc_molecule_name(mc.primclex().prim())[index] +
                       ")";
  return GenericDatumFormatter<double, ConstMonteCarloPtr>(header, header,
                                                           evaluator);
}

/// \brief Print site_frac(x) for any class MonteType with valid
/// 'Eigen::VectorXd MonteType::comp_n'
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloSiteFracFormatter(
    const MonteType &mc, int index) {
  auto evaluator = [=](const ConstMonteCarloPtr &mc) {
  ...
  std::string header = std::string("site_frac(") +
                       struc_molecule_name(mc.primclex().prim())[index] + ")";
  return GenericDatumFormatter<double, ConstMonteCarloPtr>(header, header,
                                                           evaluator);
}

/// \brief Print atom_frac(X) for any class MonteType with valid
/// 'Eigen::VectorXd MonteType::comp_n'
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloAtomFracFormatter(
    const MonteType &mc, int index) {
  auto evaluator = [=](const ConstMonteCarloPtr &mc) {
    const MonteType *mc_ptr = static_cast<const MonteType *>(mc);
    Eigen::VectorXd comp_n = mc_ptr->comp_n();
  ...
  std::string header = std::string("atom_frac(") +
                       struc_molecule_name(mc.primclex().prim())[index] + ")";
  return GenericDatumFormatter<double, ConstMonteCarloPtr>(header, header,
                                                           evaluator);
}
...
///
/// - Heat capacity (per unit cell) = cov(potential_energy,
/// potential_energy)*N/(k*T*T)
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr>
...
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloSuscXFormatter(
    std::string comp_var_i, std::string comp_var_j) {
  ...
  std::string header =
      std::string("susc_x(" + comp_var_i + "," + comp_var_j + ")");
...
/// - Heat capacity (per unit cell) = cov(x_i, x_j)*N/(k*T)
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr> MonteCarloSuscNFormatter(
    std::string species_i, std::string species_j) {
  ...
  std::string header =
      std::string("susc_n(" + species_i + "," + species_j + ")");
...
/// \arg species_i: First species "A", "B", "C", ...
///
/// - thermo-chemical susceptibility (per unit cell) = cov(potential_energy,
/// x_i)*N/(k*T)
template <typename MonteType>
GenericDatumFormatter<double, ConstMonteCarloPtr>
...
/// of the latest run
template <typename MonteType>
void write_results(const MonteSettings &settings, const MonteType &mc,
                   Log &_log) {
  try {
