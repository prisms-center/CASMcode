...
namespace CASM {
  ...
  struct BasisFunctionSpecs {
    ...
    /// \param _global_max_poly_order  Maximum polynomial order for cluster basis functions of
    ///     continuous DoF. Use -1 for default behaviour: max_poly_order == cluster size.
    /// \param _orbit_branch_max_poly_order  Allows specifying the max_poly_order per orbit
    ///      branch. The key is the orbit branch index (equal to number of sites in a cluster), and
    ///      the value is the maximum polynomial order for orbits in that branch.
    /// \param _include_functions  Allows specifying that only ClexBasis functions with these
    ///      indices should be written in the produced Clexulator. Generally, this requires
    ///      generating the full ClexBasis once (without writing the Clexulator), inspecting the
    ///      basis functions, and then specifying this value and re-generating the ClexBasis.
    /// \param _exclude_functions  Allows specifying that ClexBasis functions with these
    ///      indices should be not written in the produced Clexulator. Generally, this requires
    ///      generating the full ClexBasis once (without writing the Clexulator), inspecting the
    ///      basis functions, and then specifying this value and re-generating the ClexBasis.
    ...
    /// Use -1 for context-dependent default behaviour (usually max_poly_order == orbit branch cluster size)
    /// The global value can be overriden per orbit branch with orbit_branch_max_poly_order
    ...
    /// If not empty, specifies that only ClexBasis functions with these indices should be included
    /// in the produced Clexulator.
    std::vector<Index> include_functions;

    /// If not empty, specifies that ClexBasis functions with these indices should be excluded
    /// from the produced Clexulator.
