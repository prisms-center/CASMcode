...
namespace CASM {
  ...
  ClexBasisWriter::ClexBasisWriter(Structure const &_prim, ParamPackMixIn const &_param_pack_mix_in) {
  ...
  namespace ClexBasisWriter_impl {
    ...
    std::string clexulator_member_declarations(std::string const &class_name,
      ...
      for(auto const &specialization : _param_pack_mix_in.scalar_specializations()) {
        ...
        ss <<
           ...
           indent << "// typedef for method pointers of scalar type " << specialization.second << "\n" <<
           indent << "typedef " << specialization.second << " (" << class_name << "::*BasisFuncPtr_" << ispec << ")() const;\n\n" <<

           indent << "// typedef for method pointers\n" <<
           indent << "typedef " << specialization.second << " (" << class_name << "::*DeltaBasisFuncPtr_" << ispec << ")(int, int) const;\n\n" <<

           indent << "// array of pointers to member functions for calculating basis functions of scalar type " << specialization.second << "\n" <<
           indent << "BasisFuncPtr_" << ispec << " m_orbit_func_table_" << ispec << "[" << N_corr << "];\n\n" <<

           indent << "// array of pointers to member functions for calculating flower functions of scalar type " << specialization.second << "\n" <<
           indent << "BasisFuncPtr_" << ispec << " m_flower_func_table_" << ispec << "[" << N_branch << "][" << N_corr << "];\n\n" <<
      ...
      ss <<
         indent << "//ClexParamPack allocation for evaluated correlations \n" <<
    ...
    std::string clexulator_private_method_declarations(std::string const &class_name,
      ...
      ss <<
         ...
         indent << "/// \\brief Calculate contribution to global correlations from one unit cell " <<
         indent << "/// Result is recorded in double array starting at corr_begin\n" <<
         indent << "void _calc_global_corr_contribution(double *corr_begin) const override;\n\n" <<

         indent << "/// \\brief Calculate contribution to select global correlations from one unit cell into ClexParamPack\n" <<
         indent << "/// Result is recorded in ClexParamPack\n" <<
         indent << "void _calc_restricted_global_corr_contribution(size_type const *ind_list_begin, size_type const *ind_list_end) const override;\n\n" <<
         ...
         indent << "/// For local clexulators, 'nlist_ind' ranges over all sites in the neighborhood\n" <<
         indent << "/// Result is recorded in ClexParamPack\n" <<
         indent << "void _calc_restricted_point_corr(int nlist_ind, size_type const *ind_list_begin, size_type const *ind_list_end) const override;\n\n" <<
         ...
         indent << "/// For local clexulators, 'nlist_ind' ranges over all sites in the neighborhood\n" <<
         indent << "/// Result is recorded in ClexParamPack\n" <<
         indent << "void _calc_delta_point_corr(int nlist_ind, int occ_i, int occ_f) const override;\n\n" <<
         ...
         indent << "/// For local clexulators, 'nlist_ind' ranges over all sites in the neighborhood\n" <<
         indent << "/// Result is recorded in ClexParamPack\n" <<
         indent << "void _calc_restricted_delta_point_corr(int nlist_ind, int occ_i, int occ_f, size_type const *ind_list_begin, size_type const *ind_list_end) const override;\n\n" <<
    ...
    std::string clexulator_interface_declaration(std::string const &class_name,
      ...
      ss <<
         ...
         indent << "/// \\brief Calculate contribution to global correlations from one unit cell\n" <<
         indent << "void " << class_name << "::_calc_global_corr_contribution() const {\n" <<
      ...
      ss <<
         ...
         indent << "/// \\brief Calculate contribution to select global correlations from one unit cell\n" <<
         indent << "void " << class_name << "::_calc_restricted_global_corr_contribution(size_type const *ind_list_begin, size_type const *ind_list_end) const {\n" <<
         indent << "  m_params.pre_eval();\n";
      ...
      ss <<
         ...
         indent << "/// \\brief Calculate point correlations about basis site 'nlist_ind'\n" <<
         indent << "void " << class_name << "::_calc_point_corr(int nlist_ind) const {\n" <<
         indent << "  m_params.pre_eval();\n";
      ...
      ss <<
         ...
         indent << "/// \\brief Calculate select point correlations about basis site 'nlist_ind'\n" <<
         indent << "void " << class_name << "::_calc_restricted_point_corr(int nlist_ind, size_type const *ind_list_begin, size_type const *ind_list_end) const {\n" <<
         indent << "  m_params.pre_eval();\n";
      ...
      ss <<
         ...
         indent << "/// \\brief Calculate the change in point correlations due to changing an occupant\n" <<
         indent << "void " << class_name << "::_calc_delta_point_corr(int nlist_ind, int occ_i, int occ_f) const {\n" <<
         indent << "  m_params.pre_eval();\n";
      ...
      ss <<
         ...
         indent << "/// \\brief Calculate the change in select point correlations due to changing an occupant\n" <<
         indent << "void " << class_name << "::_calc_restricted_delta_point_corr(int nlist_ind, int occ_i, int occ_f, size_type const *ind_list_begin, size_type const *ind_list_end) const {\n" <<
         indent << "  m_params.pre_eval();\n";
