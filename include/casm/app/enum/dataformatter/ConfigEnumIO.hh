...
namespace CASM {
  ...
  //   - Note that `make_datum_formatter_adapter` has overrides that can change the name if that
  //     is appropriate.
  ...
  namespace adapter {
    ...
    struct Adapter<Configuration, ConfigEnumData<EnumeratorType, InitialStateType>> {
      Configuration const &operator()(ConfigEnumData<EnumeratorType, InitialStateType> const &adaptable) const;
    };

    template<typename EnumeratorType, typename InitialStateType>
    struct Adapter<Supercell, ConfigEnumData<EnumeratorType, InitialStateType>> {
      Supercell const &operator()(ConfigEnumData<EnumeratorType, InitialStateType> const &adaptable) const;
    };
  }
  ...
  namespace ConfigEnumIO {
    ...
    /// Template that may be specialized by enumerator type to get the current normal coordinate and
    /// enable the `normal_coordinate` DatumFormatter. Will throw if no specialization exists.
